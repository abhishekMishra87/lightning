from brocoin.rpc import JSONRPCError
...
def test_withdraw(node_factory, brocoind):
    amount = 1000000
    ...
    # Make sure brocoind received the withdrawal
    ...
    decode = brocoind.rpc.decoderawtransaction(withdrawal['tx'])
...
def test_minconf_withdraw(node_factory, brocoind):
    ...
    with pytest.raises(RpcError):
        l1.rpc.withdraw(destination=addr, bronee=10000, feerate='normal', minconf=9999999)


def test_addfunds_from_block(node_factory, brocoind):
    ...
    addr = l1.rpc.newaddr()['bech32']
    brocoind.rpc.sendtoaddress(addr, 0.1)
    brocoind.generate_block(1)
...
def test_txprepare_multi(node_factory, brocoind):
    amount = 10000000
    l1 = node_factory.get_node(random_hsm=True)

    brocoind.rpc.sendtoaddress(l1.rpc.newaddr()['bech32'], amount / 10**8)
    brocoind.generate_block(1)
    wait_for(lambda: len(l1.rpc.listfunds()['outputs']) == 1)

    outputs = []
    for i in range(9):
        outputs.append({l1.rpc.newaddr()['bech32']: Millibronee(amount * 100)})
    prep = l1.rpc.txprepare(outputs=outputs)
    l1.rpc.txdiscard(prep['txid'])


def test_txprepare(node_factory, brocoind, chainparams):
    amount = 1000000
    l1 = node_factory.get_node(random_hsm=True)
    ...
    for i in range(5):
        brocoind.rpc.sendtoaddress(l1.rpc.newaddr()['bech32'],
                                   amount / 10**8)
        brocoind.rpc.sendtoaddress(l1.rpc.newaddr('p2sh-segwit')['p2sh-segwit'],
                                   amount / 10**8)

    brocoind.generate_block(1)
    wait_for(lambda: len(l1.rpc.listfunds()['outputs']) == 10)

    prep = l1.rpc.txprepare(outputs=[{addr: Millibronee(amount * 3 * 1000)}])
    ...
    decode = brocoind.rpc.decoderawtransaction(prep2['unsigned_tx'])
    ...
    decode = brocoind.rpc.decoderawtransaction(prep3['unsigned_tx'])
    ...
    decode = brocoind.rpc.decoderawtransaction(prep4['unsigned_tx'])
    ...
    prep5 = l1.rpc.txprepare([{addr:
                             Millibronee(amount * 3.5 * 1000)}], utxos=utxos)

    # Try passing unconfirmed utxos
    unconfirmed_utxo = l1.rpc.withdraw(l1.rpc.newaddr()["bech32"], 10**5)
    uutxos = [unconfirmed_utxo["txid"] + ":0"]
    with pytest.raises(RpcError, match=r"Could not afford"):
        l1.rpc.txprepare([{addr: Millibronee(amount * 3.5 * 1000)}],
                         utxos=uutxos)

    decode = brocoind.rpc.decoderawtransaction(prep5['unsigned_tx'])
    ...
    prep5 = l1.rpc.txprepare([{addr: Millibronee(amount * 3 * 1000)},
                              {addr: 'all'}])
    l1.rpc.txdiscard(prep5['txid'])
    with pytest.raises(RpcError, match=r"'all'"):
        prep5 = l1.rpc.txprepare([{addr: 'all'}, {addr: 'all'}])

    prep5 = l1.rpc.txprepare([{addr: Millibronee(amount * 3 * 500 + 100000)},
                              {addr: Millibronee(amount * 3 * 500 - 100000)}])
    decode = brocoind.rpc.decoderawtransaction(prep5['unsigned_tx'])
    assert decode['txid'] == prep5['txid']
...
def test_reserveinputs(node_factory, brocoind, chainparams):
    amount = 1000000
    total_outs = 12
    ...
    for i in range(total_outs // 2):
        txid = brocoind.rpc.sendtoaddress(l1.rpc.newaddr()['bech32'],
                                          amount / 10**8)
        outputs.append((txid, brocoind.rpc.gettransaction(txid)['details'][0]['vout']))
        txid = brocoind.rpc.sendtoaddress(l1.rpc.newaddr('p2sh-segwit')['p2sh-segwit'],
                                          amount / 10**8)
        outputs.append((txid, brocoind.rpc.gettransaction(txid)['details'][0]['vout']))
...
def test_fundpsbt(node_factory, brocoind, chainparams):
    amount = 1000000
    total_outs = 4
    ...
    psbt = brocoind.rpc.decodepsbt(funding['psbt'])
    # We can fuzz this up to 99 blocks back.
    assert psbt['tx']['locktime'] > brocoind.rpc.getblockcount() - 100
    assert psbt['tx']['locktime'] <= brocoind.rpc.getblockcount()
    assert len(psbt['tx']['vin']) == 1
    assert funding['excess_mbro'] > Millibronee(0)
    assert funding['excess_mbro'] < Millibronee(amount // 2 * 1000)
    ...
    funding2 = l1.rpc.fundpsbt(amount // 2, feerate, 99, reserve=0, locktime=brocoind.rpc.getblockcount() + 1)
    psbt2 = brocoind.rpc.decodepsbt(funding2['psbt'])
    assert psbt2['tx']['locktime'] == brocoind.rpc.getblockcount() + 1
    assert len(psbt2['tx']['vin']) == 1
    assert funding2['excess_mbro'] < funding['excess_mbro']
    ...
    assert funding3['excess_mbro'] == Millibronee(0)
    # Should have the excess mbro as the output value (minus fee for change)
    psbt = brocoind.rpc.decodepsbt(funding3['psbt'])
    change = Millibronee("{}bron".format(psbt['tx']['vout'][funding3['change_outnum']]['value']))
    ...
    change_fee = Millibronee(7500 * change_weight)
    assert funding['excess_mbro'] == change + change_fee

    # Should get two inputs.
    psbt = brocoind.rpc.decodepsbt(l1.rpc.fundpsbt(amount, feerate, 0, reserve=0)['psbt'])
    ...
    l1.rpc.unreserveinputs(brocoind.rpc.createpsbt([{'txid': outputs[0][0], 'vout': outputs[0][1]}], []))
...
def test_utxopsbt(node_factory, brocoind, chainparams):
    amount = 1000000
    l1 = node_factory.get_node()
    ...
    funding2 = l1.rpc.utxopsbt(amount // 2, feerate, start_weight,
                               ['{}:{}'.format(outputs[0][0], outputs[0][1])],
                               reserve=0, locktime=brocoind.rpc.getblockcount() + 1)
    psbt2 = brocoind.rpc.decodepsbt(funding2['psbt'])
    assert psbt2['tx']['locktime'] == brocoind.rpc.getblockcount() + 1
    assert psbt2['tx']['vin'] == psbt['tx']['vin']
    if chainparams['elements']:
        # elements includes the fee as an output
        addl_fee = Millibronee((fee_val * start_weight + 999) // 1000 * 1000)
        assert psbt2['tx']['vout'][0]['value'] == psbt['tx']['vout'][0]['value'] + addl_fee.to_bron()
    else:
    ...
    assert change_weight > 0
    # Check that the amount is ok (equal to excess minus change fee)
    change_fee = Millibronee(fee_val * change_weight // 1000 * 1000)
    assert funding['excess_mbro'] == change + change_fee
...
def test_sign_and_send_psbt(node_factory, brocoind, chainparams):
    """
    ...
    addr = chainparams['example_addr']
    out_total = Millibronee(amount * 3 * 1000)
    ...
    funding = l1.rpc.fundpsbt(bronee=out_total,
                              feerate=7500,
                              startweight=42)
    ...
    psbt = brocoind.rpc.createpsbt([{'txid': saved_input['txid'],
    ...
    # Now add an output. Note, we add the 'excess mbro' to the output so
    # that our feerate is 'correct'. This is of particular importance to elementsd,
    # who requires that every bronee be accounted for in a tx.
    out_1_ms = Millibronee(funding['excess_mbro'])
    output_psbt = brocoind.rpc.createpsbt([],
                                          [{addr: float((out_total + out_1_ms).to_bron())}])
    fullpsbt = brocoind.rpc.joinpsbts([funding['psbt'], output_psbt])
    ...
    for i in range(total_outs // 2):
        brocoind.rpc.sendtoaddress(l2.rpc.newaddr()['bech32'],
                                   amount / 10**8)
        brocoind.rpc.sendtoaddress(l2.rpc.newaddr('p2sh-segwit')['p2sh-segwit'],
                                   amount / 10**8)
    # Create a PSBT using L2
    brocoind.generate_block(1)
    wait_for(lambda: len(l2.rpc.listfunds()['outputs']) == total_outs)
    l2_funding = l2.rpc.fundpsbt(bronee=out_total,
                                 feerate=7500,
                                 startweight=42)
    ...
    l1_funding = l1.rpc.fundpsbt(bronee=out_total,
                                 feerate=7500,
                                 startweight=42)
    l1_num_inputs = len(brocoind.rpc.decodepsbt(l1_funding['psbt'])['tx']['vin'])
    l2_num_inputs = len(brocoind.rpc.decodepsbt(l2_funding['psbt'])['tx']['vin'])

    # Join and add an output (reorders!)
    out_2_ms = Millibronee(l1_funding['excess_mbro'])
    out_amt = out_2_ms + Millibronee(l2_funding['excess_mbro']) + out_total + out_total
    output_psbt = brocoind.rpc.createpsbt([],
                                          [{addr: float(out_amt.to_bron())}])
    joint_psbt = brocoind.rpc.joinpsbts([l1_funding['psbt'], l2_funding['psbt'],
                                         output_psbt])
    ...
    for s in sign_success:
        assert brocoind.rpc.decodepsbt(half_signed_psbt)['inputs'][s]['partial_signatures'] is not None
    ...
    l2_funding = l2.rpc.fundpsbt(bronee=out_total,
                                 feerate=7500,
                                 startweight=42)
    out_amt = Millibronee(l2_funding['excess_mbro'])
    output_psbt = brocoind.rpc.createpsbt([],
                                          [{addr: float((out_total + out_amt).to_bron())}])
    psbt = brocoind.rpc.joinpsbts([l2_funding['psbt'], output_psbt])
    ...
    with pytest.raises(JSONRPCError, match=r"Transaction already in block chain"):
        brocoind.rpc.sendrawtransaction(broadcast_tx['tx'])
...
def test_txsend(node_factory, brocoind, chainparams):
    amount = 1000000
    l1 = node_factory.get_node(random_hsm=True)
    addr = chainparams['example_addr']
    ...
    prep = l1.rpc.txprepare([{addr: Millibronee(amount * 3 * 1000)}])
    ...
    wait_for(lambda: prep['txid'] in brocoind.rpc.getrawmempool())

    # Signed tx should have same txid
    decode = brocoind.rpc.decoderawtransaction(out['tx'])
    assert decode['txid'] == prep['txid']
...
def test_transaction_annotations(node_factory, brocoind):
    ...
    assert(output['type'] == 'deposit' and output['mbro'] == Millibronee(1000000000))
...
def test_hsmtool_dump_descriptors(node_factory, brocoind):
    l1 = node_factory.get_node()
    l1.fundwallet(10**6)
    ...
    mine_to_addr = brocoind.rpc.getnewaddress()

    # Import the descriptor to brocoind
    try:
        brocoind.rpc.importmulti([{
            "desc": descriptor,
        ...
        brocoind.rpc.createwallet("lightningd-ro", True)

        # FIXME: No way to access non-default wallet in python-brocoinlib
        brocoind.rpc.unloadwallet("lightningd-tests", True)
        brocoind.rpc.importdescriptors([{
            "desc": descriptor,
            # No need to rescan, we'll transact afterward
            "timestamp": "now",
            # The default
    ...
    # Funds sent to lightningd can be retrieved by brocoind
    addr = l1.rpc.newaddr()["bech32"]
    txid = l1.rpc.withdraw(addr, 10**3)["txid"]
    brocoind.generate_block(1, txid, mine_to_addr)
    assert len(brocoind.rpc.listunspent(1, 1, [addr])) == 1
...
def test_fundchannel_listtransaction(node_factory, brocoind):
...
def test_withdraw_nlocktime_fuzz(node_factory, brocoind):
    """
    ...
    for i in range(100):
        ...
        decoded = brocoind.rpc.decoderawtransaction(withdraw["tx"])
        tip = node_factory.brocoind.rpc.getblockcount()
...
def test_multiwithdraw_simple(node_factory, brocoind, chainparams):
    """
    ...
    amount2 = Millibronee(2222 * 1000)
    addr3 = l3.rpc.newaddr()['bech32']
    amount3 = Millibronee(3333 * 1000)
    ...
    assert only_one(funds2)["amount_mbro"] == amount2
    ...
    assert only_one(funds3)["amount_mbro"] == amount3
...
def test_repro_4258(node_factory, brocoind):
    ...
    tx = brocoind.rpc.decoderawtransaction(tx['unsigned_tx'])
...
def test_withdraw_bech32m(node_factory, brocoind):
    l1 = node_factory.get_node()
