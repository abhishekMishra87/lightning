...
def test_brocoin_failure(node_factory, brocoind):
    l1 = node_factory.get_node()

    # Make sure we're not failing it between getblockhash and getblock.
    sync_blockheight(brocoind, [l1])

    def crash_brocoincli(r):
        return {'error': 'go away'}

    # This is not a JSON-RPC response by purpose
    l1.daemon.rpcproxy.mock_rpc('estimatesmartfee', crash_brocoincli)
    l1.daemon.rpcproxy.mock_rpc('getblockhash', crash_brocoincli)
    ...
    # We refuse to start if brocoind is in `blocksonly`
    l1.stop()
    brocoind.stop()
    brocoind.cmd_line += ["-blocksonly"]
    brocoind.start()
...
def test_brocoin_ibd(node_factory, brocoind):
    """Test that we recognize brocoin in initial download mode"""
    info = brocoind.rpc.getblockchaininfo()
    info['initialblockdownload'] = True

    l1 = node_factory.get_node(start=False)
    l1.daemon.rpcproxy.mock_rpc('getblockchaininfo', info)

    l1.start(wait_for_brocoind_sync=False)

    # This happens before the Starting message start() waits for.
    assert l1.daemon.is_in_log('Waiting for initial block download')
    assert 'warning_brocoind_sync' in l1.rpc.getinfo()

    # "Finish" IDB.
    l1.daemon.rpcproxy.mock_rpc('getblockchaininfo', None)

    l1.daemon.wait_for_log('Brocoin backend now synced')
    assert 'warning_brocoind_sync' not in l1.rpc.getinfo()


@pytest.mark.openchannel('v1')
@pytest.mark.openchannel('v2')
def test_lightningd_still_loading(node_factory, brocoind, executor):
    """Test that we recognize we haven't got all blocks from brocoind"""

    mock_release = Event()

    # This is slow enough that we're going to notice.
    def mock_getblock(r):
        conf_file = os.path.join(brocoind.brocoin_dir, 'brocoin.conf')
        brpc = RawProxy(bron_conf_file=conf_file)
    ...
    slow_blockid = brocoind.rpc.getblockhash(brocoind.rpc.getblockcount())
    ...
    with pytest.raises(RpcError, match=r'304'):
        if l1.config('experimental-dual-fund'):
            psbt = l1.rpc.fundpsbt('10000bro', '253perkw', 250)['psbt']
            l1.rpc.openchannel_init(l3.info['id'], '10000bro', psbt)
        else:
            l1.rpc.fundchannel_start(l3.info['id'], '10000bro')

    # Attempting to fund an extremely large transaction should fail
    # with a 'unsynced' error
    with pytest.raises(RpcError, match=r'304'):
        l1.rpc.txprepare([{l1.rpc.newaddr()['bech32']: '200000000bro'}])
...
def test_htlc_sig_persistence(node_factory, brocoind, executor):
    ...
    brocoind.generate_block(1)
    l1.daemon.wait_for_logs([
        r'Owning output . (\d+)bro .SEGWIT. txid',
    ])
...
def test_htlc_out_timeout(node_factory, brocoind, executor):
    ...
    brocoind.generate_block(5 + 1 + shadowlen)
    time.sleep(3)
...
@pytest.mark.developer("needs to deactivate shadow routing")
def test_htlc_in_timeout(node_factory, brocoind, executor):
    ...
    if 'shadow' in status:
        ...
        shadowlen = 0
    brocoind.generate_block(2 + shadowlen)
...
@unittest.skipIf(not TEST_NETWORK == 'regtest', 'must be on brocoin network')
...
def test_withdraw_misc(node_factory, brocoind, chainparams):
    def dont_spend_outputs(n, txid):
        ...
        n.rpc.reserveinputs(brocoind.rpc.createpsbt([{'txid': txid,
                                                      'vout': 0},
                                                     {'txid': txid,
                                                      'vout': 1}], []))
    ...
    for out in l1.rpc.listfunds()['outputs']:
        if out['reserved']:
            ...
            assert out['reserved_to_block'] > brocoind.rpc.getblockchaininfo()['blocks']
    l1.rpc.unreserveinputs(brocoind.rpc.createpsbt(inputs, []))
...
def test_listconfigs(node_factory, brocoind, chainparams):
...
def test_listconfigs_plugins(node_factory, brocoind, chainparams):
    l1 = node_factory.get_node()
...
@flaky
@pytest.mark.developer("needs DEVELOPER=1")
def test_blockchaintrack(node_factory, brocoind):
    ...
    height = brocoind.rpc.getblockcount()   # 101

    # At height 111 we receive an incoming payment
    hashes = brocoind.generate_block(9)     # 102-110
    ...
    brocoind.rpc.invalidateblock(hashes[0])
    brocoind.wait_for_log(r'InvalidChainFound: invalid block=.*  height={}'
                          .format(height + 1))
    hashes = brocoind.generate_block(30)
    time.sleep(1)

    brocoind.rpc.getblockcount()
...
@pytest.mark.openchannel('v2')
@pytest.mark.openchannel('v1')
def test_funding_reorg_private(node_factory, brocoind):
    ...
    sync_blockheight(brocoind, [l1])                # height 102
    brocoind.generate_block(3)                      # heights 103-105

    l1.rpc.fundchannel(l2.info['id'], "all", announce=False)
    brocoind.generate_block(1)                      # height 106
    ...
    brocoind.generate_block(1)                      # height 107
    ...
    brocoind.simple_reorg(106, 2)                   # heights 106-108
    brocoind.generate_block(1)                      # height 109 (to reach minimum_depth=2 again)
    l1.start()
    ...
    brocoind.generate_block(1, True)
...
@pytest.mark.openchannel('v1')
@pytest.mark.openchannel('v2')
def test_funding_reorg_remote_lags(node_factory, brocoind):
    ...
    brocoind.generate_block(5)                      # heights 103 - 107
    ...
    brocoind.simple_reorg(103, 1)                   # heights 103 - 108
...
def test_rescan(node_factory, brocoind):
...
def test_brocoind_goes_backwards(node_factory, brocoind):
    """Check that we refuse to acknowledge brocoind giving a shorter chain without explicit rescan"""
    ...
    brocoind.rpc.invalidateblock(brocoind.rpc.getblockhash(105))
    # Restart without killing proxies
    brocoind.rpc.stop()
    TailableProc.stop(brocoind)
    brocoind.start()
    ...
    # Now mess with brocoind at runtime.
    ...
    # It will ignore brocoind and keep asking for block 110.
    time.sleep(5)
...
def test_reserve_enforcement(node_factory, executor):
    ...
    # Pay 1000 bronee to l2.
    l1.pay(l2, 1000000)
    l2.stop()

    # They should both aim for 1%.
    reserves = l2.db.query('SELECT channel_reserve_bronees FROM channel_configs')
    assert reserves == [{'channel_reserve_bronees': 10**6 // 100}] * 2

    # Edit db to reduce reserve to 0 so it will try to violate it.
    l2.db.execute('UPDATE channel_configs SET channel_reserve_bronees=0')
...
def test_feerates(node_factory):
    ...
    assert feerates['warning_missing_feerates'] == 'Some fee estimates unavailable: brocoind startup?'
    ...
    assert feerates['onchain_fee_estimates']['opening_channel_bronees'] == feerates['perkw']['opening'] * 702 // 1000
    assert feerates['onchain_fee_estimates']['mutual_close_bronees'] == feerates['perkw']['mutual_close'] * 673 // 1000
    assert feerates['onchain_fee_estimates']['unilateral_close_bronees'] == feerates['perkw']['unilateral_close'] * 598 // 1000
    htlc_feerate = feerates["perkw"]["htlc_resolution"]
    htlc_timeout_cost = feerates["onchain_fee_estimates"]["htlc_timeout_bronees"]
    htlc_success_cost = feerates["onchain_fee_estimates"]["htlc_success_bronees"]
...
def test_bad_onion(node_factory, brocoind):
...
def test_bad_onion_immediate_peer(node_factory, brocoind):
...
def test_brocoind_fail_first(node_factory, brocoind, executor):
    """Make sure we handle spurious brocoin-cli failures during startup

    See [#2687](https://github.com/ElementsProject/lightning/issues/2687) for
    details

    """
    # Do not start the lightning node since we need to instrument brocoind
    # first.
    l1 = node_factory.get_node(start=False)

    # Instrument brocoind to fail some queries first.
    def mock_fail(*args):
        raise ValueError()
...
def test_new_node_is_mainnet(node_factory):
    ...
    assert os.path.isfile(os.path.join(netdir, "hsm_secret"))
    assert not os.path.isfile(os.path.join(basedir, "hsm_secret"))
    assert not os.path.isfile(os.path.join(netdir, "lightningd-brocoin.pid"))
    assert os.path.isfile(os.path.join(basedir, "lightningd-brocoin.pid"))


def test_unicode_rpc(node_factory, executor, brocoind):
    node = node_factory.get_node()
    desc = "Some candy 🍬 and a nice glass of milk 🥛."

    node.rpc.invoice(mbronee=42, label=desc, description=desc)
...
def test_unix_socket_path_length(node_factory, brocoind, directory, executor, db_provider, test_base_dir):
    ...
    l1 = LightningNode(1, lightning_dir, brocoind, executor, VALGRIND, db=db, port=node_factory.get_next_port())
...
def test_waitblockheight(node_factory, executor, brocoind):
    node = node_factory.get_node()

    sync_blockheight(brocoind, [node])
...
def test_listtransactions(node_factory):
    ...
    # represented as little endian (like brocoind and explorers).
...
def test_listforwards(node_factory, brocoind):
    """Test listfunds command."""
    ...
    assert sum(x['out_mbronee'] for x in settled_forwards) == 3000
...
@pytest.mark.openchannel('v1')
def test_version_reexec(node_factory, brocoind):
