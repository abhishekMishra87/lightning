...
def test_millibronee_passthrough(node_factory):
    """ Ensure that Millibronee arguments and return work.
    """
    plugin_path = os.path.join(os.getcwd(), 'tests/plugins/millibronees.py')
    n = node_factory.get_node(options={'plugin': plugin_path, 'log-level': 'io'})

    # By keyword
    ret = n.rpc.call('echo', {'mbro': Millibronee(17), 'not_an_mbro': '22mbro'})['echo_mbro']
    assert type(ret) == Millibronee
    assert ret == Millibronee(17)

    # By position
    ret = n.rpc.call('echo', [Millibronee(18), '22mbro'])['echo_mbro']
    assert type(ret) == Millibronee
    assert ret == Millibronee(18)
...
def test_pay_plugin(node_factory):
    ...
    msg = 'pay bolt11 [mbronee] [label] [riskfactor] [maxfeepercent] '\
...
def test_openchannel_hook(node_factory, brocoind):
    ...
    expected = {
        'channel_flags': '1',
        'dust_limit_bronees': '546000mbro',
        'htlc_minimum_mbro': '0mbro',
        'id': l1.info['id'],
        'max_accepted_htlcs': '483',
        'max_htlc_value_in_flight_mbro': '18446744073709551615mbro',
        'to_self_delay': '5',
    }
    ...
    if l2.config('experimental-dual-fund'):
        ...
        expected.update({
            ...
            'locktime': '.*',
            'their_funding': '100000000mbro',
            'channel_max_mbro': '16777215000mbro',
        })
    else:
        expected.update({
            'channel_reserve_bronees': '1000000mbro',
            'feerate_per_kw': '7500',
            'funding_bronees': '100000000mbro',
            'push_mbro': '0mbro',
        })
...
@pytest.mark.openchannel('v1')
@pytest.mark.openchannel('v2')
def test_openchannel_hook_error_handling(node_factory, brocoind):
...
@pytest.mark.openchannel('v1')
@pytest.mark.openchannel('v2')
def test_openchannel_hook_chaining(node_factory, brocoind):
    ...
    # 100005bro fundchannel should fail fatal() for l2
    ...
    # 100000bro is good for hook_accepter, so it should fail 'on principle'
...
@pytest.mark.openchannel('v1')
@pytest.mark.openchannel('v2')
def test_channel_state_changed_bilateral(node_factory, brocoind):
    ...
    brocoind.generate_block(100, wait_for_mempool=1)  # so it gets settled
...
@pytest.mark.openchannel('v1')
@pytest.mark.openchannel('v2')
def test_channel_state_changed_unilateral(node_factory, brocoind):
...
@pytest.mark.openchannel('v1')
@pytest.mark.openchannel('v2')
def test_channel_state_change_history(node_factory, brocoind):
    ...
    l1.rpc.close(scid)
    brocoind.generate_block(100)  # so it gets settled
    brocoind.generate_block(100)  # so it gets settled
...
def test_htlc_accepted_hook_resolve(node_factory):
    ...
    inv = l3.rpc.invoice(mbronee=1000, label="lbl", description="desc", preimage="00" * 32)['bolt11']
    l1.rpc.pay(inv)
...
def test_htlc_accepted_hook_direct_restart(node_factory, executor):
    ...
    i1 = l2.rpc.invoice(mbronee=1000, label="direct", description="desc")['bolt11']
...
def test_htlc_accepted_hook_forward_restart(node_factory, executor):
    ...
    i1 = l3.rpc.invoice(mbronee=1000, label="direct", description="desc")['bolt11']
    ...
    assert onion['forward_amount'] == '1000mbro'
...
def test_invoice_payment_notification(node_factory):
    ...
    inv1 = l2.rpc.invoice(mbros, label, 'description', preimage=preimage)
    l1.dev_pay(inv1['bolt11'], use_shadow=False)

    l2.daemon.wait_for_log(r"Received invoice_payment event for label {},"
                           " preimage {}, and amount of {}mbro"
                           .format(label, preimage, mbros))
...
def test_invoice_creation_notification(node_factory):
    ...
    preimage = '1' * 64
    label = "a_descriptive_label"
    l2.rpc.invoice(mbros, label, 'description', preimage=preimage)
...
def test_forward_event_notification(node_factory, brocoind, executor):
    ...
    sync_blockheight(brocoind, [l1, l2, l3, l4, l5])
...
def test_sendpay_notifications(node_factory, brocoind):
...
def test_brocoin_backend(node_factory, brocoind):
    """
    This tests interaction with the Brocoin backend, but not specifically bcli
    """
    ...
    plugin = os.path.join(os.getcwd(), "tests/plugins/brocoin/part1.py")
    l1.daemon.opts["plugin"] = plugin
    try:
        l1.daemon.start()
    except ValueError:
        assert l1.daemon.is_in_log("Missing a Brocoin plugin command")
        ...
        l1.daemon.opts["plugin-dir"] = os.path.join(os.getcwd(),
                                                    "tests/plugins/brocoin/")
        try:
            l1.daemon.start()
        except ValueError:
            msg = "All Brocoin plugin commands registered"
    ...
    l1.start()
    assert l1.daemon.is_in_log("brocoin-cli initialized and connected to"
                               " brocoind")


def test_bcli(node_factory, brocoind, chainparams):
    """
    This tests the bcli plugin, used to gather Brocoin data from a local
    brocoind.
    ...
    # Some other brocoind-failure cases for this call are covered in
    # tests/test_misc.py
    ...
    assert (Millibronee(txo["amount"]) == Millibronee(10**4 * 3 * 10**3)
...
def test_hook_crash(node_factory, executor, brocoind):
    ...
    sync_blockheight(brocoind, [l1] + nodes)
    mine_funding_to_announce(brocoind, [l1] + nodes, wait_for_mempool=1)
...
def test_watchtower(node_factory, brocoind, directory, chainparams):
    ...
    cheat_tx = brocoind.rpc.decoderawtransaction(tx)
    lastcommitnum = 0
    for l in open(wt_file, 'r'):
        ...
        if txid == cheat_tx['txid']:
            ...
            brocoind.rpc.sendrawtransaction(penalty)
            break

    # Need this to check that l2 gets the funds
    penalty_meta = brocoind.rpc.decoderawtransaction(penalty)
...
@pytest.mark.openchannel('v1')
@pytest.mark.openchannel('v2')
def test_coin_movement_notices(node_factory, brocoind, chainparams):
    """Verify that channel coin movements are triggered correctly.  """

    l1_l2_mvts = [
        {'type': 'chain_mvt', 'credit': 0, 'debit': 0, 'tags': ['channel_open']},
        {'type': 'channel_mvt', 'credit': 100001001, 'debit': 0, 'tags': ['routed'], 'fees': '1001mbro'},
        {'type': 'channel_mvt', 'credit': 0, 'debit': 50000000, 'tags': ['routed'], 'fees': '501mbro'},
        {'type': 'channel_mvt', 'credit': 100000000, 'debit': 0, 'tags': ['invoice'], 'fees': '0mbro'},
        {'type': 'channel_mvt', 'credit': 0, 'debit': 50000000, 'tags': ['invoice'], 'fees': '0mbro'},
    ...
    l2_l3_mvts = [
        ...
        {'type': 'channel_mvt', 'credit': 0, 'debit': 100000000, 'tags': ['routed'], 'fees': '1001mbro'},
        {'type': 'channel_mvt', 'credit': 50000501, 'debit': 0, 'tags': ['routed'], 'fees': '501mbro'},
    ...
    l3_l2_mvts = [
        {'type': 'chain_mvt', 'credit': 0, 'debit': 0, 'tags': ['channel_open']},
        {'type': 'channel_mvt', 'credit': 100000000, 'debit': 0, 'tags': ['invoice'], 'fees': '0mbro'},
        {'type': 'channel_mvt', 'credit': 0, 'debit': 50000501, 'tags': ['invoice'], 'fees': '501mbro'},
...
def test_3847_repro(node_factory, brocoind):
    ...
    i1 = l3.rpc.invoice(
        mbronee=amt, label="direct", description="desc"
    )['bolt11']
    with pytest.raises(RpcError):
