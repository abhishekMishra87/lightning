...
def move_matches(exp, mv):
    if mv['type'] != exp['type']:
        return False
    if mv['credit'] != "{}mbro".format(exp['credit']):
        return False
    if mv['debit'] != "{}mbro".format(exp['debit']):
        return False
    if mv['tags'] != exp['tags']:
        return False
    if 'fees' in exp:
...
def check_coin_moves(n, account_id, expected_moves, chainparams):
    ...
    for mv in acct_moves:
        print("{{'type': '{}', 'credit': {}, 'debit': {}, 'tags': '{}' , ['fees'?: '{}']}},"
              .format(mv['type'],
                      Millibronee(mv['credit']).millibronees,
                      Millibronee(mv['debit']).millibronees,
                      mv['tags'],
...
def scriptpubkey_addr(scriptpubkey):
    if 'addresses' in scriptpubkey:
        ...
        # Modern brocoin (at least, git master)
