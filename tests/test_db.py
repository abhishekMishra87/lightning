...
def test_db_dangling_peer_fix(node_factory, brocoind):
    # Make sure brocoind doesn't think it's going backwards
    brocoind.generate_block(104)
...
def test_block_backfill(node_factory, brocoind, chainparams):
    ...
    we fetch the entire block, extract P2WSH outputs and ask `brocoin
    ...
    brocoind.rpc.sendtoaddress('bcrt1qtwxd8wg5eanumk86vfeujvp48hfkgannf77evggzct048wggsrxsum2pmm', 0.00031337)
    l1.rpc.fundchannel(l2.info['id'], 10**6, announce=True)
    wait_for(lambda: len(brocoind.rpc.getrawmempool()) == 2)
    ...
    # and call out to `brocoind` to backfill the block.
    ...
    assert(31337 in [r['bronees'] for r in l3.db_query("SELECT bronees FROM utxoset")])
...
def test_max_channel_id(node_factory, brocoind):
...
def test_scid_upgrade(node_factory, brocoind):
    brocoind.generate_block(1)
...
@unittest.skipIf(os.getenv('TEST_DB_PROVIDER', 'sqlite3') != 'sqlite3', "This test is based on a sqlite3 snapshot")
@unittest.skipIf(TEST_NETWORK != 'regtest', "The network must match the DB snapshot")
def test_last_tx_inflight_psbt_upgrade(node_factory, brocoind):
    brocoind.generate_block(12)
    ...
    for i in range(len(b64_last_txs)):
        ...
        psbt = brocoind.rpc.decodepsbt(bpsbt)
        tx = prior_txs[i]
        assert psbt['tx']['txid'] == brocoind.rpc.decoderawtransaction(tx)['txid']
...
@unittest.skipIf(os.getenv('TEST_DB_PROVIDER', 'sqlite3') != 'sqlite3', "This test is based on a sqlite3 snapshot")
@unittest.skipIf(TEST_NETWORK != 'regtest', "The network must match the DB snapshot")
def test_last_tx_psbt_upgrade(node_factory, brocoind):
    brocoind.generate_block(12)
    ...
    for i in range(len(b64_last_txs)):
        ...
        assert psbt['tx']['txid'] == brocoind.rpc.decoderawtransaction(tx)['txid']
        funding_input = only_one(psbt['inputs'])
        # Every opened channel was funded with the same amount: 1M bros
    ...
    brocoind.generate_block(232 - 113)
    ...
    brocoind.rpc.decodepsbt(base64.b64encode(last_txs[0]).decode('utf-8'))
    brocoind.rpc.decoderawtransaction(last_txs[1].hex())


@unittest.skipIf(os.getenv('TEST_DB_PROVIDER', 'sqlite3') != 'sqlite3', "This test is based on a sqlite3 snapshot")
@unittest.skipIf(TEST_NETWORK != 'regtest', "The network must match the DB snapshot")
def test_backfill_scriptpubkeys(node_factory, brocoind):
    brocoind.generate_block(214)
...
def test_optimistic_locking(node_factory, brocoind):
...
def test_local_basepoints_cache(brocoind, node_factory):
...
def test_sqlite3_builtin_backup(brocoind, node_factory):
