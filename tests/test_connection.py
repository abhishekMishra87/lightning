...
def test_remote_addr(node_factory, brocoind):
    ...
    assert not l2.daemon.is_in_log("Update our node_announcement for discovered address: 127.0.0.1:9443")
    l1.restart()
    l2.rpc.connect(l1.info['id'], 'localhost', l1.port)
    l2.daemon.wait_for_log("Peer says it sees our address as: 127.0.0.1:[0-9]{5}")
    ...
    l2.rpc.connect(l3.info['id'], 'localhost', l3.port)
    l2.daemon.wait_for_log("Peer says it sees our address as: 127.0.0.1:[0-9]{5}")
    l2.daemon.wait_for_log("Update our node_announcement for discovered address: 127.0.0.1:9443")
    l1.daemon.wait_for_log(f"Received node_announcement for node {l2.info['id']}")

    address = l1.rpc.listnodes(l2.info['id'])['nodes'][0]['addresses'][0]
    assert address['type'] == "ipv4"
    assert address['address'] == "127.0.0.1"
    assert address['port'] == 9443


@pytest.mark.developer("needs DEVELOPER=1 for having localhost remote_addr and fast gossip")
def test_remote_addr_disabled(node_factory, brocoind):
...
def test_balance(node_factory):
    ...
    assert p1['mbronee_to_us'] == 10**6 * 1000
    assert p1['mbronee_total'] == 10**6 * 1000
    assert p2['mbronee_to_us'] == 0
    assert p2['mbronee_total'] == 10**6 * 1000
...
def test_opening_tiny_channel(node_factory):
    # Test custom min-capacity-bro parameters
    #
    ...
    #
    # For each:
    #  1. Try to establish channel with capacity 1bro smaller than min_capacity_bro
    #  2. Try to establish channel with capacity exact min_capacity_bro
    #
    # BOLT2
    ...
    opts = [{'min-capacity-bro': 0},
            {'min-capacity-bro': l2_min_capacity},
            {'min-capacity-bro': l3_min_capacity},
            {'min-capacity-bro': l4_min_capacity}]
    l1, l2, l3, l4 = node_factory.get_nodes(4, opts=opts)
    l1.rpc.connect(l2.info['id'], 'localhost', l2.port)
    l1.rpc.connect(l3.info['id'], 'localhost', l3.port)
    l1.rpc.connect(l4.info['id'], 'localhost', l4.port)

    with pytest.raises(RpcError, match=r'They sent [error|warning].*channel capacity is .*, which is below .*bro'):
    ...
    with pytest.raises(RpcError, match=r"channel capacity is .*, which is below .*bro"):
...
def test_channel_abandon(node_factory, brocoind):
    ...
    # Add some for fees
    l1.fundwallet(BROS + 10000)
    l1.rpc.connect(l2.info['id'], 'localhost', l2.port)
    l1.rpc.fundchannel(l2.info['id'], BROS, feerate='1875perkw')
    ...
    assert reservations['reserved_to_block'] == brocoind.rpc.getblockchaininfo()['blocks'] + 1
    ...
    assert brocoind.rpc.decoderawtransaction(withdraw['tx'])['vout'][0]['value'] > BROS / 10**8
    brocoind.generate_block(1, wait_for_mempool=withdraw['txid'])
...
@pytest.mark.openchannel('v1')
@pytest.mark.openchannel('v2')
def test_reconnect_no_update(node_factory, executor, brocoind):
...
def test_reconnect_sender_add1(node_factory):
    ...
    route = [{'mbronee': amt, 'id': l2.info['id'], 'delay': 5, 'channel': '1x1x1'}]
...
@pytest.mark.openchannel('v1')
@pytest.mark.openchannel('v2')
def test_shutdown_awaiting_lockin(node_factory, brocoind):
    l1 = node_factory.get_node()
    ...
    l1.daemon.wait_for_logs(['sendrawtx exit 0', ' to CLOSINGD_COMPLETE'])
    l2.daemon.wait_for_logs(['sendrawtx exit 0', ' to CLOSINGD_COMPLETE'])
    assert brocoind.rpc.getmempoolinfo()['size'] == 1
...
@pytest.mark.openchannel('v1')
@pytest.mark.openchannel('v2')
def test_funding_change(node_factory, brocoind):
...
@pytest.mark.openchannel('v1')
@pytest.mark.openchannel('v2')
def test_funding_all(node_factory, brocoind):
...
def test_funding_all_too_much(node_factory):
    ...
    assert only_one(l1.rpc.listfunds()['channels'])['channel_total_bro'] == 2**24 - 1


@pytest.mark.openchannel('v1')
@pytest.mark.openchannel('v2')
def test_funding_fail(node_factory, brocoind):
    """Add some funds, fund a channel without enough funds"""
    # Previous runs with same brocoind can leave funds!
    max_locktime = 5 * 6 * 24
...
@pytest.mark.openchannel('v1')
@pytest.mark.openchannel('v2')
def test_funding_toolarge(node_factory, brocoind):
...
@unittest.skipIf(TEST_NETWORK != 'regtest', 'elementsd doesnt yet support PSBT features we need')
@pytest.mark.openchannel('v2')
def test_v2_open(node_factory, brocoind, chainparams):
    l1, l2 = node_factory.get_nodes(2)
...
@pytest.mark.openchannel('v1')
def test_funding_push(node_factory, brocoind, chainparams):
    """ Try to push peer some bros """
    ...
    with pytest.raises(RpcError, match=r'Requested to push_mbro of 20000000mbro is greater than available funding amount 10000bro'):
        l1.rpc.fundchannel(l2.info['id'], 10000, push_mbro=push_bro * 1000)

    # This should work.
    amount = amount - 1
    l1.rpc.fundchannel(l2.info['id'], amount, push_mbro=push_bro * 1000)

    brocoind.generate_block(1)
    sync_blockheight(brocoind, [l1])
    funds = only_one(l1.rpc.listfunds()['channels'])
    assert funds['channel_bro'] + push_bro == funds['channel_total_bro']

    chanid = first_channel_id(l2, l1)
    channel_mvts_1 = [
        {'type': 'chain_mvt', 'credit': 16777215000, 'debit': 0, 'tags': ['channel_open', 'opener']},
        {'type': 'channel_mvt', 'credit': 0, 'debit': 20000000, 'tags': ['pushed'], 'fees': '0mbro'},
    ]
    channel_mvts_2 = [
        {'type': 'chain_mvt', 'credit': 0, 'debit': 0, 'tags': ['channel_open']},
        {'type': 'channel_mvt', 'credit': 20000000, 'debit': 0, 'tags': ['pushed'], 'fees': '0mbro'},
    ]
    check_coin_moves(l1, chanid, channel_mvts_1, chainparams)
    check_coin_moves(l2, chanid, channel_mvts_2, chainparams)

    assert account_balance(l1, chanid) == (amount - push_bro) * 1000


@pytest.mark.openchannel('v1')
@pytest.mark.openchannel('v2')
@pytest.mark.developer
def test_funding_by_utxos(node_factory, brocoind):
...
@pytest.mark.openchannel('v1')
def test_funding_external_wallet_corners(node_factory, brocoind):
    l1 = node_factory.get_node(may_reconnect=True)
    l2 = node_factory.get_node(may_reconnect=True)
    ...
    addr = l1.rpc.newaddr()['bech32']
    brocoind.rpc.sendtoaddress(addr, (amount + 1000000) / 10**8)
    brocoind.generate_block(1)
    wait_for(lambda: len(l1.rpc.listfunds()["outputs"]) != 0)
    ...
    decode = brocoind.rpc.decoderawtransaction(prep['unsigned_tx'])
...
@pytest.mark.developer("needs dev_forget_channel")
@pytest.mark.openchannel('v2')
def test_funding_v2_corners(node_factory, brocoind):
    l1 = node_factory.get_node(may_reconnect=True)
    l2 = node_factory.get_node(may_reconnect=True)
...
@pytest.mark.openchannel('v1')
def test_funding_cancel_race(node_factory, brocoind, executor):
    l1 = node_factory.get_node()

    # make sure we can generate PSBTs.
    addr = l1.rpc.newaddr()['bech32']
    brocoind.rpc.sendtoaddress(addr, 200000 / 10**8)
    brocoind.generate_block(1)
    wait_for(lambda: len(l1.rpc.listfunds()["outputs"]) != 0)
    ...
    for count, n in enumerate(nodes):
        ...
        start = l1.rpc.fundchannel_start(n.info['id'], "100000bro")

        prep = l1.rpc.txprepare([{start['funding_address']: "100000bro"}])
...
@unittest.skipIf(SLOW_MACHINE and not VALGRIND, "Way too taxing on CI machines")
@pytest.mark.openchannel('v2')
def test_funding_v2_cancel_race(node_factory, brocoind, executor):
    l1 = node_factory.get_node()

    # make sure we can generate PSBTs.
    addr = l1.rpc.newaddr()['bech32']
    brocoind.rpc.sendtoaddress(addr, 2000000 / 10**8)
    brocoind.generate_block(1)
    wait_for(lambda: len(l1.rpc.listfunds()["outputs"]) != 0)
...
def test_funding_close_upfront(node_factory, brocoind):
    ...
    def _fundchannel(l1, l2, amount, close_to):
        ...
        for node in [l1, l2]:
            ...
            assert amount * 1000 == channel['mbronee_total']
    ...
    def _close(src, dst, addr=None):
        ...
        tx = brocoind.rpc.decoderawtransaction(r['tx'])

        addrs = [scriptpubkey_addr(vout['scriptPubKey']) for vout in tx['vout']]
        brocoind.generate_block(1, wait_for_mempool=[r['txid']])
        sync_blockheight(brocoind, [l1, l2])
        return addrs
    ...
    addr = brocoind.rpc.getnewaddress()
    _fundchannel(l1, l2, amt_normal, addr)
...
@unittest.skipIf(TEST_NETWORK != 'regtest', "External wallet support doesn't work with elements yet.")
@pytest.mark.openchannel('v1')
def test_funding_external_wallet(node_factory, brocoind):
    ...
    psbt = brocoind.rpc.walletcreatefundedpsbt([], [{address: amount / 10**8}])['psbt']
    assert l1.rpc.fundchannel_complete(l2.info['id'], psbt)['commitments_secured']

    # Broadcast the transaction manually
    process = brocoind.rpc.walletprocesspsbt(psbt)
    assert process['complete'] is True
    tx = brocoind.rpc.finalizepsbt(process['psbt'])
    txid = brocoind.rpc.sendrawtransaction(tx['hex'])
    brocoind.generate_block(1)
...
def test_multifunding_v1_v2_mixed(node_factory, brocoind):
    '''
    ...
    mine_funding_to_announce(brocoind, [l1, l2, l3, l4], wait_for_mempool=1)
...
@unittest.skipIf(TEST_NETWORK != 'regtest', 'elementsd doesnt yet support PSBT features we need')
@pytest.mark.openchannel('v2')
def test_multifunding_v2_exclusive(node_factory, brocoind):
    '''
...
@pytest.mark.openchannel('v1')
@pytest.mark.openchannel('v2')
def test_multifunding_simple(node_factory, brocoind):
    '''
...
@pytest.mark.openchannel('v1')
@pytest.mark.openchannel('v2')
def test_listpeers_crash(node_factory, brocoind, executor):
    '''
...
@pytest.mark.openchannel('v1')
@pytest.mark.openchannel('v2')
def test_multifunding_one(node_factory, brocoind):
    '''
...
def test_multifunding_feerates(node_factory, brocoind):
    '''
    ...
    entry = brocoind.rpc.getmempoolentry(res['txid'])
    weight = entry['weight']
    ...
    txfee = only_one(only_one(l1.rpc.listpeers(l2.info['id'])['peers'])['channels'])['last_tx_fee_mbro']
    ...
    entry = brocoind.rpc.getmempoolentry(close_txid)
    ...
    assert expected_fee == entry['fees']['base'] * 10 ** 8
    assert Millibronee(str(expected_fee) + 'bro') == txfee
...
def test_multifunding_best_effort(node_factory, brocoind):
    '''
...
@pytest.mark.openchannel('v1')
@pytest.mark.openchannel('v2')
def test_lockin_between_restart(node_factory, brocoind):
    l1 = node_factory.get_node(may_reconnect=True)
    l2 = node_factory.get_node(options={'funding-confirms': 3},
                               may_reconnect=True)
    l1.rpc.connect(l2.info['id'], 'localhost', l2.port)
...
@pytest.mark.openchannel('v1')
@pytest.mark.openchannel('v2')
def test_funding_while_offline(node_factory, brocoind):
    l1 = node_factory.get_node()
    addr = l1.rpc.newaddr()['bech32']
    sync_blockheight(brocoind, [l1])
    ...
    # We send funds
    brocoind.rpc.sendtoaddress(addr, (10**6 + 1000000) / 10**8)
...
@pytest.mark.openchannel('v1')
@pytest.mark.openchannel('v2')
def test_channel_persistence(node_factory, brocoind, executor):
    ...
    wait_for(lambda: only_one(l1.rpc.listpeers()['peers'][0]['channels'])['mbronee_to_us'] == 99990000)
    ...
    # L1 doesn't actually update mbronee_to_us until it receives
    # revoke_and_ack from L2, which can take a little bit.
    wait_for(lambda: only_one(l1.rpc.listpeers()['peers'][0]['channels'])['mbronee_to_us'] == 99980000)
    assert only_one(l2.rpc.listpeers()['peers'][0]['channels'])['mbronee_to_us'] == 20000

    # Finally restart l1, and make sure it remembers
    l1.restart()
    assert only_one(l1.rpc.listpeers()['peers'][0]['channels'])['mbronee_to_us'] == 99980000
...
@pytest.mark.developer
def test_update_fee(node_factory, brocoind):
    l1, l2 = node_factory.line_graph(2, fundchannel=True)
    ...
    brocoind.generate_block(99)
    l1.daemon.wait_for_log('onchaind complete, forgetting peer')
    l2.daemon.wait_for_log('onchaind complete, forgetting peer')


@pytest.mark.developer
def test_fee_limits(node_factory, brocoind):
...
def test_update_fee_dynamic(node_factory, brocoind):
...
@pytest.mark.developer
def test_update_fee_reconnect(node_factory, brocoind):
...
@pytest.mark.developer("Too slow without --dev-brocoind-poll")
...
@pytest.mark.openchannel('v1')
@pytest.mark.openchannel('v2')
def test_peerinfo(node_factory, brocoind):
    ...
    brocoind.generate_block(100, wait_for_mempool=1)
    l1.daemon.wait_for_log('onchaind complete, forgetting peer')
    l2.daemon.wait_for_log('onchaind complete, forgetting peer')
...
def test_disconnectpeer(node_factory, brocoind):
...
@pytest.mark.openchannel('v1')
@pytest.mark.openchannel('v2')
def test_fundee_forget_funding_tx_unconfirmed(node_factory, brocoind):
    ...
    brocoind.generate_block(blocks)
...
def test_no_fee_estimate(node_factory, brocoind, executor):
    ...
    # Can do mutual close.
    l1.rpc.close(l2.info['id'])
    wait_for(lambda: len(brocoind.rpc.getrawmempool()) > 0)
    brocoind.generate_block(100)
    sync_blockheight(brocoind, [l1, l2])
...
@pytest.mark.developer("needs --dev-disconnect")
def test_opener_feerate_reconnect(node_factory, brocoind):
...
def test_opener_simple_reconnect(node_factory, brocoind):
...
@pytest.mark.openchannel('v1')
@pytest.mark.openchannel('v2')
def test_dataloss_protection(node_factory, brocoind):
    ...
    closetxid = only_one(brocoind.rpc.getrawmempool(False))
...
def test_restart_multi_htlc_rexmit(node_factory, brocoind, executor):
...
@pytest.mark.developer("needs dev_disconnect")
def test_fulfill_incoming_first(node_factory, brocoind):
...
@pytest.mark.openchannel('v1')
@pytest.mark.openchannel('v2')
def test_fail_unconfirmed(node_factory, brocoind, executor):
...
@unittest.skipIf(TEST_NETWORK != 'regtest', 'elementsd doesnt yet support PSBT features we need')
@pytest.mark.openchannel('v2')
def test_fail_unconfirmed_openchannel2(node_factory, brocoind, executor):
    """Test that if we crash with an unconfirmed connection to a known
    peer, we don't have a dangling peer in db"""
    # = is a NOOP disconnect, but sets up file.
...
@pytest.mark.openchannel('v1')
@pytest.mark.openchannel('v2')
def test_change_chaining(node_factory, brocoind):
...
def test_feerate_stress(node_factory, executor):
    ...
    routel1l3 = [{'mbronee': '10002mbro', 'id': l2.info['id'], 'delay': 11, 'channel': scid12},
                 {'mbronee': '10000mbro', 'id': l3.info['id'], 'delay': 5, 'channel': scid23}]
    routel2l1 = [{'mbronee': '10000mbro', 'id': l1.info['id'], 'delay': 5, 'channel': scid12}]
...
@pytest.mark.openchannel('v1')
@pytest.mark.openchannel('v2')
def test_wumbo_channels(node_factory, brocoind):
    l1, l2, l3 = node_factory.get_nodes(3,
                                        opts=[{'large-channels': None},
                                              {'large-channels': None},
                                              {}])
    ...
    assert ([c['amount_mbro'] for c in l3.rpc.listchannels()['channels']]
            == [Millibronee(str(1 << 24) + "bro")] * 2)
    ...
    wait_for(lambda: [c['amount_mbro'] for c in l2.rpc.listchannels(l1.get_channel_scid(l3))['channels']]
             == [Millibronee(str((1 << 24) - 1) + "bro")] * 2)
    ...
    amount = [c['funding']['local_mbro'] for c in only_one(l1.rpc.listpeers(l2.info['id'])['peers'])['channels'] if c['state'] == 'CHANNELD_NORMAL'][0]
    assert amount > Millibronee(str((1 << 24) - 1) + "bro")


@pytest.mark.openchannel('v1')
@pytest.mark.openchannel('v2')
def test_channel_features(node_factory, brocoind):
    l1, l2 = node_factory.line_graph(2, fundchannel=False)

    brocoind.rpc.sendtoaddress(l1.rpc.newaddr()['bech32'], 0.1)
    brocoind.generate_block(1)
...
def test_nonstatic_channel(node_factory, brocoind):
...
def test_openchannel_init_alternate(node_factory, executor):
    ...
    psbt1 = l1.rpc.fundpsbt('1000000mbro', '253perkw', 250)['psbt']
    psbt2 = l2.rpc.fundpsbt('1000000mbro', '253perkw', 250)['psbt']
    init = l1.rpc.openchannel_init(l2.info['id'], 100000, psbt1)

    fut = executor.submit(l2.rpc.openchannel_init, l1.info['id'], '1000000mbro', psbt2)
    with pytest.raises(RpcError):
        fut.result(10)
...
@unittest.skipIf(not EXPERIMENTAL_FEATURES, "upgrade protocol not available")
@pytest.mark.developer("dev-force-features required")
def test_upgrade_statickey_onchaind(node_factory, executor, brocoind):
    ...
    brocoind.rpc.sendrawtransaction(tx)
    brocoind.generate_block(1)
...
def test_upgrade_statickey_fail(node_factory, executor, brocoind):
    ...
    # This HTLC will fail
    l1.rpc.sendpay([{'mbronee': 1000, 'id': l2.info['id'], 'delay': 5, 'channel': '1x1x1'}], '00' * 32, payment_secret='00' * 32)
...
@pytest.mark.openchannel('v1')
@pytest.mark.openchannel('v2')
def test_multichan(node_factory, executor, brocoind):
    ...
    brocoind.rpc.sendtoaddress(l2.rpc.newaddr()['bech32'], 0.1)
    brocoind.generate_block(1)
    sync_blockheight(brocoind, [l2])
    l2.rpc.fundchannel(l3.info['id'], '0.01001bron')
    ...
    assert before[chan23a_idx]['to_us_mbro'] == after[chan23a_idx]['to_us_mbro']
    assert before[chan23b_idx]['to_us_mbro'] != after[chan23b_idx]['to_us_mbro']
    ...
    assert chan23a['amount_mbro'] == Millibronee(1000000000)
    assert chan23a['short_channel_id'] == scid23a
    assert chan23b['amount_mbro'] == Millibronee(1001000000)
