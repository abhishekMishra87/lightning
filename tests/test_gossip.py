...
def test_gossip_pruning(node_factory, brocoind):
...
def test_gossip_disable_channels(node_factory, brocoind):
...
def test_announce_address(node_factory, brocoind):
...
def test_announce_and_connect_via_dns(node_factory, brocoind):
...
@unittest.skipIf(not EXPERIMENTAL_FEATURES, "BOLT7 DNS RFC #911")
@pytest.mark.developer("gossip without DEVELOPER=1 is slow")
def test_only_announce_one_dns(node_factory, brocoind):
...
def test_gossip_timestamp_filter(node_factory, brocoind, chainparams):
...
@pytest.mark.developer("needs --dev-allow-localhost")
def test_connect_by_gossip(node_factory, brocoind):
...
def test_gossip_badsig(node_factory, brocoind):
...
def test_gossip_weirdalias(node_factory, brocoind):
    weird_name = '\t \n \" \n \r \n \\'
...
def test_gossip_persistence(node_factory, brocoind):
...
def test_gossip_no_empty_announcements(node_factory, brocoind, chainparams):
...
@pytest.mark.developer("Too slow without --dev-fast-gossip")
def test_routing_gossip(node_factory, brocoind):
    ...
    mine_funding_to_announce(brocoind, nodes, num_blocks=6, wait_for_mempool=1)
...
def test_gossip_query_channel_range(node_factory, brocoind, chainparams):
    l1, l2, l3, l4 = node_factory.line_graph(4, fundchannel=False)
    genesis_blockhash = chainparams['chain_hash']
    ...
    num_tx = len(brocoind.rpc.getrawmempool())
    # We want these one block apart.
    l1.rpc.fundchannel(l2.info['id'], 10**5)['tx']
    brocoind.generate_block(wait_for_mempool=num_tx + 1)
...
@pytest.mark.developer("needs DEVELOPER=1")
def test_report_routing_failure(node_factory, brocoind):
...
def test_query_short_channel_id(node_factory, brocoind, chainparams):
    l1, l2, l3 = node_factory.get_nodes(3)
    l1.rpc.connect(l2.info['id'], 'localhost', l2.port)
    l2.rpc.connect(l3.info['id'], 'localhost', l3.port)
...
def test_gossip_addresses(node_factory, brocoind):
...
def test_gossip_lease_rates(node_factory, brocoind):
    lease_opts = {'lease-fee-basis': 50,
                  'lease-fee-base-bro': '2000mbro',
                  'channel-fee-max-base-mbro': '500bro',
    ...
    l1.rpc.call('funderupdate', {'channel_fee_max_base_mbro': '0mbro',
                                 'channel_fee_max_proportional_thousandths': 0,
                                 'funding_weight': 0,
                                 'lease_fee_base_mbro': '0mbro',
                                 'lease_fee_basis': 0})
    l2.rpc.call('funderupdate', {'channel_fee_max_base_mbro': '30000mbro',
                                 'channel_fee_max_proportional_thousandths': 100,
                                 'lease_fee_base_mbro': '400000mbro',
                                 'lease_fee_basis': 20})
    ...
    assert rates['channel_fee_max_base_mbro'] == Millibronee('30000mbro')
    assert rates['channel_fee_max_proportional_thousandths'] == 100
    assert rates['funding_weight'] == 666  # Default on regtest
    assert rates['lease_fee_base_mbro'] == Millibronee('400000mbro')
...
@pytest.mark.openchannel('v2')
def test_node_reannounce(node_factory, brocoind, chainparams):
    ...
    ad = l1.rpc.call('setleaserates',
                     {'lease_fee_base_mbro': '1000bro',
                      'lease_fee_basis': 20,
                      'funding_weight': 150,
                      'channel_fee_max_base_mbro': '2000mbro',
                      'channel_fee_max_proportional_thousandths': 22})

    assert ad['lease_fee_base_mbro'] == Millibronee('1000000mbro')
    assert ad['lease_fee_basis'] == 20
    assert ad['funding_weight'] == 150
    assert ad['channel_fee_max_base_mbro'] == Millibronee('2000mbro')
...
def test_gossip_notices_close(node_factory, brocoind):
    ...
    brocoind.generate_block(1, txid)
...
def test_getroute_exclude(node_factory, brocoind):
...
def test_gossip_store_local_channels(node_factory, brocoind):
...
@pytest.mark.developer("need dev-compact-gossip-store")
def test_gossip_store_private_channels(node_factory, brocoind):
...
def setup_gossip_store_test(node_factory, brocoind):
    ...
    wait_for(lambda: sum([c['base_fee_millibronee'] for c in l2.rpc.listchannels()['channels']]) == 21)
    ...
    wait_for(lambda: [c['base_fee_millibronee'] for c in l2.rpc.listchannels(scid23)['channels']] == [20, 20])
    ...
    wait_for(lambda: [c['base_fee_millibronee'] for c in l2.rpc.listchannels(scid12)['channels']] == [20, 20])
...
@pytest.mark.developer("need dev-compact-gossip-store")
def test_gossip_store_compact_noappend(node_factory, brocoind):
    l2 = setup_gossip_store_test(node_factory, brocoind)
...
def test_gossip_store_load_complex(node_factory, brocoind):
    l2 = setup_gossip_store_test(node_factory, brocoind)
...
@pytest.mark.developer("need dev-compact-gossip-store")
def test_gossip_store_compact(node_factory, brocoind):
    l2 = setup_gossip_store_test(node_factory, brocoind)
...
@pytest.mark.developer("need dev-compact-gossip-store")
def test_gossip_store_compact_restart(node_factory, brocoind):
    l2 = setup_gossip_store_test(node_factory, brocoind)
...
@pytest.mark.developer("gossip without DEVELOPER=1 is slow")
def test_gossip_store_compact_on_load(node_factory, brocoind):
    l2 = setup_gossip_store_test(node_factory, brocoind)
...
def test_gossip_announce_invalid_block(node_factory, brocoind):
    """brocoind lags and we might get an announcement for a block we don't have.
    ...
    opts = {}
    if DEVELOPER:
        opts['dev-brocoind-poll'] = TIMEOUT // 2

    l1 = node_factory.get_node(options=opts)
    brocoind.generate_block(1)
    assert brocoind.rpc.getblockchaininfo()['blocks'] == 102
...
def test_gossip_announce_unknown_block(node_factory, brocoind):
    ...
    assert brocoind.rpc.getblockchaininfo()['blocks'] == 103
...
def test_gossip_ratelimit(node_factory, brocoind):
    ...
    tx = brocoind.rpc.createrawtransaction(
        [],
        [
    ...
    tx = brocoind.rpc.fundrawtransaction(tx, {'changePosition': 0})['hex']
    tx = brocoind.rpc.signrawtransactionwithwallet(tx)['hex']
    txid = brocoind.rpc.sendrawtransaction(tx)
    wait_for(lambda: txid in brocoind.rpc.getrawmempool())
...
def test_statictor_onions(node_factory):
    ...
    assert l1.daemon.is_in_log('.onion:{}'.format(9443))
...
def test_gossip_store_upgrade_v7_v8(node_factory):
    ...
    assert l1.rpc.listchannels()['channels'] == [
        {'source': '022d223620a359a47ff7f7ac447c85c46c923da53389221a0054c11c1e3ca31d59',
         ...
         'public': False,
         'bronees': 1000000,
         'amount_mbro': Millibronee(1000000000),
         'message_flags': 1,
         'channel_flags': 0,
         'active': False,
         'last_update': 1588223664,
         'base_fee_millibronee': 1,
         'fee_per_millionth': 10,
         'delay': 6,
         'htlc_minimum_mbro': Millibronee(0),
         'htlc_maximum_mbro': Millibronee(990000000),
...
def test_topology_leak(node_factory, brocoind):
