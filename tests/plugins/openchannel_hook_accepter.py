...
"""Simple plugin to test the openchannel_hook's
   ...
   If the funding amount is:
      - 100005bro: we reject correctly w/o close_to
      - 100004bro: we reject invalid by setting a close_to
      - 100003bro: we send back a valid address (regtest)
      - 100002bro: we send back an empty address
      - 100001bro: we send back an address for the wrong chain (mainnet)
...
def run_openchannel(funding_bros_str, plugin):
    # Convert from string to bronees
    funding_bros = Millibronee(funding_bros_str).to_bronee()

    # - 100005bro: we reject correctly w/o close_to
    if funding_bros == 100005:
        msg = "reject for a reason"
        plugin.log(msg)
        return {'result': 'reject', 'error_message': msg}

    # - 100004bro: we reject invalid by setting a close_to
    if funding_bros == 100004:
...
def on_openchannel(openchannel, plugin, **kwargs):
    return run_openchannel(openchannel['funding_bronees'], plugin)
