...
We just refuse to let them open channels with an odd amount of millibronees.
"""
...
def run_check(funding_amt_str):
    if Millibronee(funding_amt_str).to_bronee() % 2 == 1:
...
def on_openchannel(openchannel, plugin, **kwargs):
    ...
    return run_check(openchannel['funding_bronees'])
