...
def test_closing_simple(node_factory, brocoind, chainparams):
    ...
    assert brocoind.rpc.getmempoolinfo()['size'] == 0
    ...
    billboard = only_one(l1.rpc.listpeers(l2.info['id'])['peers'][0]['channels'])['status']
    assert billboard == [
        'CLOSINGD_SIGEXCHANGE:We agreed on a closing fee of {} bronee for tx:{}'.format(fee, closetxid),
    ]
...
def test_closing_while_disconnected(node_factory, brocoind, executor):
    ...
    brocoind.generate_block(101)
    wait_for(lambda: len(l1.rpc.listchannels()['channels']) == 0)
    wait_for(lambda: len(l2.rpc.listchannels()['channels']) == 0)


def test_closing_disconnected_notify(node_factory, brocoind, executor):
...
def test_closing_different_fees(node_factory, brocoind, executor):
    ...
    brocoind.rpc.sendtoaddress(addr, 1)
    ...
    mine_funding_to_announce(brocoind, peers, num_blocks=6)
    ...
    # wait for mempool.
    wait_for(lambda: brocoind.rpc.getmempoolinfo()['size'] == num_peers)
...
def test_closing_negotiation_reconnect(node_factory, brocoind):
...
@pytest.mark.developer("needs DEVELOPER=1")
def test_closing_specified_destination(node_factory, brocoind, chainparams):
    ...
    wait_for(lambda: brocoind.rpc.getmempoolinfo()['size'] == 3)
    ...
    for n in [l2, l3, l4]:
        ...
        assert addr == scriptpubkey_addr(brocoind.rpc.gettxout(closetx, output_num1)['scriptPubKey'])
        assert 1 == brocoind.rpc.gettxout(closetx, output_num1)['confirmations']


def closing_negotiation_step(node_factory, brocoind, chainparams, opts):
    ...
    status_agreed_regex = re.compile("agreed on a closing fee of ([0-9]+) bronee")
    ...
    # Get the closing transaction from the brocoind mempool and get its fee
    ...
    def get_mempool_when_size_1():
        ...
        mempool = brocoind.rpc.getrawmempool(True)
...
def test_closing_negotiation_step_30pct(node_factory, brocoind, chainparams):
    ...
    closing_negotiation_step(node_factory, brocoind, chainparams, opts)
...
@unittest.skipIf(EXPERIMENTAL_FEATURES, "anchors uses quick-close, not negotiation")
@unittest.skipIf(TEST_NETWORK == 'liquid-regtest', "Different closing fees")
def test_closing_negotiation_step_100pct(node_factory, brocoind, chainparams):
...
@unittest.skipIf(EXPERIMENTAL_FEATURES, "anchors uses quick-close, not negotiation")
@unittest.skipIf(TEST_NETWORK == 'liquid-regtest', "Different closing fees")
def test_closing_negotiation_step_1bro(node_factory, brocoind, chainparams):
    """Test that the closing fee negotiation step works, 1bro"""
    opts = {}
...
@unittest.skipIf(EXPERIMENTAL_FEATURES, "anchors uses quick-close, not negotiation")
@unittest.skipIf(TEST_NETWORK == 'liquid-regtest', "Different closing fees")
def test_closing_negotiation_step_700bro(node_factory, brocoind, chainparams):
    """Test that the closing fee negotiation step works, 700bro"""
    opts = {}
...
def test_penalty_inhtlc(node_factory, brocoind, executor, chainparams):
...
@pytest.mark.developer("needs dev-disable-commit-after")
def test_penalty_outhtlc(node_factory, brocoind, executor, chainparams):
...
def test_channel_lease_falls_behind(node_factory, brocoind):
    '''
...
@pytest.mark.developer("requres 'dev-queryrates'")
@pytest.mark.slow_test
def test_channel_lease_post_expiry(node_factory, brocoind, chainparams):
    ...
    assert Millibronee(est_fees + amount * 1000) == Millibronee(fundings['remote_mbro'])
    ...
    channel_mvts_1 = [
        ...
        {'type': 'channel_mvt', 'credit': 0, 'debit': 6432000, 'tags': ['lease_fee'], 'fees': '0mbro'},
        {'type': 'channel_mvt', 'credit': 0, 'debit': 10000, 'tags': ['invoice'], 'fees': '0mbro'},
    ...
    channel_mvts_2 = [
        ...
        {'type': 'channel_mvt', 'credit': 6432000, 'debit': 0, 'tags': ['lease_fee'], 'fees': '0mbro'},
        {'type': 'channel_mvt', 'credit': 10000, 'debit': 0, 'tags': ['invoice'], 'fees': '0mbro'},
...
@pytest.mark.slow_test
@pytest.mark.developer("requres 'dev-queryrates'")
def test_channel_lease_unilat_closes(node_factory, brocoind):
    '''
    ...
    brocoind.generate_block(10, wait_for_mempool=2)
    ...
    for i in range(4):
        brocoind.generate_block(4032 // 4)
        sync_blockheight(brocoind, [l2, l3])
...
@pytest.mark.developer("requres 'dev-queryrates'")
def test_channel_lease_lessor_cheat(node_factory, brocoind, chainparams):
    '''
    ...
    l2.start(wait_for_brocoind_sync=True)
    l1.rpc.connect(l2.info['id'], 'localhost', l2.port)
    sync_blockheight(brocoind, [l2])
...
@unittest.skipIf(os.getenv('TEST_DB_PROVIDER', 'sqlite3') != 'sqlite3', "Makes use of the sqlite3 db")
@pytest.mark.developer("requres 'dev-queryrates'")
def test_channel_lease_lessee_cheat(node_factory, brocoind, chainparams):
    '''
...
@unittest.skipIf(os.getenv('TEST_DB_PROVIDER', 'sqlite3') != 'sqlite3', "Makes use of the sqlite3 db")
@pytest.mark.slow_test
def test_penalty_htlc_tx_fulfill(node_factory, brocoind, chainparams):
    ...
    if not chainparams['elements']:
        ...
        expected_bals_2 = [
            {'blockheight': 101, 'accounts': [{'balance': '0mbro'}]},
            {'blockheight': 108, 'accounts': [{'balance': '995433000mbro'}, {'balance': '500000000mbro'}, {'balance': '499994999mbro'}]},
...
@unittest.skipIf(os.getenv('TEST_DB_PROVIDER', 'sqlite3') != 'sqlite3', "Makes use of the sqlite3 db")
@pytest.mark.slow_test
def test_penalty_htlc_tx_timeout(node_factory, brocoind, chainparams):
    """ Test that the penalizing node claims any published
        HTLC transactions
    ...
    brocoind.generate_block(5, wait_for_mempool=0)
    sync_blockheight(brocoind, [l2])
    ...
    txid = brocoind.rpc.decoderawtransaction(tx)['txid']
    brocoind.generate_block(1, wait_for_mempool=[txid])
...
def test_penalty_rbf_normal(node_factory, brocoind, executor, chainparams):
    '''
    ...
    brocoind.rpc.sendrawtransaction(theft_tx)
    brocoind.generate_block(1)
    ...
    # would be acceptable to Brocoin.
    for tx in rbf_txes:
...
@pytest.mark.developer("uses dev_sign_last_tx")
def test_penalty_rbf_burn(node_factory, brocoind, executor, chainparams):
    '''
...
@pytest.mark.developer("needs DEVELOPER=1")
def test_onchain_first_commit(node_factory, brocoind):
...
@pytest.mark.developer("needs DEVELOPER=1")
def test_onchain_unwatch(node_factory, brocoind):
...
@pytest.mark.developer("needs DEVELOPER=1")
def test_onchaind_replay(node_factory, brocoind):
...
@pytest.mark.developer("needs DEVELOPER=1")
def test_onchain_dust_out(node_factory, brocoind, executor):
...
@pytest.mark.developer("needs DEVELOPER=1")
def test_onchain_timeout(node_factory, brocoind, executor):
...
@pytest.mark.developer("needs DEVELOPER=1")
def test_onchain_middleman_simple(node_factory, brocoind):
    # We track channel balances, to verify that accounting is ok.
    coin_mvt_plugin = os.path.join(os.getcwd(), 'tests/plugins/coin_movements.py')
...
@pytest.mark.developer("needs DEVELOPER=1")
def test_onchain_middleman_their_unilateral_in(node_factory, brocoind):
...
@pytest.mark.developer("needs DEVELOPER=1")
def test_onchain_their_unilateral_out(node_factory, brocoind):
...
def test_listfunds_after_their_unilateral(node_factory, brocoind):
...
@pytest.mark.developer("needs DEVELOPER=1")
def test_onchain_feechange(node_factory, brocoind, executor):
...
def test_onchain_all_dust(node_factory, brocoind, executor):
...
def test_onchain_different_fees(node_factory, brocoind, executor):
...
@pytest.mark.developer("needs DEVELOPER=1")
def test_permfail_new_commit(node_factory, brocoind, executor):
...
def setup_multihtlc_test(node_factory, brocoind):
    ...
    preimage = "0" * 64
    inv = nodes[0].rpc.invoice(mbronee=10**8, label='x', description='desc',
                               preimage=preimage)
    h = inv['payment_hash']
    nodes[-1].rpc.invoice(mbronee=10**8, label='x', description='desc',
    ...
    brocoind.generate_block(1)
    sync_blockheight(brocoind, nodes)
...
@pytest.mark.slow_test
def test_onchain_multihtlc_our_unilateral(node_factory, brocoind):
    """Node pushes a channel onchain with multiple HTLCs with same payment_hash """
    h, nodes = setup_multihtlc_test(node_factory, brocoind)
...
@pytest.mark.developer("needs DEVELOPER=1 for dev_ignore_htlcs")
@pytest.mark.slow_test
def test_onchain_multihtlc_their_unilateral(node_factory, brocoind):
    """Node pushes a channel onchain with multiple HTLCs with same payment_hash """
    h, nodes = setup_multihtlc_test(node_factory, brocoind)
...
@pytest.mark.developer("needs DEVELOPER=1")
def test_permfail_htlc_in(node_factory, brocoind, executor):
...
@pytest.mark.developer("needs DEVELOPER=1")
def test_permfail_htlc_out(node_factory, brocoind, executor):
...
@pytest.mark.developer("needs DEVELOPER=1")
def test_permfail(node_factory, brocoind):
    l1, l2 = node_factory.line_graph(2)
    ...
    for o in l1.rpc.listfunds()['outputs']:
        txout = brocoind.rpc.gettxout(o['txid'], o['output'])
...
def test_option_upfront_shutdown_script(node_factory, brocoind, executor):
...
@pytest.mark.developer("needs to set upfront_shutdown_script")
def test_invalid_upfront_shutdown_script(node_factory, brocoind, executor):
...
@pytest.mark.developer("needs to set upfront_shutdown_script")
@pytest.mark.slow_test
def test_segwit_shutdown_script(node_factory, brocoind, executor):
    """
...
"""
    ...
    brocoind.rpc.sendmany("", addresses)
    brocoind.generate_block(1)
...
def test_closing_higherfee(node_factory, brocoind, executor):
    ...
    l1.daemon.wait_for_log(r'deriving max fee from rate 30000 -> 16440bro \(not 1000000bro\)')
...
@pytest.mark.developer("needs dev_disconnect")
def test_you_forgot_closed_channel_onchain(node_factory, brocoind, executor):
...
def test_segwit_anyshutdown(node_factory, brocoind, executor):
...
@unittest.skipIf(TEST_NETWORK == 'liquid-regtest', "Uses regtest addresses")
def test_anysegwit_close_needs_feature(node_factory, brocoind):
...
def test_close_feerate_range(node_factory, brocoind, chainparams):
    ...
    l1.daemon.wait_for_log('Negotiating closing fee between {}bro and {}bro bronee'.format(l1_range[0], l1_range[1]))
    l2.daemon.wait_for_log('Negotiating closing fee between {}bro and {}bro bronee'.format(l2_range[0], l2_range[1]))

    overlap = [max(l1_range[0], l2_range[0]), min(l1_range[1], l2_range[1])]
    l1.daemon.wait_for_log('performing quickclose in range {}bro-{}bro'.format(overlap[0], overlap[1]))

    log = l1.daemon.is_in_log('Their actual closing tx fee is .*bro')
    rate = re.match('.*Their actual closing tx fee is ([0-9]*bro).*', log).group(1)

    assert notifications == ['Sending closing fee offer {}, with range {}bro-{}bro'.format(rate,
                                                                                           l1_range[0],
                                                                                           l1_range[1]),
                             'Received closing fee offer {}, with range {}bro-{}bro'.format(rate,
                                                                                            l2_range[0],
                                                                                            l2_range[1])]
...
def test_close_weight_estimate(node_factory, brocoind):
    ...
    signed_weight = int(brocoind.rpc.decoderawtransaction(tx)['weight'])
    assert signed_weight <= actual_weight
    assert signed_weight >= actual_weight - 2


@pytest.mark.developer("needs dev_disconnect")
def test_onchain_close_upstream(node_factory, brocoind):
...
We send an HTLC, and peer unilaterally closes: do we close upstream?
    ...
    ph1 = l3.rpc.invoice(mbronee="10000bro", label='x1', description='desc2')['payment_hash']
    ph2 = l3.rpc.invoice(mbronee="10000bro", label='x2', description='desc2')['payment_hash']
