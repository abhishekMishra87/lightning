							   ...
							   struct db_stmt *stmt)
{
	...
	if (!db_col_is_null(stmt, "mbronee")) {
		dtl->mbro = tal(dtl, struct amount_mbro);
		db_col_amount_mbro(stmt, "mbronee", dtl->mbro);
	} else {
		dtl->mbro = NULL;
	}
	...
	if (dtl->state == PAID) {
		...
		db_col_amount_mbro(stmt, "mbronee_received", &dtl->received);
		dtl->paid_timestamp = db_col_u64(stmt, "paid_timestamp");
	} else {
		db_col_ignore(stmt, "pay_index");
		db_col_ignore(stmt, "mbronee_received");
		     ...
		     const struct sha256 *local_offer_id)
{
	...
	stmt = db_prepare_v2(
		...
		"            , mbronee, label, expiry_time"
		"            , pay_index, mbronee_received"
	...
	if (mbro)
		db_bind_amount_mbro(stmt, 3, mbro);
	else
		db_bind_null(stmt, 3);
		      ...
		      struct amount_mbro received)
{
	...
	db_bind_amount_mbro(stmt, 2, &received);
