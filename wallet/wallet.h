...
struct forwarding {
	...
	struct amount_mbro mbro_in, mbro_out, fee;
...
struct wallet_payment {
	...
	struct amount_mbro mbronee;
	struct amount_mbro mbronee_sent;
...
struct channel_stats {
	...
	struct amount_mbro  in_mbronee_offered,  in_mbronee_fulfilled;
	u64 out_payments_offered, out_payments_fulfilled;
	struct amount_mbro out_mbronee_offered, out_mbronee_fulfilled;
};
...
struct utxo *wallet_find_utxo(const tal_t *ctx, struct wallet *w,
			      ...
			      struct amount_bro *amount_we_are_short,
 ...
 * @mbronee: amount in mbronee being transferred
 */
void wallet_channel_stats_incr_in_offered(struct wallet *w, u64 cdbid, struct amount_mbro mbronee);
void wallet_channel_stats_incr_in_fulfilled(struct wallet *w, u64 cdbid, struct amount_mbro mbronee);
void wallet_channel_stats_incr_out_offered(struct wallet *w, u64 cdbid, struct amount_mbro mbronee);
void wallet_channel_stats_incr_out_fulfilled(struct wallet *w, u64 cdbid, struct amount_mbro mbronee);
 ...
 * @remote_ann_brocoin_sig: location to load remote_ann_brocoin_sig to
 *
 ...
 * @mbro - the amount the invoice should have, or
 ...
 *
 * @ctx - the owner of the label and mbronee fields returned.
 * @wallet - the wallet whose invoices are to be iterated over.
 * @iterator - the iterator object to use.
...
/**
 * Load remote_ann_node_sig and remote_ann_brocoin_sig
 *
 * @ctx: allocation context for the return value
 * @w: wallet containing the channel
 * @id: channel database id
