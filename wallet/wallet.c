			    ...
			    enum wallet_output_type type)
{
	...
	db_bind_txid(stmt, 0, &utxo->outpoint.txid);
	db_bind_int(stmt, 1, utxo->outpoint.n);
	db_bind_amount_bro(stmt, 2, &utxo->amount);
...
static struct utxo *wallet_stmt2output(const tal_t *ctx, struct db_stmt *stmt)
{
	...
	db_col_amount_bro(stmt, "value", &utxo->amount);
			   ...
			   u32 unreserve)
{
	if (utxo->status != OUTPUT_STATE_RESERVED)
		fatal("UTXO %s is not reserved",
		      type_to_string(tmpctx, struct brocoin_outpoint,
				     &utxo->outpoint));
		     ...
		     const struct utxo *utxo)
{
	for (size_t i = 0; i < tal_count(excludes); i++) {
		if (brocoin_outpoint_eq(&excludes[i]->outpoint, &utxo->outpoint))
			return true;
	}
	return false;
}
			      ...
			      u32 csv_lock)
{
	...
	db_bind_txid(stmt, 0, &outpoint->txid);
	db_bind_int(stmt, 1, outpoint->n);
	db_bind_amount_bro(stmt, 2, &amount);
...
static struct brocoin_signature *
wallet_htlc_sigs_load(const tal_t *ctx, struct wallet *w, u64 channelid,
		      bool option_anchor_outputs)
{
	struct db_stmt *stmt;
	struct brocoin_signature *htlc_sigs = tal_arr(ctx, struct brocoin_signature, 0);
...
bool wallet_remote_ann_sigs_load(const tal_t *ctx, struct wallet *w, u64 id,
				 ...
				 secp256k1_ecdsa_signature **remote_ann_brocoin_sig)
{
	struct db_stmt *stmt;
	bool res;
	stmt = db_prepare_v2(
	    w->db, SQL("SELECT remote_ann_node_sig, remote_ann_brocoin_sig"
	...
	if (db_col_is_null(stmt, "remote_ann_node_sig")
	    || db_col_is_null(stmt, "remote_ann_brocoin_sig")) {
		db_col_ignore(stmt, "remote_ann_brocoin_sig");
		*remote_ann_node_sig = *remote_ann_brocoin_sig = NULL;
		tal_free(stmt);
		return true;
	}
	...
	*remote_ann_brocoin_sig = tal(ctx, secp256k1_ecdsa_signature);

	if (!db_col_signature(stmt, "remote_ann_node_sig", *remote_ann_node_sig))
		goto fail;

	if (!db_col_signature(stmt, "remote_ann_brocoin_sig", *remote_ann_brocoin_sig))
		goto fail;
...
fail:
	...
	*remote_ann_brocoin_sig = tal_free(*remote_ann_brocoin_sig);
	tal_free(stmt);
	return false;
}
...
void wallet_inflight_add(struct wallet *w, struct channel_inflight *inflight)
{
	...
	db_bind_amount_bro(stmt, 4, &inflight->funding->total_funds);
	db_bind_amount_bro(stmt, 5, &inflight->funding->our_funds);
	...
	if (inflight->lease_expiry != 0) {
		...
		db_bind_int(stmt, 11, inflight->lease_chan_max_mbro);
		...
		db_bind_amount_mbro(stmt, 15, &inflight->lease_fee);
	} else {
		db_bind_null(stmt, 10);
		db_bind_null(stmt, 11);
...
wallet_stmt2inflight(struct wallet *w, struct db_stmt *stmt,
		     struct channel *chan)
{
	struct amount_bro funding_bro, our_funding_bro;
	struct amount_mbro lease_fee;
	struct brocoin_outpoint funding;
	struct brocoin_signature last_sig;
	struct channel_inflight *inflight;

	secp256k1_ecdsa_signature *lease_commit_sig;
	u32 lease_chan_max_mbro, lease_blockheight_start;
	u16 lease_chan_max_ppt;

	db_col_txid(stmt, "funding_tx_id", &funding.txid);
	funding.n = db_col_int(stmt, "funding_tx_outnum"),
	db_col_amount_bro(stmt, "funding_bronee", &funding_bro);
	db_col_amount_bro(stmt, "our_funding_bronee", &our_funding_bro);
				       ...
				       struct channel_config *cc)
{
	...
	const char *query = SQL(
	    "SELECT dust_limit_bronees, max_htlc_value_in_flight_mbro, "
	    "channel_reserve_bronees, htlc_minimum_mbro, to_self_delay, "
	    "max_accepted_htlcs, max_dust_htlc_exposure_mbro"
	...
	cc->id = id;
	db_col_amount_bro(stmt, "dust_limit_bronees", &cc->dust_limit);
	db_col_amount_mbro(stmt, "max_htlc_value_in_flight_mbro",
			   &cc->max_htlc_value_in_flight);
	db_col_amount_bro(stmt, "channel_reserve_bronees",
			  &cc->channel_reserve);
	db_col_amount_mbro(stmt, "htlc_minimum_mbro", &cc->htlc_minimum);
	cc->to_self_delay = db_col_int(stmt, "to_self_delay");
	cc->max_accepted_htlcs = db_col_int(stmt, "max_accepted_htlcs");
	db_col_amount_mbro(stmt, "max_dust_htlc_exposure_mbro",
			   &cc->max_dust_htlc_exposure_mbro);
	tal_free(stmt);
	return ok;
}
...
static struct channel *wallet_stmt2channel(struct wallet *w, struct db_stmt *stmt)
{
	...
	struct brocoin_outpoint funding;
	struct brocoin_outpoint *shutdown_wrong_funding;
	struct brocoin_signature last_sig;
	...
	struct amount_bro funding_bro, our_funding_bro;
	struct amount_mbro push_mbro, our_mbro, mbro_to_us_min, mbro_to_us_max, htlc_minimum_mbro, htlc_maximum_mbro;
	struct channel_type *type;
	secp256k1_ecdsa_signature *lease_commit_sig;
	u32 lease_chan_max_mbro;
	u16 lease_chan_max_ppt;
	...
	if (db_col_is_null(stmt, "shutdown_wrong_txid")) {
		...
		shutdown_wrong_funding = tal(tmpctx, struct brocoin_outpoint);
	...
	db_col_amount_bro(stmt, "funding_bronee", &funding_bro);
	db_col_amount_bro(stmt, "our_funding_bronee", &our_funding_bro);
	db_col_amount_mbro(stmt, "push_mbronee", &push_mbro);
	db_col_amount_mbro(stmt, "mbronee_local", &our_mbro);
	db_col_amount_mbro(stmt, "mbronee_to_us_min", &mbro_to_us_min);
	db_col_amount_mbro(stmt, "mbronee_to_us_max", &mbro_to_us_max);
	db_col_amount_mbro(stmt, "htlc_minimum_mbro", &htlc_minimum_mbro);
	db_col_amount_mbro(stmt, "htlc_maximum_mbro", &htlc_maximum_mbro);
	...
	chan = new_channel(peer, db_col_u64(stmt, "id"),
			   ...
			   &cid,
			   our_mbro,
			   mbro_to_us_min, /* mbronee_to_us_min */
			   mbro_to_us_max, /* mbronee_to_us_max */
			   db_col_psbt_to_tx(tmpctx, stmt, "last_tx"),
			   &last_sig,
				 ...
				 struct amount_mbro mbro)
{
	...
	case IN_OFFERED:
		query = SQL("UPDATE channels"
			    ...
			    "     , in_mbronee_offered = COALESCE(in_mbronee_offered, 0) + ?"
			    " WHERE id = ?;");
		break;
	case IN_FULLFILLED:
		query = SQL("UPDATE channels"
			    "   SET in_payments_fulfilled = COALESCE(in_payments_fulfilled, 0) + 1"
			    "     , in_mbronee_fulfilled = COALESCE(in_mbronee_fulfilled, 0) + ?"
			    " WHERE id = ?;");
		break;
	case OUT_OFFERED:
		query = SQL("UPDATE channels"
			    "   SET out_payments_offered = COALESCE(out_payments_offered, 0) + 1"
			    "     , out_mbronee_offered = COALESCE(out_mbronee_offered, 0) + ?"
			    " WHERE id = ?;");
		break;
	case OUT_FULLFILLED:
		query = SQL("UPDATE channels"
			    "   SET out_payments_fulfilled = COALESCE(out_payments_fulfilled, 0) + 1"
			    "     , out_mbronee_fulfilled = COALESCE(out_mbronee_fulfilled, 0) + ?"
			    " WHERE id = ?;");
		break;
	}
			       ...
			       struct channel_stats *stats)
{
	...
	stmt = db_prepare_v2(w->db, SQL(
				     ...
				     ",  in_mbronee_offered,  in_mbronee_fulfilled"
				     ", out_payments_offered, out_payments_fulfilled"
				     ", out_mbronee_offered, out_mbronee_fulfilled"
				     "  FROM channels"
				     " WHERE id = ?"));
	db_bind_u64(stmt, 0, id);
	db_query_prepared(stmt);
	...
	db_col_amount_mbro_or_default(stmt, "in_mbronee_offered",
				      &stats->in_mbronee_offered,
				      AMOUNT_MBRO(0));
	db_col_amount_mbro_or_default(stmt, "in_mbronee_fulfilled",
				      &stats->in_mbronee_fulfilled,
				      AMOUNT_MBRO(0));
	...
	db_col_amount_mbro_or_default(stmt, "out_mbronee_offered",
				      &stats->out_mbronee_offered,
				      AMOUNT_MBRO(0));
	db_col_amount_mbro_or_default(stmt, "out_mbronee_fulfilled",
				      &stats->out_mbronee_fulfilled,
				      AMOUNT_MBRO(0));
	tal_free(stmt);
}
				       ...
				       const struct channel_config *cc)
{
	...
	stmt = db_prepare_v2(w->db, SQL("UPDATE channel_configs SET"
					"  dust_limit_bronees=?,"
					"  max_htlc_value_in_flight_mbro=?,"
					"  channel_reserve_bronees=?,"
					"  htlc_minimum_mbro=?,"
					"  to_self_delay=?,"
					"  max_accepted_htlcs=?,"
					"  max_dust_htlc_exposure_mbro=?"
					" WHERE id=?;"));
	db_bind_amount_bro(stmt, 0, &cc->dust_limit);
	db_bind_amount_mbro(stmt, 1, &cc->max_htlc_value_in_flight);
	db_bind_amount_bro(stmt, 2, &cc->channel_reserve);
	db_bind_amount_mbro(stmt, 3, &cc->htlc_minimum);
	db_bind_int(stmt, 4, cc->to_self_delay);
	db_bind_int(stmt, 5, cc->max_accepted_htlcs);
	db_bind_amount_mbro(stmt, 6, &cc->max_dust_htlc_exposure_mbro);
			      ...
			      secp256k1_ecdsa_signature *remote_ann_brocoin_sig)
{
	...
	stmt = db_prepare_v2(w->db, SQL("UPDATE channels SET"
					"  remote_ann_node_sig=?,"
					"  remote_ann_brocoin_sig=?"
					" WHERE id=?"));

	db_bind_signature(stmt, 0, remote_ann_node_sig);
	db_bind_signature(stmt, 1, remote_ann_brocoin_sig);
...
void wallet_channel_save(struct wallet *w, struct channel *chan)
{
	...
	stmt = db_prepare_v2(w->db, SQL("UPDATE channels SET"
					...
					"  mbronee_to_us_min=?," // 25
					"  mbronee_to_us_max=?," // 26
					"  feerate_base=?," // 27
					"  feerate_ppm=?," // 28
					...
					"  lease_expiry=?," // 38
					"  lease_commit_sig=?," // 39
					"  lease_chan_max_mbro=?," // 40
					"  lease_chan_max_ppt=?," // 41
					"  htlc_minimum_mbro=?," // 42
					"  htlc_maximum_mbro=?" // 43
					" WHERE id=?")); // 44
	...
	db_bind_amount_bro(stmt, 12, &chan->funding_bros);
	db_bind_amount_bro(stmt, 13, &chan->our_funds);
	db_bind_int(stmt, 14, chan->remote_funding_locked);
	db_bind_amount_mbro(stmt, 15, &chan->push);
	db_bind_amount_mbro(stmt, 16, &chan->our_mbro);
	...
	db_bind_amount_mbro(stmt, 25, &chan->mbro_to_us_min);
	db_bind_amount_mbro(stmt, 26, &chan->mbro_to_us_max);
	...
	if (chan->lease_commit_sig) {
		...
		db_bind_int(stmt, 40, chan->lease_chan_max_mbro);
	...
	db_bind_amount_mbro(stmt, 42, &chan->htlc_minimum_mbro);
	db_bind_amount_mbro(stmt, 43, &chan->htlc_maximum_mbro);
				 ...
				 struct amount_bro *total)
{
	...
	*total = AMOUNT_BRO(0);
	for (size_t output = 0; output < wtx->num_outputs; output++) {
		...
		utxo->amount = amount_asset_to_bro(&asset);
		utxo->status = OUTPUT_STATE_AVAILABLE;
		...
		if (!amount_bro_add(total, *total, utxo->amount))
			fatal("Cannot add utxo output %zu/%zu %s + %s",
			      output, wtx->num_outputs,
			      type_to_string(tmpctx, struct amount_bro, total),
			      type_to_string(tmpctx, struct amount_bro,
					     &utxo->amount));
			 ...
			 const struct channel *chan, struct htlc_in *in)
{
	...
	db_bind_amount_mbro(stmt, 3, &in->mbro);
			  ...
			  struct htlc_out *out)
{
	...
	if (out->in)
		...
		db_bind_null(stmt, 3);
	db_bind_amount_mbro(stmt, 4, &out->mbro);
	...
	db_bind_amount_mbro(stmt, 12, &out->fees);
				...
				struct db_stmt *stmt, struct htlc_in *in)
{
	...
	db_col_amount_mbro(stmt, "mbronee", &in->mbro);
				 ...
				 struct htlc_in_map *unconnected_htlcs_in)
{
	...
	db_col_amount_mbro(stmt, "mbronee", &out->mbro);
	...
	out->in = NULL;
	db_col_amount_mbro(stmt, "fees_mbro", &out->fees);
...
static void fixup_hin(struct wallet *wallet, struct htlc_in *hin)
{
	...
	log_broken(wallet->log, "HTLC #%"PRIu64" (%s) "
		   ...
		   type_to_string(tmpctx, struct amount_mbro, &hin->mbro),
...
bool wallet_invoice_create(struct wallet *wallet,
			   ...
			   const struct amount_mbro *mbro TAKES,
...
{
	return invoices_create(wallet->invoices, pinvoice, mbro, label, expiry, b11enc, description, features, r, rhash, local_offer_id);
}
...
bool wallet_invoice_resolve(struct wallet *wallet,
			    struct invoice invoice,
			    struct amount_mbro mbronee_received)
{
	return invoices_resolve(wallet->invoices, invoice, mbronee_received);
}
			  ...
			  struct wallet_payment *payment TAKES)
{
	...
	db_bind_amount_mbro(stmt, 3, &payment->mbronee);
	...
	db_bind_amount_mbro(stmt, 8, &payment->mbronee_sent);
	...
	db_bind_amount_mbro(stmt, 11, &payment->total_mbro);
						  ...
						  struct db_stmt *stmt)
{
	...
	db_col_amount_mbro(stmt, "mbronee", &payment->mbronee);
	...
	db_col_amount_mbro(stmt, "mbronee_sent", &payment->mbronee_sent);
	...
	if (!db_col_is_null(stmt, "total_mbro"))
		db_col_amount_mbro(stmt, "total_mbro", &payment->total_mbro);
	else
		payment->total_mbro = AMOUNT_MBRO(0);
...
void wallet_htlc_sigs_save(struct wallet *w, u64 channel_id,
			   const struct brocoin_signature *htlc_sigs)
{
...
{
	struct brocoin_blkid chainhash;
	struct db_stmt *stmt = db_prepare_v2(
	...
	if (db_step(stmt)) {
		...
		if (!brocoin_blkid_eq(&chainhash,
			...
			log_broken(w->log, "Wallet blockchain hash does not "
					   ...
					   "(--network={one of %s})",
				   type_to_string(w, struct brocoin_blkid,
						  &chainhash),
				   type_to_string(w, struct brocoin_blkid,
						  &chainparams->genesis_blockhash),
...
void wallet_filteredblock_add(struct wallet *w, const struct filteredblock *fb)
{
	...
	for (size_t i = 0; i < tal_count(fb->outpoints); i++) {
		...
		db_bind_amount_bro(stmt, 6, &o->amount);
		db_exec_prepared_v2(take(stmt));
					  ...
					  const struct short_channel_id *scid)
{
	...
	db_col_amount_bro(stmt, "bronees", &op->bro);
	tal_free(stmt);
...
static void wallet_annotation_add(struct wallet *w, const struct brocoin_txid *txid, int num,
...
void wallet_annotate_txin(struct wallet *w, const struct brocoin_txid *txid,
...
void wallet_transaction_annotate(struct wallet *w,
				 const struct brocoin_txid *txid, enum wallet_tx_type type,
				 u64 channel_id)
{
	struct db_stmt *stmt = db_prepare_v2(
	...
	if (!db_step(stmt))
		fatal("Attempting to annotate a transaction we don't have: %s",
		      type_to_string(tmpctx, struct brocoin_txid, txid));
...
bool wallet_transaction_type(struct wallet *w, const struct brocoin_txid *txid,
...
struct brocoin_tx *wallet_transaction_get(const tal_t *ctx, struct wallet *w,
					  const struct brocoin_txid *txid)
{
...
u32 wallet_transaction_height(struct wallet *w, const struct brocoin_txid *txid)
{
	u32 blockheight;
	struct db_stmt *stmt = db_prepare_v2(
	    w->db, SQL("SELECT blockheight FROM transactions WHERE id=?"));
...
struct brocoin_txid *wallet_transactions_by_height(const tal_t *ctx,
						   struct wallet *w,
						   const u32 blockheight)
{
	struct db_stmt *stmt;
	struct brocoin_txid *txids = tal_arr(ctx, struct brocoin_txid, 0);
	int count = 0;
	stmt = db_prepare_v2(
...
void wallet_channeltxs_add(struct wallet *w, struct channel *chan,
			   const int type, const struct brocoin_txid *txid,
					    ...
					    enum forward_style forward_style)
{
	...
	stmt = db_prepare_v2(w->db,
				 ...
				 ", forward_style=?"
				 " WHERE in_htlc_id=?"));
	db_bind_amount_mbro(stmt, 0, &in->mbro);

	if (out) {
		db_bind_amount_mbro(stmt, 1, &out->mbro);
	} else {
		db_bind_null(stmt, 1);
	}
				  ...
				  enum onion_wire failcode)
{
	...
	if (out) {
		...
		db_bind_amount_mbro(stmt, 5, &out->mbro);
	} else {
	...
	db_bind_amount_mbro(stmt, 4, &in->mbro);
...
notify:
	notify_forward_event(w->ld, in, scid_out, out ? &out->mbro : NULL,
			     state, failcode, resolved_time, forward_style);
}

struct amount_mbro wallet_total_forward_fees(struct wallet *w)
{
	struct db_stmt *stmt;
	struct amount_mbro total;
	bool res;

	stmt = db_prepare_v2(w->db, SQL("SELECT"
					" CAST(COALESCE(SUM(in_mbronee - out_mbronee), 0) AS BIGINT)"
	...
	db_col_amount_mbro(stmt, "CAST(COALESCE(SUM(in_mbronee - out_mbronee), 0) AS BIGINT)", &total);
	tal_free(stmt);
						       ...
						       const struct short_channel_id *chan_out)
{
	...
	for (count=0; db_step(stmt); count++) {
		...
		db_col_amount_mbro(stmt, "in_mbronee", &cur->mbro_in);

		if (!db_col_is_null(stmt, "out_mbronee")) {
			db_col_amount_mbro(stmt, "out_mbronee", &cur->mbro_out);
			if (!amount_mbro_sub(&cur->fee, cur->mbro_in, cur->mbro_out)) {
				log_broken(w->log, "Forwarded in %s less than out %s!",
					   type_to_string(tmpctx, struct amount_mbro,
							  &cur->mbro_in),
					   type_to_string(tmpctx, struct amount_mbro,
							  &cur->mbro_out));
				cur->fee = AMOUNT_MBRO(0);
			}
		}
		else {
			assert(cur->status == FORWARD_LOCAL_FAILED);
			cur->mbro_out = AMOUNT_MBRO(0);
...
struct wallet_transaction *wallet_transactions_get(struct wallet *w, const tal_t *ctx)
{
	...
	for (count = 0; db_step(stmt); count++) {
		...
		if (!brocoin_txid_eq(&last, &curtxid)) {
			last = curtxid;
			     ...
			     const struct penalty_base *pb)
{
	...
	db_bind_amount_bro(stmt, 4, &pb->amount);
							  ...
							  u64 chan_id)
{
	...
	while (db_step(stmt)) {
		...
		db_col_amount_bro(stmt, "amount", &pb.amount);
