		      ...
		      u8 **out_redeemscript)
{
	...
	if (is_p2sh_p2wpkh) {
		redeemscript = brocoin_redeem_p2sh_p2wpkh(ctx, pubkey);
			  ...
			  const struct utxo *utxo)
{
	...
	json_add_amount_bro_compat(response, utxo->amount,
				   "value", "amount_mbro");
	...
	if (utxo->is_p2sh) {
		...
		json_add_hex_talarr(response, "redeemscript",
				    brocoin_redeem_p2sh_p2wpkh(tmpctx, &key));
	}
	...
	list_for_each(&cmd->ld->peers, p, list) {
		...
		list_for_each(&p->channels, c, list) {
			...
			json_add_amount_bro_compat(response,
						   amount_mbro_to_bro_round_down(c->our_mbro),
						   "channel_bro",
						   "our_amount_mbro");
			json_add_amount_bro_compat(response, c->funding_bros,
						   "channel_total_bro",
						   "amount_mbro");
...
static void process_utxo_result(struct brocoind *brocoind,
				...
				void *arg)
{
	...
	wallet_update_output_status(brocoind->ld->wallet, &u->outpoint,
				    u->status, newstate);
	...
	if (tal_count(rescan->utxos) == 0) {
		...
		brocoind_getutxout(brocoind->ld->topology->brocoind,
...
static const struct json_command dev_rescan_output_command = {
	...
	false,
	"For each output stored in the internal wallet ask `brocoind` whether we are in sync with its state (spent vs. unspent)"
};
				     ...
				     const struct wallet_transaction *tx)
{
	struct wally_tx *wtx = tx->tx->wtx;
		...
		for (size_t i = 0; i < wtx->num_inputs; i++) {
			struct brocoin_txid prevtxid;
			struct wally_tx_input *in = &wtx->inputs[i];
			brocoin_tx_input_get_txid(tx->tx, i, &prevtxid);
		...
		for (size_t i = 0; i < wtx->num_outputs; i++) {
			...
			struct amount_asset amt = brocoin_tx_output_get_amount(tx->tx, i);
			struct amount_bro bro;

			/* TODO We should eventually handle non-brocoin assets as well. */
			if (amount_asset_is_main(&amt))
				bro = amount_asset_to_bro(&amt);
			else
				bro = AMOUNT_BRO(0);
			...
			if (deprecated_apis)
				json_add_amount_bro_only(response, "bronees", bro);
			json_add_amount_bro_only(response, "mbro", bro);
...
static void sendpsbt_done(struct brocoind *brocoind UNUSED,
					    ...
					    const jsmntok_t *params)
{
	...
	brocoind_sendrawtx(cmd->ld->topology->brocoind,
			   tal_hex(tmpctx,
