...
There are a number of conventions adhered to throughout this document:
 ...
 - HMAC: the integrity verification of the packet is based on Keyed-Hash
   ...
   algorithm.
 - Elliptic curve: for all computations involving elliptic curves, the Brocoin
...
The writer:
  ...
  - For the final node:
    ...
    - if the recipient provided `payment_secret`:
      ...
      - MUST set `total_mbro` to the total amount it will send

The reader:
  - MUST return an error if `amt_to_forward` or `outgoing_cltv_value` are not present.
  - if it is the final node:
    - MUST treat `total_mbro` as if it were equal to `amt_to_forward` if it
      is not present.
...
`total_mbro` field containing a greater value is a promise by the
...
The writer:
  - if the invoice offers the `basic_mpp` feature:
    ...
    - if the invoice specifies an `amount`:
       - MUST set `total_mbro` to at least that `amount`, and less
         than or equal to twice `amount`.
    - otherwise:
      - MUST set `total_mbro` to the amount it wishes to pay.
    - MUST ensure that the total `amount_mbro` of the HTLC set which arrives at the payee
      is equal to `total_mbro`.
    - MUST NOT send another HTLC if the total `amount_mbro` of the HTLC set is already greater or equal to `total_mbro`.
    - MUST include `payment_secret`.
  - otherwise:
    - MUST set `total_mbro` equal to `amt_to_forward`.

The final node:
  - MUST fail the HTLC if dictated by Requirements under [Failure Messages](#failure-messages)
    - Note: "amount paid" specified there is the `total_mbro` field.
  - if it does not support `basic_mpp`:
    - MUST fail the HTLC if `total_mbro` is not exactly equal to `amt_to_forward`.
  - otherwise, if it supports `basic_mpp`:
    - MUST add it to the HTLC set corresponding to that `payment_hash`.
    - SHOULD fail the entire HTLC set if `total_mbro` is not the same for
      all HTLCs in the set.
    - if the total `amount_mbro` of this HTLC set equals `total_mbro`:
      - SHOULD fulfill all HTLCs in the HTLC set
    - otherwise, if the total `amount_mbro` of this HTLC set is less than
      `total_mbro`:
...
```Go
func NewOnionPacket(paymentPath []*bronec.PublicKey, sessionKey *bronec.PrivateKey,
	...
	for i := 0; i < numHops; i++ {
		...
		ephemeralPrivKey := bronec.PrivKeyFromBytes(bronec.S256(), ephemeralKey.Bytes())
		...
		ephemeralKey.Mod(&ephemeralKey, bronec.S256().Params().N)
	}
...
The HTLC amount was below the `htlc_minimum_mbro` of the channel from
the processing node.
...
the channel from the processing node: it does not broisfy the following
requirement:
...
The `htlc_mbro` parameter is superfluous, but left in for backwards
compatibility. The value of `htlc_mbro` always matches the amount specified in
