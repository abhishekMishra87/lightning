...
The `announcement_signatures` message is created by constructing a `channel_announcement` message, corresponding to the newly established channel, and signing it with the secrets matching an endpoint's `node_id` and `brocoin_key`. After it's signed, the
...
A recipient node:
  ...
  - if the `node_signature` OR the `brocoin_signature` is NOT correct:
...
each on-chain Brocoin key to the associated Lightning node key, and vice-versa.
...
1. proving that the funding transaction pays to `brocoin_key_1` and
   `brocoin_key_2`
2. proving that `node_1` owns `brocoin_key_1`
3. proving that `node_2` owns `brocoin_key_2`
...
`brocoin_signature_1` and `brocoin_signature_2` are generated for each
`brocoin_key` and each of the corresponding `node_id`s are signed.
...
2. data:
    ...
    * [`signature`:`brocoin_signature_1`]
    * [`signature`:`brocoin_signature_2`]
    * [`u16`:`len`]
    * [`len*byte`:`features`]
...
The origin node:
  ...
  - MUST set `brocoin_key_1` and `brocoin_key_2` to `node_id_1` and `node_id_2`'s
  ...
  - MUST set `node_signature_1` and `node_signature_2` to valid
    ...
    secrets).
  - MUST set `brocoin_signature_1` and `brocoin_signature_2` to valid
  signatures of the hash `h` (using `brocoin_key_1` and `brocoin_key_2`'s
  respective secrets).
...
The receiving node:
  ...
  - if the `short_channel_id`'s output does NOT correspond to a P2WSH (using
    `brocoin_key_1` and `brocoin_key_2`, as specified in
  ...
  - if the specified `chain_hash` is unknown to the receiver:
    - MUST ignore the message.
  - otherwise:
    - if `brocoin_signature_1`, `brocoin_signature_2`, `node_signature_1` OR
...
Brocoin signatures proves that they control the channel.
...
of *relaying* payments, not *sending* payments. When making a payment
 ...
 value for `amount_mbro` and the minimal value for `cltv_expiry`, to be used for
...
2. data:
    ...
    * [`u64`:`htlc_maximum_mbro`] (option_channel_htlc_max)
...
| ------------- | ------------------------- | -------------------------------- |
| 0             | `option_channel_htlc_max` | `htlc_maximum_mbro`              |

Note that the `htlc_maximum_mbro` field is static in the current
...
The origin node:
  ...
  - if the `htlc_maximum_mbro` field is present:
    - MUST set the `option_channel_htlc_max` bit of `message_flags` to 1.
    - MUST set `htlc_maximum_mbro` to the maximum value it will send through this channel for a single HTLC.
      - MUST set this to less than or equal to the channel capacity.
      - MUST set this to less than or equal to `max_htlc_value_in_flight_mbro`
  ...
  - MUST set `htlc_minimum_mbro` to the minimum HTLC value (in millibronee)
  that the channel peer will accept.
  - MUST set `fee_base_mbro` to the base fee (in millibronee) it will charge
  for any HTLC.
  - MUST set `fee_proportional_millionths` to the amount (in millionths of a
  bronee) it will charge per transferred bronee.
...
The receiving node:
  ...
  - if the `option_channel_htlc_max` bit of `message_flags` is 0:
    - MUST consider `htlc_maximum_mbro` not to be present.
  - otherwise:
    - if `htlc_maximum_mbro` is not present or greater than channel capacity:
      - MAY blacklist this `node_id`
      - SHOULD ignore this channel during route considerations.
    - otherwise:
      - SHOULD consider the `htlc_maximum_mbro` when routing.
...
of `htlc_maximum_mbro` (rather than having `htlc_maximum_mbro` implied
...
millibronees in Brocoin, the `htlc_maximum_mbro` has the same restriction.
...
1. A->B: `cltv_expiry_delta` = 10, `fee_base_mbro` = 100, `fee_proportional_millionths` = 1000
1. A->D: `cltv_expiry_delta` = 10, `fee_base_mbro` = 100, `fee_proportional_millionths` = 1000
1. B->A: `cltv_expiry_delta` = 20, `fee_base_mbro` = 200, `fee_proportional_millionths` = 2000
1. D->A: `cltv_expiry_delta` = 40, `fee_base_mbro` = 400, `fee_proportional_millionths` = 4000
1. B->C: `cltv_expiry_delta` = 20, `fee_base_mbro` = 200, `fee_proportional_millionths` = 2000
1. D->C: `cltv_expiry_delta` = 40, `fee_base_mbro` = 400, `fee_proportional_millionths` = 4000
1. C->B: `cltv_expiry_delta` = 30, `fee_base_mbro` = 300, `fee_proportional_millionths` = 3000
1. C->D: `cltv_expiry_delta` = 30, `fee_base_mbro` = 300, `fee_proportional_millionths` = 3000

**B->C.** If B were to send 4,999,999 millibronee directly to C, it would
...
**A->B->C.** If A were to send 4,999,999 millibronee to C via B, it needs to
