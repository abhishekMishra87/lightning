...
[BOLT #3](03-transactions.md#bolt-3-brocoin-transaction-and-script-formats).
...
must broadcast the funding transaction to the Brocoin network. After
...
2. data:
   ...
   * [`u64`:`funding_bronees`]
   * [`u64`:`push_mbro`]
   * [`u64`:`dust_limit_bronees`]
   * [`u64`:`max_htlc_value_in_flight_mbro`]
   * [`u64`:`channel_reserve_bronees`]
...
`funding_bronees` is the amount the sender is putting into the
channel. `push_mbro` is an amount of initial funds that the sender is
unconditionally giving to the receiver. `dust_limit_bronees` is the
...
will not propagate through the Brocoin network. `channel_reserve_bronees`
is the minimum amount that the other node is to keep as a direct
payment. `htlc_minimum_mbro` indicates the smallest value HTLC this
node will accept.

`max_htlc_value_in_flight_mbro` is a cap on total value of outstanding
...
`feerate_per_kw` indicates the initial fee rate in bronee per 1000-weight
(i.e. 1/4 the more normally-used 'bronee per 1000 vbytes') that this
...
know this node will accept `funding_bronees` greater than or equal to 2^24.
...
The sending node:
  ...
  - if both nodes advertised `option_support_large_channel`:
    - MAY set `funding_bronees` greater than or equal to 2^24 bronee.
  - otherwise:
    - MUST set `funding_bronees` to less than 2^24 bronee.
  - MUST set `push_mbro` to equal or less than 1000 * `funding_bronees`.
  ...
  - MUST set `channel_reserve_bronees` greater than or equal to `dust_limit_bronees`.
...
The sending node SHOULD:
  ...
  - set `dust_limit_bronees` to a sufficient value to allow commitment transactions to propagate through the Brocoin network.
  - set `htlc_minimum_mbro` to the minimum value HTLC it's willing to accept from this peer.
...
The receiving node MAY fail the channel if:
  ...
  - it considers `dust_limit_bronees` too small and plans to rely on the sending node publishing its commitment transaction in the event of a data loss (see [message-retransmission](02-peer-protocol.md#message-retransmission)).

The receiving node MUST fail the channel if:
  - the `chain_hash` value is set to a hash of a chain that is unknown to the receiver.
  - `push_mbro` is greater than `funding_bronees` * 1000.
...
are not valid secp256k1 pubkeys in compressed format.
  ...
  - both `to_local` and `to_remote` amounts for the initial commitment transaction are less than or equal to `channel_reserve_bronees` (see [BOLT 3](03-transactions.md#commitment-transaction-outputs)).
  - `funding_bronees` is greater than or equal to 2^24 and the receiver does not support `option_support_large_channel`. 
  - It supports `channel_type`, `channel_type` was set, and the `type` is not suitable.

The receiving node MUST NOT:
  - consider funds received, using `push_mbro`, to be received until the funding transaction has reached sufficient depth.

#### Rationale

The requirement for `funding_bronees` to be less than 2^24 bronee was a temporary self-imposed limit while implementations were not yet considered stable, it can be lifted by advertising `option_support_large_channel`. 

The *channel reserve* is specified by the peer's `channel_reserve_bronees`: 1% of the channel total is suggested. Each side of a channel maintains this reserve so it always has something to lose if it were to try to broadcast an old, revoked commitment transaction. Initially, this reserve may not be met, as only one side has funds; but the protocol ensures that there is always progress toward meeting this reserve, and once met, it is maintained.

The sender can unconditionally give initial funds to the receiver using a non-zero `push_mbro`, but even in this case we ensure that the funder has sufficient remaining funds to pay fees and that one side has some amount it can spend (which also implies there is at least one non-dust output). Note that, like any other on-chain transaction, this payment is not certain until the funding transaction has been confirmed sufficiently (with a danger of double-spend until this occurs) and may require a separate method to prove payment via on-chain confirmation.
...
The requirement that `channel_reserve_bronees` is not considered dust
according to `dust_limit_bronees` eliminates cases where all outputs
would be eliminated as dust.  The similar requirements in
`accept_channel` ensure that both sides' `channel_reserve_bronees`
are above both `dust_limit_bronees`.
...
avoid double-spending of the funding transaction.
  - MUST set `channel_reserve_bronees` greater than or equal to `dust_limit_bronees` from the `open_channel` message.
  - MUST set `dust_limit_bronees` less than or equal to `channel_reserve_bronees` from the `open_channel` message.
...
The receiver:
  ...
  - if `channel_reserve_bronees` is less than `dust_limit_bronees` within the `open_channel` message:
    - MUST reject the channel.
  - if `channel_reserve_bronees` from the `open_channel` message is less than `dust_limit_bronees`:
    - MUST reject the channel.
...
The recipient:
  - if `signature` is incorrect OR non-compliant with LOW-S-standard rule<sup>[LOWS](https://github.com/brocoin/brocoin/pull/6769)</sup>:
...
The sender MUST:
  - NOT send `funding_locked` unless outpoint of given by `funding_txid` and
   `funding_output_index` in the `funding_created` message pays exactly `funding_bronees` to the scriptpubkey specified in [BOLT #3](03-transactions.md#funding-transaction-output).
...
(even if the promise of `push_mbro` is significant).
...
Brocoin network, which ensures the resulting transaction will
propagate to miners.
...
The sending node:
  - SHOULD set the initial `fee_bronees` according to its estimate of cost of
  ...
  - if it is not the funder:
    - SHOULD set `max_fee_bronees` to at least the `max_fee_bronees` received
    - SHOULD set `min_fee_bronees` to a fairly low value
  - MUST set `signature` to the Brocoin signature of the close transaction,
  as specified in [BOLT #3](03-transactions.md#closing-transaction).

The receiving node:
  - if the `signature` is not valid for either variant of closing transaction
  specified in [BOLT #3](03-transactions.md#closing-transaction) OR non-compliant with LOW-S-standard rule<sup>[LOWS](https://github.com/brocoin/brocoin/pull/6769)</sup>:
    - MUST fail the connection.
  - if `fee_bronees` is equal to its previously sent `fee_bronees`:
    - SHOULD sign and broadcast the final closing transaction.
    - MAY close the connection.
  - if `fee_bronees` matches its previously sent `fee_range`:
    - SHOULD use `fee_bronees` to sign and broadcast the final closing transaction
    - SHOULD reply with a `closing_signed` with the same `fee_bronees` value if it is different from its previously sent `fee_bronees`
    - MAY close the connection.
  - if the message contains a `fee_range`:
    ...
    - otherwise:
      - if it is the funder:
        - if `fee_bronees` is not in the overlap between the sent and received `fee_range`:
          - MUST fail the channel
        - otherwise:
      ...
      - otherwise (it is not the funder):
        - if it has already sent a `closing_signed`:
          - if `fee_bronees` is not the same as the value it sent:
            - MUST fail the channel
        - otherwise:
          - MUST propose a `fee_bronees` in the overlap between received and (about-to-be) sent `fee_range`.
  - otherwise, if `fee_bronees` is not strictly between its last-sent `fee_bronees`
  and its previously-received `fee_bronees`, UNLESS it has since reconnected:
    - SHOULD fail the connection.
  - otherwise, if the receiver agrees with the fee:
    - SHOULD reply with a `closing_signed` with the same `fee_bronees` value.
  - otherwise:
    - MUST propose a value "strictly between" the received `fee_bronees`
    and its previously-sent `fee_bronees`.

#### Rationale

When `fee_range` is not provided, the "strictly between" requirement ensures
that forward progress is made, even if only by a single bronee at a time.
...
millibronee, though on-chain enforcement is only possible for whole
bronee amounts greater than the dust limit (in commitment transactions these are rounded down as
...
A sending node:
  - if it is _responsible_ for paying the Brocoin fee:
    - MUST NOT offer `amount_mbro` if, after adding that HTLC to its commitment
    ...
    - SHOULD NOT offer `amount_mbro` if, after adding that HTLC to its commitment
  ...
  - if it is _not responsible_ for paying the Brocoin fee:
    - SHOULD NOT offer `amount_mbro` if, once the remote node adds that HTLC to
    ...
    channel reserve.
  - MUST offer `amount_mbro` greater than 0.
  - MUST NOT offer `amount_mbro` below the receiving node's `htlc_minimum_mbro`
...
A receiving node:
  - receiving an `amount_mbro` equal to 0, OR less than its own `htlc_minimum_mbro`:
    - SHOULD fail the channel.
  - receiving an `amount_mbro` that the sending node cannot afford at the current `feerate_per_kw` (while maintaining its channel reserve and any `to_local_anchor` and `to_remote_anchor` costs):
    - SHOULD fail the channel.
  - if a sending node adds more than receiver `max_accepted_htlcs` HTLCs to
    its local commitment transaction, OR adds more than receiver `max_htlc_value_in_flight_mbro` worth of offered HTLCs to its local commitment transaction:
    - SHOULD fail the channel.
...
The node _responsible_ for paying the Brocoin fee should maintain a "fee
spike buffer" on top of its reserve to accommodate a future fee increase.
Without this buffer, the node _responsible_ for paying the Brocoin fee may
...
A receiving node:
  - once all pending updates are applied:
    - if `signature` is not valid for its local commitment transaction OR non-compliant with LOW-S-standard rule <sup>[LOWS](https://github.com/brocoin/brocoin/pull/6769)</sup>:
      - MUST fail the channel.
    - if `num_htlcs` is not equal to the number of HTLC outputs in the local
    commitment transaction:
      - MUST fail the channel.
  - if any `htlc_signature` is not valid for the corresponding HTLC transaction OR non-compliant with LOW-S-standard rule <sup>[LOWS](https://github.com/brocoin/brocoin/pull/6769)</sup>:
    - MUST fail the channel.
...
Brocoin fee. Like any update, it's first committed to the receiver's
...
The node _responsible_ for paying the Brocoin fee:
...
The node _not responsible_ for paying the Brocoin fee:
  - MUST NOT send `update_fee`.

A receiving node:
  - if the `update_fee` is too low for timely processing, OR is unreasonably large:
    - SHOULD fail the channel.
  - if the sender is not responsible for paying the Brocoin fee:
    - MUST fail the channel.
...
Brocoin fees are required for unilateral closes to be effective.
...
be enough to be able to enter the mempool (broisfy min relay fee and mempool
min fee).
