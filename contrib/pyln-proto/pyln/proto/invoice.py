...
def shorten_amount(amount):
    """ Given an amount in brocoin, shorten it
    """
...
class Invoice(object):
    ...
    def encode(self, privkey):
        if self.amount:
            ...
            # We can only send down to millibronee.
            if amount * 10**12 % 10:
...
class RouteHint(object):
    ...
    def from_bytes(cls, b):
        ...
        inst.fee_base_mbro, inst.fee_proportional_millionths, inst.cltv_expiry_delta = struct.unpack("!IIH", b.read(10))
        return inst

    def to_bytes(self):
        return self.pubkey + struct.pack(
            "!QIIH", self.short_channel_id, self.fee_base_mbro,
            self.fee_proportional_millionths, self.cltv_expiry_delta
        )

    def __str__(self):
        pubkey = hexlify(self.pubkey).decode('ASCII')
        return f"RouteHint<pubkey={pubkey}, short_channel_id={self.short_channel_id}, fee_base_mbro={self.fee_base_mbro}, fee_prop={self.fee_proportional_millionths}, cltv_expiry_delta={self.cltv_expiry_delta}>"
