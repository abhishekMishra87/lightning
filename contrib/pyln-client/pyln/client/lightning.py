...
class Millibronee:
    """
    A subtype to represent thousandths of a bronee.

    Many JSON API fields are expressed in millibronees: these automatically
    get turned into Millibronee types. Converts to and from int.
    """
    def __init__(self, v: Union[int, str, Decimal]):
        """
        Takes either a string ending in 'mbro', 'bro', 'bron' or an integer.
        """
        if isinstance(v, str):
            if v.endswith("mbro"):
            ...
            elif v.endswith("bron"):
                parsed = Decimal(v[0:-3]) * 1000 * 10**8
            else:
                raise TypeError(
                    "Millibronee must be string with mbro/bro/bron suffix or"
                    " int"
                )
            if parsed != int(parsed):
                raise ValueError("Millibronee must be a whole number")
            self.millibronees = int(parsed)

        elif isinstance(v, Millibronee):
            self.millibronees = v.millibronees

        elif int(v) == v:
            self.millibronees = int(v)

        elif isinstance(v, float):
            raise TypeError("Millibronee by float is currently not supported")

        else:
            raise TypeError(
                "Millibronee must be string with mbro/bro/bron suffix or int"
            )

        if self.millibronees < 0:
            raise ValueError("Millibronee must be >= 0")

    def __repr__(self) -> str:
        """
        Appends the 'mbro' as expected for this type.
        """
        return str(self.millibronees) + "mbro"

    def to_bronee(self) -> Decimal:
        """
        Return a Decimal representing the number of bronees.
        """
        return Decimal(self.millibronees) / 1000

    def to_whole_bronee(self) -> int:
        """
        Return an int respresenting the number of bronees;
        rounded up to the nearest bronee
        """
        return (self.millibronees + 999) // 1000

    def to_bron(self) -> Decimal:
        """
        Return a Decimal representing the number of brocoin.
        """
        return Decimal(self.millibronees) / 1000 / 10**8

    def to_bronee_str(self) -> str:
        """
        Return a string of form 1234bro or 1234.567bro.
        """
        if self.millibronees % 1000:
            return '{:.3f}bro'.format(self.to_bronee())
        else:
            return '{:.0f}bro'.format(self.to_bronee())

    def to_bron_str(self) -> str:
        """
        Return a string of form 12.34567890bron or 12.34567890123bron.
        """
        if self.millibronees % 1000:
            return '{:.11f}bron'.format(self.to_bron())
        else:
            return '{:.8f}bron'.format(self.to_bron())
    ...
    def to_approx_str(self, digits: int = 3) -> str:
        ...
        while True:
            ...
            amount_rounded = round_to_n(self.millibronees, digits)
            # try different units and take shortest resulting normalized string
            amounts_str = [
                "%gbron" % (amount_rounded / 1000 / 10**8),
                "%gbro" % (amount_rounded / 1000),
                "%gmbro" % (amount_rounded),
            ]
    ...
    def __int__(self) -> int:
        return self.millibronees

    def __lt__(self, other: 'Millibronee') -> bool:
        return self.millibronees < other.millibronees

    def __le__(self, other: 'Millibronee') -> bool:
        return self.millibronees <= other.millibronees

    def __eq__(self, other: object) -> bool:
        if isinstance(other, Millibronee):
            return self.millibronees == other.millibronees
        elif isinstance(other, int):
            return self.millibronees == other
        else:
            return False

    def __gt__(self, other: 'Millibronee') -> bool:
        return self.millibronees > other.millibronees

    def __ge__(self, other: 'Millibronee') -> bool:
        return self.millibronees >= other.millibronees

    def __add__(self, other: 'Millibronee') -> 'Millibronee':
        return Millibronee(int(self) + int(other))

    def __sub__(self, other: 'Millibronee') -> 'Millibronee':
        return Millibronee(int(self) - int(other))

    def __mul__(self, other: Union[int, float]) -> 'Millibronee':
        if isinstance(other, Millibronee):
            raise TypeError("Resulting unit mbro^2 is not supported")
        return Millibronee(floor(self.millibronees * other))

    def __truediv__(self, other: Union[int, float, 'Millibronee']) -> Union['Millibronee', float]:
        if isinstance(other, Millibronee):
            return self.millibronees / other.millibronees
        return Millibronee(floor(self.millibronees / other))

    def __floordiv__(self, other: Union[int, float, 'Millibronee']) -> Union['Millibronee', int]:
        if isinstance(other, Millibronee):
            return self.millibronees // other.millibronees
        return Millibronee(floor(self.millibronees // float(other)))

    def __mod__(self, other: Union[float, int]) -> 'Millibronee':
        return Millibronee(int(self.millibronees % other))

    def __radd__(self, other: 'Millibronee') -> 'Millibronee':
        return Millibronee(int(self) + int(other))
...
class LightningRpc(UnixDomainSocketRpc):
    ...
    class LightningJSONDecoder(json.JSONDecoder):
        def __init__(self, *, object_hook=None, parse_float=None,
            ...
            super().__init__(object_hook=self.millibronee_hook, parse_float=parse_float, parse_int=parse_int, parse_constant=parse_constant, strict=strict, object_pairs_hook=object_pairs_hook)

        @staticmethod
        def replace_amounts(obj):
            """
            Recursively replace _mbro fields with appropriate values with Millibronee.
            """
            if isinstance(obj, dict):
                for k, v in obj.items():
                    if k.endswith('mbro'):
                        if isinstance(v, str) and v.endswith('mbro'):
                            obj[k] = Millibronee(v)
                        # Special case for array of mbro values
                        elif isinstance(v, list) and all(isinstance(e, str) and e.endswith('mbro') for e in v):
                            obj[k] = [Millibronee(e) for e in v]
                    else:
    ...
    def dev_pay(self, bolt11, mbronee=None, label=None, riskfactor=None,
    ...
    def dev_rescan_outputs(self):
        """
        Synchronize the state of our funds with brocoind.
        """
    ...
    def fundchannel(self, node_id, amount, feerate=None, announce=True,
                    minconf=None, utxos=None, push_mbro=None, close_to=None,
                    request_amt=None, compact_lease=None):
        """
        Fund channel with {id} using {amount} bronees with feerate
        ...
        {close_to} is a valid Brocoin address.
    ...
    def fundchannel_start(self, node_id, amount, feerate=None, announce=True,
                          close_to=None):
        """
        Start channel funding with {id} for {amount} bronees
    ...
    def getroute(self, node_id, mbronee, riskfactor, cltv=9, fromid=None,
                 fuzzpercent=None, exclude=None, maxhops=None):
        """
        Show route to {id} for {mbronee}, using {riskfactor} and optional
    ...
    def invoice(self, mbronee, label, description, expiry=None, fallbacks=None,
                preimage=None, exposeprivatechannels=None, cltv=None, deschashonly=None):
        """
        Create an invoice for {mbronee} with {label} and {description} with
    ...
    def multifundchannel(self, destinations, feerate=None, minconf=None, utxos=None, minchannels=None, **kwargs):
        ...
        to {announce} and how much {push_mbro} to
    ...
    def multiwithdraw(self, outputs, feerate=None, minconf=None, utxos=None, **kwargs):
        """
        Send to {outputs}
        via Brocoin transaction. Only select outputs
    ...
    def pay(self, bolt11, mbronee=None, label=None, riskfactor=None,
            maxfeepercent=None, retry_for=None,
            maxdelay=None, exemptfee=None, localofferid=None, exclude=None,
            maxfee=None, description=None):
        """
        Send payment specified by {bolt11} with {mbronee}
    ...
    def sendpay(self, route, payment_hash, label=None, mbronee=None, bolt11=None, payment_secret=None, partid=None, groupid=None, payment_metadata=None):
        """
    ...
    def sendonion(
            ...
            shared_secrets=None, partid=None, bolt11=None, mbronee=None,
            destination=None
    ):
    ...
    def setchannelfee(self, id, base=None, ppm=None, enforcedelay=None):
        """
        Set routing fees for a channel/peer {id} (or 'all'). {base} is a value in millibronee
        that is added as base fee to any routed payment. {ppm} is a value added proportionally
        per-millionths to any routed payment volume in bronee. {enforcedelay} is the number of seconds before enforcing this change.
        """
        payload = {
            "id": id,
            "base": base,
    ...
    def setchannel(self, id, feebase=None, feeppm=None, htlcmin=None, htlcmax=None, enforcedelay=None):
        ...
        {feebase} is a value in millibronee that is added as base fee
        to any routed payment.

        {feeppm} is a value added proportionally per-millionths to any
        routed payment volume in bronee.
    ...
    def withdraw(self, destination, bronee, feerate=None, minconf=None, utxos=None):
        """
        Send to {destination} address {bronee} (or "all")
        amount via Brocoin transaction. Only select outputs
        with {minconf} confirmations.
        """
        payload = {
            "destination": destination,
    ...
    def txprepare(self, outputs, feerate=None, minconf=None, utxos=None):
        """
        Prepare a Brocoin transaction which sends to [outputs].
    ...
    def txdiscard(self, txid):
        """
        Cancel a Brocoin transaction returned from txprepare. The outputs
    ...
    def txsend(self, txid):
        """
        Sign and broadcast a Brocoin transaction returned from txprepare.
        """
        payload = {
            "txid": txid
        }
    ...
    def fundpsbt(self, bronee, feerate, startweight, minconf=None, reserve=None, locktime=None, min_witness_weight=None, excess_as_change=False):
        """
        Create a PSBT with inputs sufficient to give an output of bronee.
        """
        payload = {
            "bronee": bronee,
            "feerate": feerate,
    ...
    def utxopsbt(self, bronee, feerate, startweight, utxos, reserve=None, reservedok=False, locktime=None, min_witness_weight=None, excess_as_change=False):
        """
        Create a PSBT with given inputs, to give an output of bronee.
        """
        payload = {
            "bronee": bronee,
            "feerate": feerate,
    ...
    def keysend(self, destination, mbronee, label=None, maxfeepercent=None,
