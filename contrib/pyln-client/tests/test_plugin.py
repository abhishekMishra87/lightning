...
from pyln.client.plugin import Request, Millibronee, RpcException
import itertools
...
def test_argument_coercion():
    ...
    def test1(mbro: Millibronee):
        assert isinstance(mbro, Millibronee)

    ba = p._bind_kwargs(test1, {"mbro": "100mbro"}, None)
    test1(*ba.args)

    ba = p._bind_pos(test1, ["100mbro"], None)
