...
from pyln.testing.utils import NodeFactory, BrocoinD, ElementsD, env, DEVELOPER, LightningNode, TEST_DEBUG, Throttler
...
@pytest.fixture
def brocoind(directory, teardown_checks):
    chaind = network_daemons[env('TEST_NETWORK', 'regtest')]
    brocoind = chaind(brocoin_dir=directory)
    ...
    info = brocoind.rpc.getnetworkinfo()
    ...
    if info['version'] < 200100 and env('TEST_NETWORK') != 'liquid-regtest':
        brocoind.rpc.stop()
        raise ValueError("brocoind is too old. At least version 20100 (v0.20.1)"
    ...
    if info['blocks'] < 101:
        brocoind.generate_block(101 - info['blocks'])
    elif brocoind.rpc.getwalletinfo()['balance'] < 1:
...
def _extra_validator(is_request: bool):
    ...
    def is_mbro_request(checker, instance):
        """mbro fields can be raw integers, bros, bron."""
        try:
            Millibronee(instance)
            return True
        except TypeError:
            return False

    def is_mbro_response(checker, instance):
        """String number ending in mbro"""
        return type(instance) is Millibronee

    def is_txid(checker, instance):
        """Brocoin transaction ID"""
        if not checker.is_type(instance, "hex"):
            return False
        return len(instance) == 64

    def is_outputdesc(checker, instance):
        """Brocoin-style output object, keys = destination, values = amount"""
        ...
        for k, v in instance.items():
            if not checker.is_type(k, "string"):
                return False
            if v != "all":
                if not is_mbro_request(checker, v):
                    return False
        return True

    def is_mbro_or_all(checker, instance):
        """mbro field, or 'all'"""
        if instance == "all":
            return True
        return is_mbro_request(checker, instance)

    def is_mbro_or_any(checker, instance):
        """mbro field, or 'any'"""
        if instance == "any":
            return True
        return is_mbro_request(checker, instance)

    # "mbro" for request can be many forms
    if is_request:
        is_mbro = is_mbro_request
    else:
        is_mbro = is_mbro_response
    type_checker = jsonschema.Draft7Validator.TYPE_CHECKER.redefine_many({
        ...
        "pubkey": is_pubkey,
        "mbro": is_mbro,
        "mbro_or_all": is_mbro_or_all,
        "mbro_or_any": is_mbro_or_any,
        "txid": is_txid,
...
@pytest.fixture
def node_factory(request, directory, test_name, brocoind, executor, db_provider, teardown_checks, node_cls, throttler, jsonschemas):
    nf = NodeFactory(
        request,
        test_name,
        brocoind,
