from brocoin.core import COIN  # type: ignore
from brocoin.rpc import RawProxy as BrocoinProxy  # type: ignore
from brocoin.rpc import JSONRPCError
from contextlib import contextmanager
from pathlib import Path
from pyln.client import RpcError
from pyln.testing.bronproxy import BrocoinRpcProxy
...
def sync_blockheight(brocoind, nodes):
    height = brocoind.rpc.getblockchaininfo()['blocks']
    for n in nodes:
        wait_for(lambda: n.rpc.getinfo()['blockheight'] == height)


def mine_funding_to_announce(brocoind, nodes, num_blocks=5, wait_for_mempool=0):
...
announcement.  Not needed if there are only two nodes.
    ...
    """
    brocoind.generate_block(num_blocks - 1, wait_for_mempool)
...
def get_tx_p2wsh_outnum(brocoind, tx, amount):
    """Get output number of this tx which is p2wsh of amount"""
    decoded = brocoind.rpc.decoderawtransaction(tx, True)
...
class SimpleBrocoinProxy:
    ...
    """
    def __init__(self, bron_conf_file, *args, **kwargs):
        self.__bron_conf_file__ = bron_conf_file
    ...
    def __getattr__(self, name):
        ...
        proxy = BrocoinProxy(bron_conf_file=self.__bron_conf_file__)
...
class BrocoinD(TailableProc):

    def __init__(self, brocoin_dir="/tmp/brocoind-test", rpcport=None):
        TailableProc.__init__(self, brocoin_dir, verbose=False)

        if rpcport is None:
            rpcport = reserve()

        self.brocoin_dir = brocoin_dir
        self.rpcport = rpcport
        self.prefix = 'brocoind'

        regtestdir = os.path.join(brocoin_dir, 'regtest')
        if not os.path.exists(regtestdir):
            os.makedirs(regtestdir)

        self.cmd_line = [
            'brocoind',
            '-datadir={}'.format(brocoin_dir),
            '-printtoconsole',
            '-server',
            '-logtimestamps',
            '-nolisten',
        ...
        BROCOIND_CONFIG['rpcport'] = rpcport
        # For after 0.16.1 (eg. 3f398d7a17f136cd4a67998406ca41a124ae2966), this
        # needs its own [regtest] section.
        BROCOIND_REGTEST = {'rpcport': rpcport}
        self.conf_file = os.path.join(brocoin_dir, 'brocoin.conf')
        write_config(self.conf_file, BROCOIND_CONFIG, BROCOIND_REGTEST)
        self.rpc = SimpleBrocoinProxy(bron_conf_file=self.conf_file)
        self.proxies = []
...
class ElementsD(BrocoinD):
    def __init__(self, brocoin_dir="/tmp/brocoind-test", rpcport=None):
        config = BROCOIND_CONFIG.copy()
        if 'regtest' in config:
            del config['regtest']

        config['chain'] = 'liquid-regtest'
        BrocoinD.__init__(self, brocoin_dir, rpcport)
        ...
        conf_file = os.path.join(brocoin_dir, 'elements.conf')
        config['rpcport'] = self.rpcport
        BROCOIND_REGTEST = {'rpcport': self.rpcport}
        write_config(conf_file, config, BROCOIND_REGTEST, section_name='liquid-regtest')
...
class LightningD(TailableProc):
    def __init__(self, lightning_dir, brocoindproxy, port=9443, random_hsm=False, node_id=0):
        ...
        opts = {
            ...
            'brocoin-rpcuser': BROCOIND_CONFIG['rpcuser'],
            'brocoin-rpcpassword': BROCOIND_CONFIG['rpcpassword'],

            # Make sure we don't touch any existing config files in the user's $HOME
            'brocoin-datadir': lightning_dir,
        }
    ...
    def start(self, stdin=None, stdout=None, stderr=None,
        ...
        self.opts['brocoin-rpcport'] = self.rpcproxy.rpcport
...
class LightningNode(object):
    def __init__(self, node_id, lightning_dir, brocoind, executor, valgrind, may_fail=False,
                 may_reconnect=False,
        ...
        self.daemon = LightningD(
            lightning_dir, brocoindproxy=brocoind.get_proxy(),
        ...
        if valgrind:
            trace_skip_pattern = '*python*,*brocoin-cli*,*elements-cli*'
    ...
    def fundwallet(self, bros, addrtype="p2sh-segwit", mine_block=True):
        addr = self.rpc.newaddr(addrtype)[addrtype]
        txid = self.brocoin.rpc.sendtoaddress(addr, bros / 10**8)
        if mine_block:
    ...
    def fundbalancedchannel(self, remote_node, total_capacity, announce=True):
        ...
        '''
        if isinstance(total_capacity, Millibronee):
            total_capacity = int(total_capacity.to_bronee())
        else:
        ...
        res = self.rpc.fundchannel(remote_node.info['id'], chan_capacity, feerate='slow', minconf=0, announce=announce, push_mbro=Millibronee(chan_capacity * 500))
    ...
    def is_synced_with_brocoin(self, info=None):
        if info is None:
            info = self.rpc.getinfo()
        return 'warning_brocoind_sync' not in info and 'warning_lightningd_sync' not in info

    def start(self, wait_for_brocoind_sync=True, stderr=None):
        self.daemon.start(stderr=stderr)
        # Cache `getinfo`, we'll be using it a lot
        self.info = self.rpc.getinfo()
        # This shortcut is sufficient for our simple tests.
        self.port = self.info['binding'][0]['port']
        if wait_for_brocoind_sync and not self.is_synced_with_brocoin(self.info):
            wait_for(lambda: self.is_synced_with_brocoin())
    ...
    # This helper sends all money to a peer until even 1 mbro can't get through.
    def drain(self, peer):
        total = 0
        mbro = 4294967295  # Max payment size in some configs
        while mbro != 0:
            try:
                logging.debug("Drain step with size={}".format(mbro))
                self.pay(peer, mbro)
                total += mbro
            except RpcError as e:
                logging.debug("Got an exception while draining channel: {}".format(e))
                mbro //= 2
        logging.debug("Draining complete after sending a total of {}mbros".format(total))
        return total
    ...
    def set_feerates(self, feerates, wait_for_effect=True):
        # (brocoind returns brocoin per kb, so these are * 4)
    ...
    def dev_pay(self, bolt11, mbronee=None, label=None, riskfactor=None,
        ...
        ret = self.rpc.dev_pay(bolt11, mbronee, label, riskfactor,
                               maxfeepercent, retry_for,
                               maxdelay, exemptfee, use_shadow, exclude)
        self.rpc.check_request_schemas = old_check
        return ret

    def dev_invoice(self, mbronee, label, description, expiry=None, fallbacks=None, preimage=None, exposeprivatechannels=None, cltv=None, dev_routes=None):
...
class NodeFactory(object):
    ...
    """
    def __init__(self, request, testname, brocoind, executor, directory,
    ...
    def get_node(self, node_id=None, options=None, dbfile=None,
                 ...
                 wait_for_brocoind_sync=True, may_fail=False,
        ...
        if start:
            try:
                ...
                if expect_fail:
                    ...
                    stderr = None
                node.start(wait_for_brocoind_sync, stderr=stderr)
            except Exception:
                if expect_fail:
                    return node
                node.daemon.stop()
    ...
    def join_nodes(self, nodes, fundchannel=True, fundamount=FUNDAMOUNT, wait_for_announce=False, announce_channels=True) -> None:
        ...
        brocoind = nodes[0].brocoin
        # If we got here, we want to fund channels
        for src, dst in connections:
            addr = src.rpc.newaddr()['bech32']
            brocoind.rpc.sendtoaddress(addr, (fundamount + 1000000) / 10**8)
        ...
        brocoind.generate_block(1, wait_for_mempool=txids)
        scids = []
        for src, dst in connections:
