...
static struct brocoin_tx *tx_from_hex(const tal_t *ctx, const char *hex)
{
	return brocoin_tx_from_hex(ctx, hex, strlen(hex));
}
...
static const struct htlc **include_htlcs(struct channel *channel, enum side side)
{
	...
	for (i = 0; i < 5; i++) {
		...
		enum side sender;
		struct amount_mbro mbronee = AMOUNT_MBRO(0);

		switch (i) {
		case 0:
			sender = !side;
			mbronee = AMOUNT_MBRO(1000000);
			break;
		case 1:
			sender = !side;
			mbronee = AMOUNT_MBRO(2000000);
			break;
		case 2:
			sender = side;
			mbronee = AMOUNT_MBRO(2000000);
		...
		case 3:
			sender = side;
			mbronee = AMOUNT_MBRO(3000000);
			break;
		case 4:
			sender = !side;
			mbronee = AMOUNT_MBRO(4000000);
			break;
		}
		assert(mbronee.millibronees != 0);

		memset(&preimage, i, sizeof(preimage));
		sha256(&hash, &preimage, sizeof(preimage));
		e = channel_add_htlc(channel, sender, i, mbronee, 500+i, &hash,
...
static void txs_must_be_eq(struct brocoin_tx **a, struct brocoin_tx **b)
{
	size_t i;
				  ...
				  struct amount_mbro mbronee)
{
	...
	assert(channel_add_htlc(channel, sender, 1337, mbronee, 900, &rhash,
				dummy_routing, NULL, NULL, NULL, true)
	       == CHANNEL_ERR_ADD_OK);
...
int main(int argc, const char *argv[])
{
	...
	struct brocoin_tx *raw_tx, **txs, **txs2;
	...
	funding.n = 0;
	funding_amount = AMOUNT_BRO(10000000);

	remote_config->to_self_delay = 144;
	local_config->dust_limit = AMOUNT_BRO(546);
	/* This matters only because we check if added HTLC will create new
	 * output, for fee considerations. */
	remote_config->dust_limit = AMOUNT_BRO(546);

	local_config->max_htlc_value_in_flight = AMOUNT_MBRO(-1ULL);
	remote_config->max_htlc_value_in_flight = AMOUNT_MBRO(-1ULL);
	local_config->max_dust_htlc_exposure_mbro = AMOUNT_MBRO(-1ULL);
	remote_config->max_dust_htlc_exposure_mbro = AMOUNT_MBRO(-1ULL);
	local_config->channel_reserve = AMOUNT_BRO(0);
	remote_config->channel_reserve = AMOUNT_BRO(0);
	local_config->htlc_minimum = AMOUNT_MBRO(0);
	remote_config->htlc_minimum = AMOUNT_MBRO(0);
	...
	*/
	...
	to_local = AMOUNT_MBRO(7000000000);
	to_remote = AMOUNT_MBRO(3000000000);
	 ...
	 *    local_feerate_per_kw: 0
	 */
	to_local = AMOUNT_MBRO(6988000000);
	to_remote = AMOUNT_MBRO(3000000000);
	...
	send_and_fulfill_htlc(lchannel, LOCAL, AMOUNT_MBRO(7000000));
	send_and_fulfill_htlc(rchannel, REMOTE, AMOUNT_MBRO(7000000));

	assert(lchannel->view[LOCAL].owed[LOCAL].millibronees
	       == rchannel->view[REMOTE].owed[REMOTE].millibronees);
	assert(lchannel->view[REMOTE].owed[REMOTE].millibronees
	       == rchannel->view[LOCAL].owed[LOCAL].millibronees);
	...
	brocoin_tx_input_set_witness(raw_tx, 0, NULL);
