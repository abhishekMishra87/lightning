...
/* brocoind loves its backwards txids! */
static struct brocoin_txid txid_from_hex(const char *hex)
{
	struct brocoin_txid txid;

	if (!brocoin_txid_from_hex(hex, strlen(hex), &txid))
		abort();
	return txid;
}
...
static struct secret secret_from_hex(const char *hex)
{
	 ...
	 *   (Brocoin's convention for "compressed" private keys, i.e. keys
...
static void tx_must_be_eq(const struct brocoin_tx *a,
			  const struct brocoin_tx *b)
{
	u8 *lina, *linb;
	size_t i;
...
static const struct htlc **setup_htlcs_0_to_4(const tal_t *ctx)
{
	...
	for (i = 0; i < 5; i++) {
		...
		case 0:
			...
			htlc->amount = AMOUNT_MBRO(1000000);
			break;
		case 1:
			htlc->state = RCVD_ADD_ACK_REVOCATION;
			htlc->amount = AMOUNT_MBRO(2000000);
			break;
		case 2:
		...
		case 3:
			htlc->state = SENT_ADD_ACK_REVOCATION;
			htlc->amount = AMOUNT_MBRO(3000000);
			break;
		case 4:
			htlc->state = RCVD_ADD_ACK_REVOCATION;
			htlc->amount = AMOUNT_MBRO(4000000);
			break;
		}
...
static const struct htlc **setup_htlcs_1_5_and_6(const tal_t *ctx)
{
	...
	for (i = 0; i < 3; i++) {
		...
		case 5:
			htlc->state = SENT_ADD_ACK_REVOCATION;
			htlc->amount = AMOUNT_MBRO(5000000);
			htlc->expiry.locktime = 505;
			memset(htlc->r, 5, sizeof(*htlc->r));
			break;
		case 6:
			htlc->state = SENT_ADD_ACK_REVOCATION;
			htlc->amount = AMOUNT_MBRO(5000001);
...
static void report_htlcs(const struct brocoin_tx *tx,
			 ...
			 bool option_anchor_outputs)
{
	...
	struct brocoin_tx **htlc_tx;
	struct brocoin_signature *remotehtlcsig;
	struct keyset keyset;
	u8 **wscript;

	htlc_tx = tal_arrz(tmpctx, struct brocoin_tx *, tal_count(htlc_map));
	remotehtlcsig = tal_arr(tmpctx, struct brocoin_signature,
				tal_count(htlc_map));
	...
	for (i = 0; i < tal_count(htlc_map); i++) {
		struct brocoin_signature localhtlcsig;
		...
		if (htlc_owner(htlc) == LOCAL) {
			wscript[i] = brocoin_wscript_htlc_offer(tmpctx,
								local_htlckey,
								remote_htlckey,
								&htlc->rhash,
			...
			wscript[i] = brocoin_wscript_htlc_receive(tmpctx,
								  &htlc->expiry,
								  local_htlckey,
								  remote_htlckey,
								  &htlc->rhash,
...
static void report(struct brocoin_tx *tx,
		   const u8 *wscript,
...
{
	char *txhex;
	struct brocoin_signature localsig, remotesig;
	u8 **witness;
	...
	witness =
	    brocoin_witness_2of2(tx, &localsig, &remotesig,
...
#ifdef DEBUG
static struct amount_bro calc_fee(const struct brocoin_tx *tx, struct amount_bro input)
{
	size_t i;
	struct amount_bro output = AMOUNT_BRO(0), fee;

	for (i = 0; i < tal_count(tx->output); i++)
		output.bronees += tx->output[i].amount;

	if (!amount_sub_bro(&fee, input, output))
		abort();
	return fee;
}
...
int main(int argc, const char *argv[])
{
	...
	struct amount_bro funding_amount, dust_limit;
	u32 feerate_per_kw;
	u16 to_self_delay;
	...
	struct brocoin_tx *tx, *tx2;
	struct keyset keyset;
	u8 *wscript;
	u64 commitment_number, cn_obscurer;
	struct amount_mbro to_local, to_remote;
	 ...
	 *  - Private keys are displayed as 32 bytes plus a trailing 1
	 *    (Brocoin's convention for "compressed" private keys, i.e. keys for
	 ...
	 *     funding_amount_bronee: 10000000
	 *     commitment_number: 42
	 *     local_delay: 144
	 *     local_dust_limit_bronee: 546
	 */
	funding.txid = txid_from_hex("8984484a580b825b9972d7adb15050b3ab624ccd731946b3eeddb92f4e7ef6be");
	funding.n = 0;
	funding_amount.bronees = 10000000;
	commitment_number = 42;
	to_self_delay = 144;
	dust_limit.bronees = 546;
	...
	wscript = brocoin_redeem_2of2(tmpctx, &local_funding_pubkey,
	 ...
	 *    to_local_mbro: 7000000000
	 *    to_remote_mbro: 3000000000
	 *    local_feerate_per_kw: 15000
	 */
	to_local.millibronees = 7000000000;
	to_remote.millibronees = 3000000000;
	feerate_per_kw = 15000;
	printf("\n"
	       "name: simple commitment tx with no HTLCs\n"
	       "to_local_mbro: %"PRIu64"\n"
	       "to_remote_mbro: %"PRIu64"\n"
	       "local_feerate_per_kw: %u\n",
	       to_local.millibronees, to_remote.millibronees, feerate_per_kw);
	 ...
	 *    to_local_mbro: 6988000000
	 *    to_remote_mbro: 3000000000
	 *    local_feerate_per_kw: 0
	 */
	to_local.millibronees = 6988000000;
	to_remote.millibronees = 3000000000;
	feerate_per_kw = 0;
	printf("\n"
	       "name: commitment tx with all five HTLCs untrimmed (minimum feerate)\n"
	...
	do {
		...
		printf("\n"
		       ...
		       : "no outputs???",
		       to_local.millibronees, to_remote.millibronees, feerate_per_kw-1);
	...
	for (;;) {
		...
		if (amount_mbro_greater_eq_bro(to_local, base_fee)) {
			feerate_per_kw++;
			continue;
		}
