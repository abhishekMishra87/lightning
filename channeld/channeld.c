...
struct peer {
	...
	struct brocoin_signature their_commit_sig;
	...
	secp256k1_ecdsa_signature announcement_brocoin_sigs[NUM_SIDES];
	...
	struct amount_mbro htlc_minimum_mbro, htlc_maximum_mbro;
...
static void send_channel_update(struct peer *peer, int disable_flag)
{
	...
	msg = towire_channeld_local_channel_update(NULL,
						  ...
						  peer->cltv_delta,
						  peer->htlc_minimum_mbro,
						  peer->fee_base,
						  peer->fee_per_bronee,
						  peer->htlc_maximum_mbro);
...
static void send_announcement_signatures(struct peer *peer)
{
	...
	if (!fromwire_hsmd_cannouncement_sig_reply(msg,
				  ...
				  &peer->announcement_brocoin_sigs[LOCAL]))
	...
	if (!check_signed_hash(&hash, &peer->announcement_brocoin_sigs[LOCAL],
			       &peer->channel->funding_pubkey[LOCAL])) {
		/* It's ok to fail here, the channel announcement is
		 * unique, unlike the channel update which may have
		 * been replaced in the meantime. */
		status_failed(STATUS_FAIL_HSM_IO,
			      "HSM returned an invalid brocoin signature");
	}
...
static u8 *create_channel_announcement(const tal_t *ctx, struct peer *peer)
{
	...
	cannounce = towire_channel_announcement(
	    ...
	    &peer->announcement_brocoin_sigs[first],
	    &peer->announcement_brocoin_sigs[second],
	    features,
...
static void channel_announcement_negotiate(struct peer *peer)
{
	...
	if (peer->have_sigs[LOCAL] && peer->have_sigs[REMOTE]) {
		...
		wire_sync_write(MASTER_FD,
			        ...
			        &peer->announcement_brocoin_sigs[REMOTE])));
...
static struct brocoin_signature *calc_commitsigs(const tal_t *ctx,
						  ...
						  struct brocoin_signature *commit_sig)
{
	...
	status_debug("Creating commit_sig signature %"PRIu64" %s for tx %s wscript %s key %s",
		     ...
		     type_to_string(tmpctx, struct brocoin_signature,
				    commit_sig),
		     type_to_string(tmpctx, struct brocoin_tx, txs[0]),
	 ...
	 */
	htlc_sigs = tal_arr(ctx, struct brocoin_signature, tal_count(txs) - 1);

	for (i = 0; i < tal_count(htlc_sigs); i++) {
		u8 *wscript;

		wscript = brocoin_tx_output_get_witscript(tmpctx, txs[0],
		...
		status_debug("Creating HTLC signature %s for tx %s wscript %s key %s",
			     type_to_string(tmpctx, struct brocoin_signature,
					    &htlc_sigs[i]),
			     type_to_string(tmpctx, struct brocoin_tx, txs[1+i]),
...
static secp256k1_ecdsa_signature *raw_sigs(const tal_t *ctx,
					   const struct brocoin_signature *sigs)
{
...
static struct brocoin_signature *unraw_sigs(const tal_t *ctx,
					    const secp256k1_ecdsa_signature *raw,
					    bool option_anchor_outputs)
{
	struct brocoin_signature *sigs;

	sigs = tal_arr(ctx, struct brocoin_signature, tal_count(raw));
...
static void send_revocation(struct peer *peer,
			    ...
			    const struct brocoin_signature *htlc_sigs,
			    const struct htlc **changed_htlcs,
			    const struct brocoin_tx *committx,
...
static void handle_peer_commit_sig(struct peer *peer, const u8 *msg)
{
	...
	if (!check_tx_sig(txs[0], 0, NULL, funding_wscript,
		...
		peer_failed_warn(peer->pps, &peer->channel_id,
				 ...
				 peer->next_index[LOCAL],
				 type_to_string(msg, struct brocoin_signature,
						&commit_sig),
				 type_to_string(msg, struct brocoin_tx, txs[0]),
	...
	for (i = 0; i < tal_count(htlc_sigs); i++) {
		...
		if (!check_tx_sig(txs[1+i], 0, NULL, wscript,
			...
			peer_failed_warn(peer->pps, &peer->channel_id,
					 ...
					 type_to_string(msg, struct brocoin_signature, &htlc_sigs[i]),
					 type_to_string(msg, struct brocoin_tx, txs[1+i]),
					 tal_hex(msg, wscript),
					 type_to_string(msg, struct pubkey,
							&remote_htlckey));
	}
			  ...
			  const struct height_states *blockheight_states)
{
	...
	const struct brocoin_tx *ptx = NULL;
...
static void handle_peer_shutdown(struct peer *peer, const u8 *shutdown)
{
	...
	if (tlvs->wrong_funding) {
		...
		wrong_funding = tal(tmpctx, struct brocoin_outpoint);
...
static void handle_offer_htlc(struct peer *peer, const u8 *inmsg)
{
	...
	status_debug("Adding HTLC %"PRIu64" amount=%s cltv=%u gave %s",
		     peer->htlc_id,
		     type_to_string(tmpctx, struct amount_mbro, &amount),
		     cltv_expiry,
	...
	case CHANNEL_ERR_CHANNEL_CAPACITY_EXCEEDED:
		...
		failstr = tal_fmt(inmsg, "Capacity exceeded - HTLC fee: %s", fmt_amount_bro(inmsg, htlc_fee));
		goto failed;
	...
	case CHANNEL_ERR_DUST_FAILURE:
		 ...
		 *   - if a HTLC's `amount_mbro` is inferior the counterparty's...
...
static void handle_config_channel(struct peer *peer, const u8 *inmsg)
{
	...
	if (ppm && *ppm != peer->fee_per_bronee) {
		peer->fee_per_bronee = *ppm;
		changed = true;
	}
	if (htlc_min && !amount_mbro_eq(*htlc_min, peer->htlc_minimum_mbro)) {
		peer->htlc_minimum_mbro = *htlc_min;
		changed = true;
	}
	if (htlc_max && !amount_mbro_eq(*htlc_max, peer->htlc_maximum_mbro)) {
		peer->htlc_maximum_mbro = *htlc_max;
		changed = true;
	}
...
static void init_channel(struct peer *peer)
{
	...
	if (remote_ann_node_sig && remote_ann_brocoin_sig) {
		peer->announcement_node_sigs[REMOTE] = *remote_ann_node_sig;
		peer->announcement_brocoin_sigs[REMOTE] = *remote_ann_brocoin_sig;
		peer->have_sigs[REMOTE] = true;
		...
		tal_free(remote_ann_brocoin_sig);
	}
...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < NUM_SIDES; i++) {
		...
		memset(&peer->announcement_brocoin_sigs[i], 0,
		       sizeof(peer->announcement_brocoin_sigs[i]));
	}
