...
struct msg_announcement_signatures {
	...
	secp256k1_ecdsa_signature announcement_brocoin_signature;
	struct short_channel_id short_channel_id;
};
...
struct msg_channel_announcement {
	...
	secp256k1_ecdsa_signature brocoin_signature_1;
	secp256k1_ecdsa_signature brocoin_signature_2;
	u8 *features;
	struct brocoin_blkid chain_hash;
	struct short_channel_id short_channel_id;
...
struct msg_update_add_htlc {
	struct channel_id channel_id;
	u64 id;
	struct amount_mbro amount_mbro;
	u32 expiry;
						...
						const struct msg_channel_announcement *s)
{
	return towire_channel_announcement(ctx,
					   &s->node_signature_1,
					   &s->node_signature_2,
					   &s->brocoin_signature_1,
					   &s->brocoin_signature_2,
					   s->features,
					   &s->chain_hash,
					   &s->short_channel_id,
					   &s->node_id_1,
						...
						const struct msg_open_channel *s)
{
	return towire_open_channel(ctx,
				   ...
				   s->funding_bronees,
				   s->push_mbro,
				   s->dust_limit_bronees,
				   s->max_htlc_value_in_flight_mbro,
				   s->channel_reserve_bronees,
				   s->htlc_minimum_mbro,
				   s->feerate_per_kw,
				   s->to_self_delay,
						...
						const struct msg_announcement_signatures *s)
{
	return towire_announcement_signatures(ctx,
				     ...
				     &s->announcement_brocoin_signature);
}
				    ...
				    const struct msg_channel_announcement *b)
{
	return eq_upto(a, b, features)
		...
		&& eq_between(a, b, brocoin_key_1, brocoin_key_2);
}
		    ...
		    const struct msg_init *b)
{
	...
	for (size_t i = 0; i < tal_count(a->tlvs->networks); i++)
		if (!brocoin_blkid_eq(&a->tlvs->networks[i],
				      &b->tlvs->networks[i]))
			return false;
	return true;
}
...
int main(int argc, char *argv[])
{
	...
	set_pubkey(&ca.brocoin_key_1);
	set_pubkey(&ca.brocoin_key_2);
	...
	for (i = 0; i < ARRAY_SIZE(networks); i++) {
		...
		init.tlvs->networks = tal_arr(init.tlvs, struct brocoin_blkid, 1);
