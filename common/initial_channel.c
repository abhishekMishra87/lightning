...
struct channel *new_initial_channel(const tal_t *ctx,
				    ...
				    struct amount_mbro local_mbronee,
				    ...
				    enum side opener)
{
	...
	struct amount_mbro remote_mbronee;

	channel->cid = *cid;
	channel->funding = *funding;
	channel->funding_bros = funding_bros;
	channel->minimum_depth = minimum_depth;
	channel->lease_expiry = lease_expiry;
	if (!amount_bro_sub_mbro(&remote_mbronee,
				 channel->funding_bros, local_mbronee))
...
struct brocoin_tx *initial_channel_tx(const tal_t *ctx,
				      const u8 **wscript,
				      ...
				      char** err_reason)
{
	...
	*wscript = brocoin_redeem_2of2(ctx,
