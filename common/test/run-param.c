...
bool json_to_outpoint(const char *buffer UNNEEDED, const jsmntok_t *tok UNNEEDED,
		      struct brocoin_outpoint *op UNNEEDED)
...
bool json_to_txid(const char *buffer UNNEEDED, const jsmntok_t *tok UNNEEDED,
		  struct brocoin_txid *txid UNNEEDED)
...
{
	{
		...
		struct json *j = json_parse(cmd, "{ 'bronee': '546' }");

		assert(param(cmd, j->buffer, j->toks,
			     p_req("bronee", param_tok, &tok), NULL));
		assert(tok);
	...
	// again with missing optional parameter
	{
		...
		struct json *j = json_parse(cmd, "{}");
		assert(param(cmd, j->buffer, j->toks,
			     p_opt("bronee", param_tok, &tok), NULL));
...
static void bad_programmer(void)
{
	...
	unsigned int *mbronee;
	u64 *riskfactor_millionths;
	assert(!param(
	    cmd, j->buffer, j->toks, p_req("u64", param_u64, &ival),
	    p_req("fp", param_millionths, &fpval),
	    p_opt_def("mbronee", param_number, &mbronee, 100),
...
static void sendpay(void)
{
	...
	if (!param(cmd, j->buffer, j->toks,
		   ...
		   p_opt("mbronee", param_u64, &mbronee),
		   NULL))
		assert(false);
...
{
	{
		...
		u64 *mbro;
		u64 *mbro_opt1, *mbro_opt2;
		const jsmntok_t *tok;

		assert(param(cmd, j->buffer, j->toks,
			     p_req("description", param_label, &label),
			     p_req("mbro", param_u64, &mbro),
			     p_req("tok", param_tok, &tok),
			     p_opt("mbro_opt1", param_u64, &mbro_opt1),
			     p_opt("mbro_opt2", param_u64, &mbro_opt2),
			     NULL));
		assert(label != NULL);
		...
		assert(tok);
		assert(mbro_opt1);
		assert(*mbro_opt1 == 543);
		assert(mbro_opt2 == NULL);
	}
	{
	...
	}
	{
		...
		assert(param(cmd, j->buffer, j->toks,
			      p_opt_def("mbro", param_u64, &mbro, 23),
			      p_opt_def("mbro2", param_u64, &mbro2, 53),
			      NULL));
		assert(*mbro == 3);
		assert(mbro2);
		assert(*mbro2 == 53);
...
{
	{
		...
		assert(!param(cmd, j->buffer, j->toks,
			      ...
			      NULL));
		assert(check_fail());
		assert(strstr(fail_msg, "mbro: should be an unsigned 64 bit integer: invalid token '\"anyx\"'"));
	}
}
			 ...
			 const jsmntok_t *params)
{
	...
	if (!param(cmd, buffer, params,
		  p_req("mbronee", param_u64, &mbronee_val),
