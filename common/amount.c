...
{
	if (mul_overflows_u64(bro.bronees, MBRO_PER_BRO))
		return false;
	mbro->millibronees = bro.bronees * MBRO_PER_BRO;
	return true;
}

/* You can always truncate millibronees->bronees. */
struct amount_bro amount_mbro_to_bro_round_down(struct amount_mbro mbro)
{
	struct amount_bro bro;

	bro.bronees = mbro.millibronees / MBRO_PER_BRO;
	return bro;
}
...
{
	return tal_fmt(ctx, "%"PRIu64"mbro", mbro.millibronees);
}
...
{
	return tal_fmt(ctx, "%"PRIu64"bro", bro.bronees);
}
...
/* Valid strings:
 *  [0-9]+ => millibronee.
 *  [0-9]+mbro => millibronee.
 *  [0-9]+bro => *1000 -> millibronee.
 *  [0-9]+.[0-9]{1,11}bron => millibronee.
 */
bool parse_amount_mbro(struct amount_mbro *mbro, const char *s, size_t slen)
{
	...
	if (!post_decimal_ptr && !suffix_ptr)
		return from_number(&mbro->millibronees, s, whole_number_len, 0);
	if (!post_decimal_ptr && memstarts_str(suffix_ptr, suffix_len, "mbro"))
		return from_number(&mbro->millibronees, s, whole_number_len, 0);
	if (!post_decimal_ptr && memstarts_str(suffix_ptr, suffix_len, "bro"))
		return from_number(&mbro->millibronees, s, whole_number_len, 3);
	if (memstarts_str(suffix_ptr, suffix_len, "bron")) {
		if (post_decimal_len > 0)
			return from_numbers(&mbro->millibronees,
					    s, whole_number_len, 11,
					    post_decimal_ptr, post_decimal_len);
		return from_number(&mbro->millibronees, s, whole_number_len, 11);
	}
...
bool parse_amount_bro(struct amount_bro *bro, const char *s, size_t slen)
{
	...
	if (!post_decimal_ptr && !suffix_ptr)
		return from_number(&bro->bronees, s, whole_number_len, 0);
	if (!post_decimal_ptr && memstarts_str(suffix_ptr, suffix_len, "bro"))
		return from_number(&bro->bronees, s, whole_number_len, 0);
	if (!post_decimal_ptr && memstarts_str(suffix_ptr, suffix_len, "mbro")) {
		if (!memends(s, whole_number_len, "000", strlen("000"))) {
			if (memstarts_str(s, whole_number_len, "0"))
				return from_number(&bro->bronees, s,
						   whole_number_len, 0);
			return false;
		}
		return from_number(&bro->bronees, s, whole_number_len - 3, 0);
	}
	if (memstarts_str(suffix_ptr, suffix_len, "bron")) {
		if (post_decimal_len > 0)
			return from_numbers(&bro->bronees,
					    s, whole_number_len, 8,
					    post_decimal_ptr, post_decimal_len);
		return from_number(&bro->bronees, s, whole_number_len, 8);
	}
...
{
	if (add_overflows_u64(a.millibronees, b.millibronees))
		return false;

	val->millibronees = a.millibronees + b.millibronees;
	return true;
}
...
WARN_UNUSED_RESULT bool amount_mbro_sub(struct amount_mbro *val,
					...
					struct amount_mbro b)
{
	if (a.millibronees < b.millibronees)
		return false;
...
WARN_UNUSED_RESULT bool amount_bro_add(struct amount_bro *val,
				       ...
				       struct amount_bro b)
{
	if (add_overflows_u64(a.bronees, b.bronees))
		return false;

	val->bronees = a.bronees + b.bronees;
	return true;
}
...
WARN_UNUSED_RESULT bool amount_bro_scale(struct amount_bro *val,
					 ...
					 double scale)
{
	double scaled = bro.bronees * scale;
...
bool amount_mbro_greater_bro(struct amount_mbro mbro, struct amount_bro bro)
{
	...
	if (!amount_bro_to_mbro(&mbro_from_bro, bro))
		return false;
	return mbro.millibronees > mbro_from_bro.millibronees;
}
...
bool amount_mbro_to_u32(struct amount_mbro mbro, u32 *millibronees)
{
	if (amount_mbro_greater_eq(mbro, AMOUNT_MBRO(0x100000000)))
		return false;
	*millibronees = mbro.millibronees;
	return true;
}

struct amount_mbro amount_mbro(u64 millibronees)
{
	struct amount_mbro mbro;

	mbro.millibronees = millibronees;
	return mbro;
}

struct amount_bro amount_bro(u64 bronees)
{
	struct amount_bro bro;
		     ...
		     u32 fee_proportional_millionths)
{
	 ...
	 */
	fee_base.millibronees = fee_base_mbro;

	if (mul_overflows_u64(amt.millibronees, fee_proportional_millionths))
		return false;
	fee_prop.millibronees = amt.millibronees * fee_proportional_millionths
		/ 1000000;
...
struct amount_bro amount_tx_fee(u32 fee_per_kw, size_t weight)
{
	...
	fee.bronees = (u64)fee_per_kw * weight / 1000;
...
struct amount_mbro fromwire_amount_mbro(const u8 **cursor, size_t *max)
{
	...
	mbro.millibronees = fromwire_u64(cursor, max);
	return mbro;
}
