		       ...
		       const u8 *enctlv)
{
	 ...
	 */
	tlv->amt_to_forward = &forward.millibronees; /* Raw: TLV convert */
		    ...
		    const u8 *payment_metadata)
{
	...
	/* These go together! */
	if (!payment_secret)
		assert(amount_mbro_eq(total_mbro, forward));
	...
	if (payment_secret) {
		...
		tlv_pdata.total_mbro = total_mbro.millibronees; /* Raw: TLV convert */
				   ...
				   size_t *failtlvpos)
{
	...
	if (p->type == ONION_V0_PAYLOAD && rs->nextcase == ONION_FORWARD) {
		...
		p->total_mbro = NULL;
		p->amt_to_forward = fromwire_amount_mbro(&cursor, &max);
	...
	p->amt_to_forward = amount_mbro(*tlv->amt_to_forward);
	...
	if (rs->nextcase == ONION_FORWARD) {
		 ...
		 *   - MUST treat `total_mbro` as if it were equal to
		 *     `amt_to_forward` if it is not present. */
		p->total_mbro = tal_dup(p, struct amount_mbro,
					&p->amt_to_forward);
	}
	...
	if (tlv->payment_data) {
		...
		tal_free(p->total_mbro);
		p->total_mbro = tal(p, struct amount_mbro);
		*p->total_mbro
			= amount_mbro(tlv->payment_data->total_mbro);
	}
