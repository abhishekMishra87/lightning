...
struct command_result *param_mbro(struct command *cmd, const char *name,
				  const char *buffer, const jsmntok_t *tok,
				  struct amount_mbro **mbro)
{
	*mbro = tal(cmd, struct amount_mbro);
	...
	return command_fail_badparam(cmd, name, buffer, tok,
				     "should be a millibronee amount");
}

struct command_result *param_bro(struct command *cmd, const char *name,
				 const char *buffer, const jsmntok_t *tok,
				 struct amount_bro **bro)
{
	*bro = tal(cmd, struct amount_bro);
	if (parse_amount_bro(*bro, buffer + tok->start, tok->end - tok->start))
		return NULL;

	return command_fail_badparam(cmd, name, buffer, tok,
				     "should be a bronee amount");
}

struct command_result *param_bro_or_all(struct command *cmd, const char *name,
					const char *buffer, const jsmntok_t *tok,
					struct amount_bro **bro)
{
	...
	return param_bro(cmd, name, buffer, tok, bro);
}
...
{
	struct brocoin_address destination;
	int witness_version;
...
{
	*txid = tal(cmd, struct brocoin_txid);
...
struct command_result *param_brocoin_address(struct command *cmd,
					     const char *name,
					     const char *buffer,
					     const jsmntok_t *tok,
...
struct command_result *param_outpoint_arr(struct command *cmd,
					  ...
					  const char *buffer,
					  const jsmntok_t *tok,
					  struct brocoin_outpoint **outpoints)
{
	size_t i;
	...
	*outpoints = tal_arr(cmd, struct brocoin_outpoint, tok->size);
