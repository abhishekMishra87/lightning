...
{
	char *end;
	unsigned long bron, bro;

	bron = strtoul(buffer + tok->start, &end, 10);
	if (bron == ULONG_MAX && errno == ERANGE)
		return false;
	if (end != buffer + tok->end) {
		/* Expect always 8 decimal places. */
		if (*end != '.' || buffer + tok->end - end != 9)
			return false;
		bro = strtoul(end+1, &end, 10);
		if (bro == ULONG_MAX && errno == ERANGE)
			return false;
		if (end != buffer + tok->end)
			return false;
	} else
	...
	*bronee = bron * (uint64_t)100000000 + bro;
	if (*bronee != bron * (uint64_t)100000000 + bro)
		return false;
...
{
	return parse_amount_bro(bro, buffer + tok->start, tok->end - tok->start);
}
			...
			struct amount_bro *bro)
{
	...
	return json_to_bro(buffer, tok, bro);
}
...
{
	return brocoin_txid_from_hex(buffer + tok->start,
		   ...
		   const struct brocoin_txid *txid)
{
	...
	brocoin_txid_to_hex(txid, hex, sizeof(hex));
		       ...
		       const struct brocoin_outpoint *out)
{
	...
	brocoin_txid_to_hex(&out->txid, hex, sizeof(hex));
...
{
	json_add_u64(result, rawfieldname, mbro.millibronees); /* Raw: low-level helper */
	json_add_amount_mbro_only(result, mbrofieldname, mbro);
}
...
void json_add_amount_mbro_only(struct json_stream *result,
			  ...
			  struct amount_mbro mbro)
{
	json_add_string(result, mbrofieldname,
...
void json_add_amount_bro_compat(struct json_stream *result,
				...
				const char *mbrofieldname)
{
	json_add_u64(result, rawfieldname, bro.bronees); /* Raw: low-level helper */
	json_add_amount_bro_only(result, mbrofieldname, bro);
}
...
{
	json_add_amount_bro_only(result, "lease_fee_base_mbro",
	...
	json_add_amount_mbro_only(result,
