...
static bool update_channel(struct gossmap *map, size_t cupdate_off)
{
	 ...
	 * htlc_maximum_mbro, so we don't bother setting the rest to the
			      ...
			      int dir)
{
	...
	mod->hc[dir].htlc_min
		= u64_to_fp16(htlc_min.millibronees, /* Raw: to fp16 */
			      false);
	mod->hc[dir].htlc_max
		= u64_to_fp16(htlc_max.millibronees, /* Raw: to fp16 */
			      true);
			       ...
			       struct amount_bro *amount)
{
	 ...
	 * which is 8 (bronees) + 2 (len) */
	if (c->private) {
		*amount = amount_bro(map_be64(map, c->cann_off - 8 - 2));
		return true;
	}
	...
	*amount = amount_bro(map_be64(map, off + sizeof(ghdr) + sizeof(be16)));
	return true;
}
...
{
	if (amount_mbro_less_fp16(amount, chan->half[direction].htlc_min))
		return false;

	if (amount_mbro_greater_fp16(amount, chan->half[direction].htlc_max))
		return false;
				     ...
				     struct amount_mbro *htlc_maximum_mbro)
{
	...
	if (fee_base_mbro)
		*fee_base_mbro = map_be32(map, fee_base_off);
	if (fee_proportional_millionths)
		*fee_proportional_millionths = map_be32(map, fee_prop_off);
	if (htlc_minimum_mbro)
		*htlc_minimum_mbro
			= amount_mbro(map_be64(map, htlc_minimum_off));
	if (htlc_maximum_mbro && (mflags & 1))
		*htlc_maximum_mbro
			= amount_mbro(map_be64(map, htlc_maximum_off));
}
