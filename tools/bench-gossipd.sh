#! /bin/sh
# Needs brocoind -regtest running.
...
wait_for_start()
{
    ...
    # Wait for it to catch up with brocoind.
    while [ "$($LCLI1 -H getinfo | grep '^blockheight=' | cut -d= -f2)" != "$(brocoin-cli -regtest getblockcount)" ]; do sleep 1; done
    echo "$ID"
}
...
if ! brocoin-cli -regtest ping >/dev/null 2>&1; then
    brocoind -regtest > "$DIR"/brocoind.log &

    while ! brocoin-cli -regtest ping >/dev/null 2>&1; do sleep 1; done
fi
...
if [ -z "$DIR" ]; then
    ...
    DIR="$(mktemp -d)"
    ./devtools/create-gossipstore --csv "$MCP_DIR"/scidBronees.csv -i "$MCP_DIR"/1M.gossip -o "$DIR"/gossip_store
fi
...
if [ -z "${TARGETS##* routing_sec *}" ]; then
	...
	# Channels have htlc_min of 10000 mbro.
	# shellcheck disable=SC2086
