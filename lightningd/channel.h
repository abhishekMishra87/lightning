...
struct funding_info {
	...
	u32 feerate;
	struct amount_bro total_funds;

	/* Our original funds, in funding amount */
	struct amount_bro our_funds;
};
...
struct channel {
	...
	/* Statistics for min and max our_mbronee. */
	struct amount_mbro mbro_to_us_min;
	struct amount_mbro mbro_to_us_max;

	/* Last tx they gave us. */
	struct brocoin_tx *last_tx;
	enum wallet_tx_type last_tx_type;
	struct brocoin_signature last_sig;
	const struct brocoin_signature *last_htlc_sigs;
	...
	/* Whether closing_fee_negotiation_step is in bronee or %. */
	u8 closing_fee_negotiation_step_unit;

	/* optional wrong_funding for mutual close */
	const struct brocoin_outpoint *shutdown_wrong_funding;
	...
	struct amount_mbro old_htlc_minimum_mbro, old_htlc_maximum_mbro;
	...
	/* Lease commited maximum channel fee base mbro */
	u32 lease_chan_max_mbro;
...
struct channel *new_channel(struct peer *peer, u64 dbid,
			    ...
			    struct channel_id *cid,
			    struct amount_mbro our_mbronee,
			    struct amount_mbro mbronee_to_us_min,
			    struct amount_mbro mbronee_to_us_max,
			    struct brocoin_tx *last_tx STEALS,
			    const struct brocoin_signature *last_sig,
			    /* NULL or stolen */
			    const struct brocoin_signature *last_htlc_sigs STEALS,
			    ...
			    /* NULL or stolen */
			    const struct brocoin_outpoint *shutdown_wrong_funding STEALS,
...
new_inflight(struct channel *channel,
	     const struct brocoin_outpoint *funding_outpoint,
	     u32 funding_feerate,
	     ...
	     const struct amount_mbro lease_fee);
...
struct amount_mbro htlc_max_possible_send(const struct channel *channel);
