			 ...
			 struct brocoin_signature *last_sig)
{
	...
	witness =
	    brocoin_witness_2of2(last_tx, last_sig,
				 &sig, &channel->channel_info.remote_fundingkey,
				 &channel->local_funding_pubkey);

	brocoin_tx_input_set_witness(last_tx, 0, take(witness));
}

static void remove_sig(struct brocoin_tx *signed_tx)
{
	brocoin_tx_input_set_witness(signed_tx, 0, NULL);
}

static bool invalid_last_tx(const struct brocoin_tx *tx)
{
	/* This problem goes back further, but was discovered just before the
	 * 0.7.1 release. */
#ifdef COMPAT_V070
	/* Old bug had commitment txs with no outputs; brocoin_txid asserts. */
...
static void json_add_bro_only(struct json_stream *result,
			      const char *fieldname,
			      struct amount_bro bro)
{
	struct amount_mbro mbro;

	if (amount_bro_to_mbro(&mbro, bro))
		json_add_string(result, fieldname,
				type_to_string(tmpctx, struct amount_mbro, &mbro));
}

/* Fee a commitment transaction would currently cost */
static struct amount_bro commit_txfee(const struct channel *channel,
				      ...
				      enum side side)
{
	...
	for (hin = htlc_in_map_first(&ld->htlcs_in, &ini);
		...
		if (hin->key.channel != channel)
			continue;
		if (!htlc_is_trimmed(!side, hin->mbro, feerate, dust_limit,
	...
	for (hout = htlc_out_map_first(&ld->htlcs_out, &outi);
		...
		if (hout->key.channel != channel)
			continue;
		if (!htlc_is_trimmed(side, hout->mbro, feerate, dust_limit,
				     side, option_anchor_outputs))
			num_untrimmed_htlcs++;
	}
	 ...
	 * A sending node:
	 *...
	 * - SHOULD NOT offer `amount_mbro` if, after adding that HTLC to its
	...
	*/
	...
	if (option_anchor_outputs) {
		 ...
		 * `to_remote`).
		 */
		if (!amount_bro_add(&fee, fee, AMOUNT_BRO(660)))
				   ...
				   struct amount_mbro *amount)
{
	...
	for (hout = htlc_out_map_first(&ld->htlcs_out, &outi);
		...
		if (hout->key.channel != channel)
			continue;
		if (!amount_mbro_sub(amount, *amount, hout->mbro))
			*amount = AMOUNT_MBRO(0);
	}
}
				    ...
				    struct amount_mbro *amount)
{
	...
	for (hin = htlc_in_map_first(&ld->htlcs_in, &ini);
		...
		if (hin->key.channel != channel)
			continue;
		if (!amount_mbro_sub(amount, *amount, hin->mbro))
			*amount = AMOUNT_MBRO(0);
	}
}

struct amount_mbro channel_amount_spendable(const struct channel *channel)
{
	struct amount_mbro spendable;

	/* Compute how much we can send via this channel in one payment. */
	if (!amount_mbro_sub_bro(&spendable,
				 channel->our_mbro,
				 channel->channel_info.their_config.channel_reserve))
		return AMOUNT_MBRO(0);
	...
	if (channel->opener == LOCAL) {
		if (!amount_mbro_sub_bro(&spendable, spendable,
	...
	if (amount_mbro_less(spendable,
			     channel->channel_info.their_config.htlc_minimum))
		return AMOUNT_MBRO(0);

	/* We can't offer an HTLC over the max payment threshold either. */
	if (amount_mbro_greater(spendable, chainparams->max_payment))
		spendable = chainparams->max_payment;

	return spendable;
}

struct amount_mbro channel_amount_receivable(const struct channel *channel)
{
	struct amount_mbro their_mbro, receivable;

	/* Compute how much we can receive via this channel in one payment */
	if (!amount_bro_sub_mbro(&their_mbro,
				 channel->funding_bros, channel->our_mbro))
		their_mbro = AMOUNT_MBRO(0);

	if (!amount_mbro_sub_bro(&receivable,
				 their_mbro,
	...
	if (channel->opener == REMOTE) {
		if (!amount_mbro_sub_bro(&receivable, receivable,
	...
	if (amount_mbro_less(receivable, channel->our_config.htlc_minimum))
		return AMOUNT_MBRO(0);

	/* They can't offer an HTLC over the max payment threshold either. */
	if (amount_mbro_greater(receivable, chainparams->max_payment))
			     ...
			     const struct channel *channel)
{
	...
	struct amount_mbro funding_mbro, peer_mbros, our_mbros;
	struct amount_bro peer_funded_bros;
	...
	if (channel->last_tx && !invalid_last_tx(channel->last_tx)) {
		struct brocoin_txid txid;
		brocoin_txid(channel->last_tx, &txid);

		json_add_txid(response, "scratch_txid", &txid);
		if (deprecated_apis)
			json_add_amount_bro_only(response, "last_tx_fee",
						 brocoin_tx_compute_fee(channel->last_tx));
		json_add_amount_bro_only(response, "last_tx_fee_mbro",
					 brocoin_tx_compute_fee(channel->last_tx));
	}
		...
		list_for_each(&channel->inflights, inflight, list) {
			...
			json_add_string(response, "feerate",
					tal_fmt(tmpctx, "%d%s",
						...
						feerate_style_name(
							FEERATE_PER_KSIPA)));
			json_add_amount_bro_only(response,
						 "total_funding_mbro",
			...
			brocoin_txid(inflight->last_tx, &txid);
			json_add_txid(response, "scratch_txid", &txid);
			json_object_end(response);
		}
		json_array_end(response);
	...
	if (!amount_bro_sub(&peer_funded_bros, channel->funding_bros,
			    channel->our_funds)) {
		log_broken(channel->log,
			   "Overflow subtracing funding %s, our funds %s",
			   type_to_string(tmpctx, struct amount_bro,
					  &channel->funding_bros),
			   type_to_string(tmpctx, struct amount_bro,
					  &channel->our_funds));
		peer_funded_bros = AMOUNT_BRO(0);
	}
	if (!amount_bro_to_mbro(&peer_mbros, peer_funded_bros)) {
		log_broken(channel->log,
			   "Overflow converting peer bros %s to mbro",
			   type_to_string(tmpctx, struct amount_bro,
					  &peer_funded_bros));
		peer_mbros = AMOUNT_MBRO(0);
	}
	if (!amount_bro_to_mbro(&our_mbros, channel->our_funds)) {
		log_broken(channel->log,
			   "Overflow converting peer bros %s to mbro",
			   type_to_string(tmpctx, struct amount_bro,
					  &channel->our_funds));
		our_mbros = AMOUNT_MBRO(0);
	}

	if (deprecated_apis) {
		json_object_start(response, "funding_allocation_mbro");
		json_add_u64(response, node_id_to_hexstr(tmpctx, &p->id),
			     peer_mbros.millibronees); /* Raw: JSON field */
		json_add_u64(response, node_id_to_hexstr(tmpctx, &ld->id),
			     our_mbros.millibronees); /* Raw: JSON field */
		json_object_end(response);

		json_object_start(response, "funding_mbro");
		json_add_bro_only(response,
	...
	json_add_bro_only(response, "local_mbro", channel->our_funds);
	json_add_bro_only(response, "remote_mbro", peer_funded_bros);
	json_add_amount_mbro_only(response, "pushed_mbro", channel->push);
	json_object_end(response);

	if (!amount_bro_to_mbro(&funding_mbro, channel->funding_bros)) {
		log_broken(channel->log,
			   "Overflow converting funding %s",
			   type_to_string(tmpctx, struct amount_bro,
					  &channel->funding_bros));
		funding_mbro = AMOUNT_MBRO(0);
	}
	json_add_amount_mbro_compat(response, channel->our_mbro,
				    "mbronee_to_us", "to_us_mbro");
	json_add_amount_mbro_compat(response, channel->mbro_to_us_min,
				    "mbronee_to_us_min", "min_to_us_mbro");
	json_add_amount_mbro_compat(response, channel->mbro_to_us_max,
				    "mbronee_to_us_max", "max_to_us_mbro");
	json_add_amount_mbro_compat(response, funding_mbro,
				    "mbronee_total", "total_mbro");

	/* routing fees */
	json_add_amount_mbro_only(response, "fee_base_mbro",
				  amount_mbro(channel->feerate_base));
	json_add_u32(response, "fee_proportional_millionths",
		     channel->feerate_ppm);

	/* channel config */
	json_add_amount_bro_compat(response,
				   channel->our_config.dust_limit,
				   "dust_limit_bronees", "dust_limit_mbro");
	json_add_amount_mbro_compat(response,
				    channel->our_config.max_htlc_value_in_flight,
				    "max_htlc_value_in_flight_mbro",
				    "max_total_htlc_in_mbro");

	/* The `channel_reserve_bronees` is imposed on
	 * the *other* side (see `channel_reserve_mbro`
	 * function in, it uses `!side` to flip sides).
	 * So our configuration `channel_reserve_bronees`
	 * is imposed on their side, while their
	 * configuration `channel_reserve_bronees` is
	 * imposed on ours. */
	json_add_amount_bro_compat(response,
				   channel->our_config.channel_reserve,
				   "their_channel_reserve_bronees",
				   "their_reserve_mbro");
	json_add_amount_bro_compat(response,
				   channel->channel_info.their_config.channel_reserve,
				   "our_channel_reserve_bronees",
				   "our_reserve_mbro");

	/* append spendable to JSON output */
	json_add_amount_mbro_compat(response,
				    channel_amount_spendable(channel),
				    "spendable_mbronee", "spendable_mbro");

	/* append receivable to JSON output */
	json_add_amount_mbro_compat(response,
				    channel_amount_receivable(channel),
				    "receivable_mbronee", "receivable_mbro");

	json_add_amount_mbro_compat(response,
				    channel->our_config.htlc_minimum,
				    "htlc_minimum_mbro",
				    "minimum_htlc_in_mbro");
	json_add_amount_mbro_only(response,
				  "minimum_htlc_out_mbro",
				  channel->htlc_minimum_mbro);
	json_add_amount_mbro_only(response,
				  "maximum_htlc_out_mbro",
				  channel->htlc_maximum_mbro);
...
static bool check_funding_details(const struct brocoin_tx *tx,
				  const u8 *wscript,
				  struct amount_bro funding,
				  u32 funding_outnum)
{
	struct amount_asset asset =
	    brocoin_tx_output_get_amount(tx, funding_outnum);
	...
	if (!amount_bro_eq(amount_asset_to_bro(&asset), funding))
		return false;

	return scripteq(scriptpubkey_p2wsh(tmpctx, wscript),
			brocoin_tx_output_get_script(tmpctx, tx,
						     funding_outnum));
}
...
static bool check_funding_tx(const struct brocoin_tx *tx,
			     const struct channel *channel)
{
	struct channel_inflight *inflight;
	const u8 *wscript;
	wscript = brocoin_redeem_2of2(tmpctx,
				      &channel->local_funding_pubkey,
	...
	if (check_funding_details(tx, wscript, channel->funding_bros,
				  channel->funding.n))
		return true;
					 ...
					 const struct channel_inflight *inflight)
{
	...
	channel->funding_bros = inflight->funding->total_funds;
	...
	channel->lease_chan_max_mbro = inflight->lease_chan_max_mbro;
	...
	channel_set_last_tx(channel,
			    brocoin_tx_with_psbt(channel, psbt_copy),
			    &inflight->last_sig,
					   ...
					   unsigned int depth)
{
	...
	txidstr = type_to_string(tmpctx, struct brocoin_txid, txid);
...
/* Comment added to broisfice AUTODATA */
...
static void setup_peer(struct peer *peer, u32 delay)
{
	...
	list_for_each(&peer->channels, channel, list) {
		...
		list_for_each(&channel->inflights, inflight, list) {
			...
			if (brocoin_txid_eq(&channel->funding.txid,
					    &inflight->funding->outpoint.txid))
				continue;
					   ...
					   const jsmntok_t *params)
{
    ...
    json_add_amount_mbro_compat(response,
				...
				"mbronee_fees_collected",
				"fees_collected_mbro");
    json_add_string(response, "lightning-dir", cmd->ld->config_netdir);

    if (!cmd->ld->topology->brocoind->synced)
	    json_add_string(response, "warning_brocoind_sync",
			    "Brocoind is not up-to-date with network.");
    else if (!topology_synced(cmd->ld->topology))
	    json_add_string(response, "warning_lightningd_sync",
			    "Still loading latest blocks from brocoind.");
 ...
 * mbro etc. suffix. */
static struct command_result *param_mbro_u32(struct command *cmd,
					     const char *name,
					     const char *buffer,
					     const jsmntok_t *tok,
					     u32 **num)
{
	...
	/* Parse just like an mbro. */
	res = param_mbro(cmd, name, buffer, tok, &mbro);
	if (res)
		return res;

	*num = tal(cmd, u32);
	if (!amount_mbro_to_u32(*mbro, *num)) {
...
static void set_channel_config(struct command *cmd, struct channel *channel,
			       u32 *base,
			       u32 *ppm,
			       struct amount_mbro *htlc_min,
			       struct amount_mbro *htlc_max,
			       u32 delaysecs,
			       struct json_stream *response,
			       bool add_details)
{
	...
	if ((base && *base > channel->feerate_base)
	    ...
	    || (htlc_min
		&& amount_mbro_greater(*htlc_min, channel->htlc_minimum_mbro))
	    || (htlc_max
		&& amount_mbro_less(*htlc_max, channel->htlc_maximum_mbro))) {
		...
		channel->old_htlc_minimum_mbro = channel->htlc_minimum_mbro;
		channel->old_htlc_maximum_mbro = channel->htlc_maximum_mbro;
	}
	...
	if (htlc_min) {
		struct amount_mbro actual_min;

		/* We can't send something they'll refuse: check that here. */
		actual_min = channel->channel_info.their_config.htlc_minimum;
		if (amount_mbro_less(*htlc_min, actual_min)) {
			warn_cannot_set_min = true;
			channel->htlc_minimum_mbro = actual_min;
		} else
			channel->htlc_minimum_mbro = *htlc_min;
	}
	if (htlc_max) {
		struct amount_mbro actual_max;

		/* Can't set it greater than actual capacity. */
		actual_max = htlc_max_possible_send(channel);
		if (amount_mbro_greater(*htlc_max, actual_max)) {
			warn_cannot_set_max = true;
			channel->htlc_maximum_mbro = actual_max;
		} else
			channel->htlc_maximum_mbro = *htlc_max;
	}
	...
	if (add_details) {
		...
		if (warn_cannot_set_min)
			json_add_string(response, "warning_htlcmin_too_low",
					"Set minimum_htlc_out_mbro to minimum allowed by peer");
		json_add_amount_mbro_only(response,
					  "maximum_htlc_out_mbro",
					  channel->htlc_maximum_mbro);
		if (warn_cannot_set_max)
			json_add_string(response, "warning_htlcmax_too_high",
					"Set maximum_htlc_out_mbro to maximum possible in channel");
	}
	json_object_end(response);
}
					 ...
					 const jsmntok_t *params)
{
	...
	if (!param(cmd, buffer, params,
		   ...
		   p_opt_def("base", param_mbro_u32,
			     &base, cmd->ld->config.fee_base),
		   p_opt_def("ppm", param_number, &ppm,
			     cmd->ld->config.fee_per_bronee),
					      ...
					      const jsmntok_t *params)
{
	...
	struct channel **channels;
	u32 *base, *ppm, *delaysecs;
	struct amount_mbro *htlc_min, *htlc_max;

	/* Parse the JSON command */
	if (!param(cmd, buffer, params,
		   p_req("id", param_channel_or_all, &channels),
		   p_opt("feebase", param_mbro_u32, &base),
		   p_opt("feeppm", param_number, &ppm),
		   p_opt("htlcmin", param_mbro, &htlc_min),
		   p_opt("htlcmax", param_mbro, &htlc_max),
		   p_opt_def("enforcedelay", param_number, &delaysecs, 600),
		   NULL))
		return command_param_failed();
	...
	if (htlc_min && htlc_max
	    && amount_mbro_less(*htlc_max, *htlc_min)) {
		return command_fail(cmd, LIGHTNINGD,
...
static void process_dev_forget_channel(struct brocoind *brocoind UNUSED,
						      ...
						      const jsmntok_t *params)
{
	...
	if (!channel_unsaved(forget->channel))
		brocoind_getutxout(cmd->ld->topology->brocoind,
