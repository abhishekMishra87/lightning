...
{
	if (amount_mbro_eq(hin->mbro, AMOUNT_MBRO(0)))
		return corrupt(abortstr, "zero mbronee");
...
struct htlc_in *new_htlc_in(const tal_t *ctx,
			    ...
			    struct amount_mbro mbro, u32 cltv_expiry,
				...
				const char *abortstr)
{
	...
	if (hout->in) {
		if (amount_mbro_less(hout->in->mbro, hout->mbro))
			      ...
			      struct htlc_in *in)
{
	...
	if (am_origin) {
		...
		if (!amount_mbro_sub(&hout->fees, mbro, final_mbro))
	...
	if (in) {
		...
		/* Stash the fees (for accounting) */
		if (!amount_mbro_sub(&hout->fees, in->mbro, mbro))
			return corrupt("new_htlc_out",
				       "overflow subtract %s-%s",
				       type_to_string(tmpctx,
						      struct amount_mbro,
