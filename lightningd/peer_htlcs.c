...
/* BOLT #4:
 *
 * * `amt_to_forward`: The amount, in millibronees, to forward to the next
...
static bool check_fwd_amount(struct htlc_in *hin,
			     struct amount_mbro amt_to_forward,
			     struct amount_mbro amt_in_htlc,
			     u32 feerate_base, u32 feerate_ppm)
{
	struct amount_mbro fee;
	struct amount_mbro fwd;

	if (!amount_mbro_fee(&fee, amt_to_forward,
	...
	if (amount_mbro_sub(&fwd, amt_in_htlc, fee)
	    && amount_mbro_greater_eq(fwd, amt_to_forward))
		return true;

	log_debug(hin->key.channel->log, "HTLC %"PRIu64" incorrect amount:"
		  " %s in, %s out, fee reqd %s",
		  hin->key.id,
		  type_to_string(tmpctx, struct amount_mbro, &amt_in_htlc),
		  type_to_string(tmpctx, struct amount_mbro, &amt_to_forward),
			    ...
			    const u8 *payment_metadata)
{
	 ...
	 */
	if (!amount_mbro_eq(amt_to_forward, hin->mbro)) {
		 ...
		 */
		failmsg = towire_final_incorrect_htlc_amount(NULL, hin->mbro);
		goto fail;
	}
	...
	htlc_set_add(ld, hin, total_mbro, payment_secret);
	return;
...
const u8 *send_htlc_out(const tal_t *ctx,
			struct channel *out,
			struct amount_mbro amount, u32 cltv,
			struct amount_mbro final_mbro,
			 ...
			 const struct pubkey *next_blinding)
{
	...
	if (next) {
		...
		struct amount_mbro best_spendable = channel_amount_spendable(next);
		...
		list_for_each(&peer->channels, channel, list) {
			...
			if (!amount_mbro_greater(spendable, best_spendable))
				continue;
			...
			if (amount_mbro_less(amt_to_forward,
	 ...
	 */
	if (!check_fwd_amount(hin, amt_to_forward, hin->mbro,
			      next->feerate_base,
			      next->feerate_ppm)) {
		/* Are we in old-fee grace-period? */
		if (!time_before(time_now(), next->old_feerate_timeout)
		    || !check_fwd_amount(hin, amt_to_forward, hin->mbro,
					 next->old_feerate_base,
					 next->old_feerate_ppm)) {
			failmsg = towire_fee_insufficient(tmpctx, hin->mbro,
	...
	if (amount_mbro_greater(amt_to_forward, next->htlc_maximum_mbro)
	    || amount_mbro_less(amt_to_forward, next->htlc_minimum_mbro)) {
		/* Are we in old-range grace-period? */
		if (!time_before(time_now(), next->old_feerate_timeout)
		    || amount_mbro_less(amt_to_forward, next->old_htlc_minimum_mbro)
		    || amount_mbro_greater(amt_to_forward, next->old_htlc_maximum_mbro)) {
			failmsg = towire_temporary_channel_failure(tmpctx,
								   get_channel_update(next));
			goto fail;
		}
		log_info(hin->key.channel->log,
			 "Allowing htlc using older htlc_minimum/maximum_mbro");
	}
	...
	failmsg = send_htlc_out(tmpctx, next, amt_to_forward,
				outgoing_cltv_value, AMOUNT_MBRO(0),
				&hin->payment_hash,
					 ...
					 struct plugin *plugin)
{
	...
	if (p->payload) {
		...
		json_add_amount_mbro_only(s, "forward_amount",
					  p->payload->amt_to_forward);
		json_add_u32(s, "outgoing_cltv_value", p->payload->outgoing_cltv);
		/* These are specified together in TLV, so only print total_mbro
		 * if payment_secret set (ie. modern, and final hop) */
		if (p->payload->payment_secret) {
			json_add_amount_mbro_only(s, "total_mbro",
						  *p->payload->total_mbro);
	...
	json_add_amount_mbro_only(s, "amount", hin->mbro);
...
static void remove_htlc_in(struct channel *channel, struct htlc_in *hin)
{
	...
	if (hin->preimage) {
		struct amount_mbro oldamt = channel->our_mbro;
		const struct channel_coin_mvt *mvt;

		if (!amount_mbro_add(&channel->our_mbro, channel->our_mbro,
				     hin->mbro)) {
			channel_internal_error(channel,
					       "Overflow our_mbro %s + HTLC %s",
		...
		log_debug(channel->log, "Balance %s -> %s",
			  type_to_string(tmpctx, struct amount_mbro, &oldamt),
			  type_to_string(tmpctx, struct amount_mbro,
					 &channel->our_mbro));
		if (amount_mbro_greater(channel->our_mbro,
					channel->mbro_to_us_max))
			channel->mbro_to_us_max = channel->our_mbro;
...
static void remove_htlc_out(struct channel *channel, struct htlc_out *hout)
{
	...
	if (!hout->preimage) {
		...
		if (!amount_mbro_sub(&channel->our_mbro, channel->our_mbro,
				     hout->mbro)) {
			channel_internal_error(channel,
					       "Underflow our_mbro %s - HTLC %s",
					       type_to_string(tmpctx,
							      struct amount_mbro,
							      &channel->our_mbro),
					       type_to_string(tmpctx,
		...
		log_debug(channel->log, "Balance %s -> %s",
			  ...
			  type_to_string(tmpctx, struct amount_mbro,
					 &channel->our_mbro));
		if (amount_mbro_less(channel->our_mbro, channel->mbro_to_us_min))
			channel->mbro_to_us_min = channel->our_mbro;
...
static bool peer_save_commitsig_received(struct channel *channel, u64 commitnum,
					 ...
					 const struct brocoin_signature *commit_sig)
{
...
void peer_sending_commitsig(struct channel *channel, const u8 *msg)
{
	...
	struct brocoin_signature commit_sig;
				     ...
				     const struct added_htlc *added)
{
	 ...
	 */
	if (amount_mbro_eq(added->amount, AMOUNT_MBRO(0))
	    || amount_mbro_less(added->amount, channel->our_config.htlc_minimum)) {
		channel_internal_error(channel,
...
void peer_got_commitsig(struct channel *channel, const u8 *msg)
{
	...
	struct fee_states *fee_states;
	struct height_states *blockheight_states;
	struct brocoin_signature commit_sig, *htlc_sigs;
	struct added_htlc *added;
	struct fulfilled_htlc *fulfilled;
	struct failed_htlc **failed;
	struct changed_htlc *changed;
	...
	/* If we're not synced with brocoin network, we can't accept
...
struct commitment_revocation_payload {
	struct brocoin_txid commitment_txid;
...
static void fixup_hout(struct lightningd *ld, struct htlc_out *hout)
{
	...
	log_broken(ld->log, "HTLC #%"PRIu64" (%s) "
		   ...
		   hout->key.id, htlc_state_name(hout->hstate),
		   type_to_string(tmpctx, struct amount_mbro, &hout->mbro),
