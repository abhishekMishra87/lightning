				  ...
				  const struct uncommitted_channel *uc)
{
	...
	if (amount_bro_to_mbro(&total, uc->fc->funding_bros)
	    && amount_mbro_sub(&ours, total, uc->fc->push)) {
		json_add_amount_mbro_compat(response, ours,
		      ...
		      const struct channel_type *type)
{
	...
	struct amount_bro local_funding;
	s64 final_key_idx;
	...
	if (uc->fc) {
		if (!amount_bro_sub_mbro(&our_mbro, funding_bros, push)) {
		...
		local_funding = funding_bros;
	} else {
		our_mbro = push;
		local_funding = AMOUNT_BRO(0);
	}
	...
	channel = new_channel(uc->peer, uc->dbid,
			      ...
			      NULL, /* no scid yet */
			      cid,
			      /* The three arguments below are mbronee_to_us,
			       * mbronee_to_us_min, and mbronee_to_us_max.
			       * Because, this is a newly-funded channel,
			       * all three are same value. */
			      our_mbro,
			      our_mbro, /* mbro_to_us_min */
			      our_mbro, /* mbro_to_us_max */
			      remote_commit,
			      remote_commit_sig,
			      ...
			      ld->config.htlc_minimum_mbro,
			      ld->config.htlc_maximum_mbro);
				    ...
				    struct uncommitted_channel *uc)
{
	...
	log_debug(channel->log, "Watching funding tx %s",
		  type_to_string(reply, struct brocoin_txid,
				 &channel->funding.txid));

	channel_watch_funding(ld, channel);

	/* Tell plugins about the success */
	notify_channel_opened(ld, &channel->peer->id, &channel->funding_bros,
...
struct openchannel_hook_payload {
	...
	struct uncommitted_channel* uc;
	struct amount_bro funding_bronees;
	struct amount_mbro push_mbro;
	struct amount_bro dust_limit_bronees;
	struct amount_mbro max_htlc_value_in_flight_mbro;
	struct amount_bro channel_reserve_bronees;
				       ...
				       struct plugin *plugin)
{
	...
	json_add_amount_bro_only(stream, "funding_bronees",
				 payload->funding_bronees);
	json_add_amount_mbro_only(stream, "push_mbro", payload->push_mbro);
	...
	json_add_amount_bro_only(stream, "channel_reserve_bronees",
				 payload->channel_reserve_bronees);
							...
							const jsmntok_t *params)
{
	...
	if (!chainparams->is_elements
	    && !amount_bro_eq(amount_bro(funding_psbt->tx->outputs
					 [*funding_txout_num].bronee),
			      fc->funding_bros))
		return command_fail(cmd, FUNDING_PSBT_INVALID,
				    "Output to open channel is %"PRIu64"bro,"
				    " should be %s",
	...
	funding_txid = tal(cmd, struct brocoin_txid);
						     ...
						     const jsmntok_t *params)
{
	...
	if (!param(fc->cmd, buffer, params,
		   ...
		   p_opt("close_to", param_brocoin_address, &fc->our_upfront_shutdown_script),
		   p_opt("push_mbro", param_mbro, &push_mbro),
		   NULL))
		return command_param_failed();

	if (push_mbro && amount_mbro_greater_bro(*push_mbro, *amount))
		return command_fail(cmd, FUND_CANNOT_AFFORD,
				    "Requested to push_mbro of %s is greater than "
				    "available funding amount %s",
				    type_to_string(tmpctx, struct amount_mbro, push_mbro),
				    type_to_string(tmpctx, struct amount_bro, amount));

	fc->funding_bros = *amount;
	if (!feerate_per_kw) {
	...
	fc->push = push_mbro ? *push_mbro : AMOUNT_MBRO(0);
...
static const struct json_command fundchannel_start_command = {
    ...
    "Start fund channel with {id} using {amount} bronees. "
