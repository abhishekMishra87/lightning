			     ...
			     const struct wallet_payment *t)
{
	...
	if (amount_mbro_greater(t->mbronee, AMOUNT_MBRO(0)))
		json_add_amount_mbro_compat(response, t->mbronee, "mbronee",
					    "amount_mbro");

	json_add_amount_mbro_compat(response, t->mbronee_sent,
				    "mbronee_sent", "amount_sent_mbro");
...
static const u8 *send_onion(const tal_t *ctx, struct lightningd *ld,
			    ...
			    const struct amount_mbro final_amount,
			    const struct sha256 *payment_hash,
		  ...
		  const struct sha256 *local_offer_id)
{
	...
	struct htlc_out *hout;
	struct routing_failure *fail;
	struct amount_mbro mbro_already_pending = AMOUNT_MBRO(0);
	...
	for (size_t i = 0; i < tal_count(payments); i++) {
		...
		case PAYMENT_COMPLETE:
			...
			if (!amount_mbro_eq(payments[i]->mbronee, mbro)) {
		...
		case PAYMENT_PENDING:
			...
			else if (!amount_mbro_eq(payments[i]->total_mbro,
						 total_mbro))
				return command_fail(cmd, JSONRPC2_INVALID_PARAMS,
						    "mbronee was previously %s, now %s",
						    type_to_string(tmpctx,
								   struct amount_mbro,
								   &payments[i]->total_mbro),
						    type_to_string(tmpctx,
								   struct amount_mbro,
								   &total_mbro));


			if (!amount_mbro_add(&mbro_already_pending,
					     mbro_already_pending,
					     payments[i]->mbronee)) {
	...
	/* BOLT #4:
	 *
	 * - MUST NOT send another HTLC if the total `amount_mbro` of the HTLC
	 *   set is already greater or equal to `total_mbro`.
	 */
	/* We don't do this for single 0-value payments (sendonion does this) */
	if (!amount_mbro_eq(total_mbro, AMOUNT_MBRO(0))
	    && amount_mbro_greater_eq(mbro_already_pending, total_mbro)) {
		return command_fail(cmd, PAY_IN_PROGRESS,
	...
	failmsg = send_onion(tmpctx, ld, packet, first_hop, mbro,
			     rhash, NULL, partid,
			     group, channel, &hout);
	...
	payment->mbronee = mbro;
	payment->mbronee_sent = first_hop->amount;
	payment->total_mbro = total_mbro;
	     ...
	     const u8 *payment_metadata)
{
	...
	onion = onion_final_hop(cmd,
				...
				total_mbro, route[i].blinding, route[i].enctlv,
	...
	log_info(ld->log, "Sending %s over %zu hops to deliver %s",
		 type_to_string(tmpctx, struct amount_mbro, &route[0].amount),
		 n_hops, type_to_string(tmpctx, struct amount_mbro, &mbro));
		...
		const jsmntok_t *tok, struct route_hop **hop)
{
	...
	amounttok = json_get_member(buffer, tok, "amount_mbro");
	...
	if (!json_to_mbro(buffer, amounttok, &res->amount))
		return command_fail_badparam(cmd, name, buffer, amounttok,
					     "should be a valid amount_mbro");
					     ...
					     const jsmntok_t *params)
{
	...
	if (!param(cmd, buffer, params,
		   ...
		   p_opt_def("mbronee", param_mbro, &mbro, AMOUNT_MBRO(0)),
					       ...
					       struct route_hop **hops)
{
	...
	*hops = tal_arr(cmd, struct route_hop, tok->size);
	json_for_each_arr(i, t, tok) {
		struct amount_mbro *mbro, *amount_mbro;
		struct node_id *id;
		...
		if (!param(cmd, buffer, t,
			   ...
			   p_opt("amount_mbro", param_mbro, &amount_mbro),
		...
		if (!mbro && !amount_mbro)
			return command_fail(cmd, JSONRPC2_INVALID_PARAMS,
					    "%s[%zi]: must have mbronee"
					    " or amount_mbro", name, i);
		if (!id || !channel || !delay)
			return command_fail(cmd, JSONRPC2_INVALID_PARAMS,
					    "%s[%zi]: must have id, channel"
					    " and delay", name, i);
		if (mbro && amount_mbro && !amount_mbro_eq(*mbro, *amount_mbro))
			return command_fail(cmd, JSONRPC2_INVALID_PARAMS,
					    "%s[%zi]: mbronee %s != amount_mbro %s",
					    name, i,
					    type_to_string(tmpctx,
							   struct amount_mbro,
							   mbro),
					   ...
					   const jsmntok_t *params)
{
	...
	if (*partid && !mbro)
		return command_fail(cmd, JSONRPC2_INVALID_PARAMS,
				    "Must specify mbronee with partid");

	const struct amount_mbro final_amount = route[tal_count(route)-1].amount;
	...
	if (mbro && !*partid && !amount_mbro_eq(*mbro, final_amount))
		return command_fail(cmd, JSONRPC2_INVALID_PARAMS,
				    "Do not specify mbronee (%s) without"
	 ...
	 * we promise to send (mbronee).  So no single payment can be
	 * > than that. */
	if (*partid) {
		if (amount_mbro_greater(final_amount, *mbro))
			return command_fail(cmd, JSONRPC2_INVALID_PARAMS,
