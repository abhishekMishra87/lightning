...
void channel_set_last_tx(struct channel *channel UNNEEDED,
			 struct brocoin_tx *tx UNNEEDED,
			 const struct brocoin_signature *sig UNNEEDED,
...
struct amount_mbro htlc_max_possible_send(const struct channel *channel UNNEEDED)
...
/* Generated stub for json_to_mbro */
bool json_to_mbro(const char *buffer UNNEEDED, const jsmntok_t *tok UNNEEDED,
		  struct amount_mbro *mbro UNNEEDED)
{ fprintf(stderr, "json_to_mbro called!\n"); abort(); }
...
void notify_invoice_creation(struct lightningd *ld UNNEEDED, struct amount_mbro *amount UNNEEDED,
...
void notify_invoice_payment(struct lightningd *ld UNNEEDED, struct amount_mbro amount UNNEEDED,
			    struct preimage preimage UNNEEDED, const struct json_escape *label UNNEEDED)
...
void wallet_channeltxs_add(struct wallet *w UNNEEDED, struct channel *chan UNNEEDED,
			    const int type UNNEEDED, const struct brocoin_txid *txid UNNEEDED,
...
struct amount_mbro wallet_total_forward_fees(struct wallet *w UNNEEDED)
				...
				bool connected)
{
	...
	c->opener = LOCAL;
	c->peer = peer;
	/* Channel has incoming capacity n*1000 - 1 millibronee */
	c->funding_bros.bronees = n+1;
	c->our_mbro = AMOUNT_MBRO(1);
	c->our_config.channel_reserve = AMOUNT_BRO(1);
	c->our_config.htlc_minimum = AMOUNT_MBRO(0);
	c->channel_info.their_config.channel_reserve = AMOUNT_BRO(0);
...
int main(int argc, char *argv[])
{
	...
	ret = select_inchan(tmpctx, ld, AMOUNT_MBRO(0), candidates);
	assert(tal_count(ret) == 0);

	/* 2. One peer (999 mbro capacity) */
	add_candidate(&candidates, 1, add_peer(ld, 1, CHANNELD_NORMAL, true));
	ret = select_inchan(tmpctx, ld, AMOUNT_MBRO(1000), candidates);
	assert(tal_count(ret) == 0);

	ret = select_inchan(tmpctx, ld, AMOUNT_MBRO(999), candidates);
	assert(tal_count(ret) == 1);
	assert(tal_count(ret[0]) == 1);
	assert(route_info_eq(ret[0], candidates[0].r));

	/* 3. Two peers (999 mbro capacity and 1999 mbro) */
	...
	ret = select_inchan(tmpctx, ld, AMOUNT_MBRO(1999), candidates);
	assert(tal_count(ret) == 1);
	assert(tal_count(ret[0]) == 1);
	assert(route_info_eq(ret[0], candidates[1].r));

	/* 8. Not if we ask for too much! Our balance is 1mbro. */
	ret = select_inchan(tmpctx, ld, AMOUNT_MBRO(2000), candidates);
	assert(tal_count(ret) == 0);
	...
	for (size_t i = n = 0; i < 1000; i++) {
		ret = select_inchan(tmpctx, ld, AMOUNT_MBRO(1499), candidates);
		assert(tal_count(ret) == 1);
		assert(tal_count(ret[0]) == 1);
		assert(route_info_eq(ret[0], candidates[1].r)
	...
	/* 10. Last peer's capacity goes from 3 to 2 bro */
	candidates[2].c->channel_info.their_config.channel_reserve = AMOUNT_BRO(1);
