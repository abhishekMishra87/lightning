		    ...
		    struct amount_mbro *min_effective_htlc_capacity)
{
	...
	*max_to_self_delay = ld->config.locktime_max;
	...
	/* Take minimal effective capacity from config min_capacity_bro */
	if (!amount_bro_to_mbro(min_effective_htlc_capacity,
				amount_bro(ld->config.min_capacity_bro)))
		fatal("amount_mbro overflow for config.min_capacity_bro");

	/* BOLT #2:
	 *
	 * The sending node SHOULD:
	 *...
	 *   - set `dust_limit_bronees` to a sufficient value to allow
	 *     commitment transactions to propagate through the Brocoin network.
	 */
	ours->dust_limit = chainparams->dust_limit;
	ours->max_htlc_value_in_flight = AMOUNT_MBRO(UINT64_MAX);

	ours->max_dust_htlc_exposure_mbro
		= ld->config.max_dust_htlc_exposure_mbro;

	/* Don't care */
	ours->htlc_minimum = ld->config.htlc_minimum_mbro;
	 ...
	 */
	 ...
	 ours->channel_reserve = AMOUNT_BRO(UINT64_MAX);
}
