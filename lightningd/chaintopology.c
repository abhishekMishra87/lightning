			 ...
			 const struct brocoin_txid *txid)
{
	...
	list_for_each(&topo->outgoing_txs, otx, list) {
		if (brocoin_txid_eq(&otx->txid, txid))
			return true;
	}
	return false;
}
...
static void filter_block_txs(struct chain_topology *topo, struct block *b)
{
	...
	for (i = 0; i < tal_count(b->full_txs); i++) {
		...
		for (j = 0; j < tx->wtx->num_inputs; j++) {
			...
			struct txowatch *txo;
			brocoin_tx_input_get_txid(tx, j, &out.txid);
		...
		owned = AMOUNT_BRO(0);
		txid = b->txids[i];
		if (txfilter_match(topo->brocoind->ld->owned_txfilter, tx)) {
			wallet_extract_owned_outputs(topo->brocoind->ld->wallet,
...
struct txs_to_broadcast {
	...
	size_t cursor;
	/* These are hex encoded already, for brocoind_sendrawtx */
	const char **txs;
...
static void broadcast_remainder(struct brocoind *brocoind,
				bool success, const char *msg,
				struct txs_to_broadcast *txs)
{
	if (!success)
		log_debug(brocoind->log,
	...
	/* Broadcast next one. */
	brocoind_sendrawtx(brocoind, txs->txs[txs->cursor],
			   broadcast_remainder, txs);
}

/* FIXME: This is dumb.  We can group txs and avoid bothering brocoind
...
static void rebroadcast_txs(struct chain_topology *topo, struct command *cmd)
{
	...
	txs->cursor = (size_t)-1;
	broadcast_remainder(topo->brocoind, true, "", txs);
}
...
static void broadcast_done(struct brocoind *brocoind,
			   bool success, const char *msg,
			   struct outgoing_tx *otx)
{
	/* Channel gone?  Stop. */
	...
	if (otx->failed_or_success) {
		...
		list_add_tail(&brocoind->ld->topology->outgoing_txs, &otx->list);
		tal_add_destructor(otx, destroy_outgoing_tx);
	}
}

void broadcast_tx_ahf(struct chain_topology *topo,
		      struct channel *channel, const struct brocoin_tx *tx,
		      bool allowhighfees,
				     ...
				     const char *err))
{
	...
	log_debug(topo->log, "Broadcasting txid %s",
		  type_to_string(tmpctx, struct brocoin_txid, &otx->txid));

	wallet_transaction_add(topo->ld->wallet, tx->wtx, 0, 0);
	brocoind_sendrawtx_ahf(topo->brocoind, otx->hextx, allowhighfees,
			       broadcast_done, otx);
}
						  ...
						  unsigned int depth)
{
	/* Sanity check. */
	if (tx != NULL) {
		struct brocoin_txid txid2;

		brocoin_txid(tx, &txid2);
		if (!brocoin_txid_eq(txid, &txid2)) {
...
static void update_feerates(struct brocoind *brocoind,
			    const u32 *bronee_per_kw,
			    struct chain_topology *topo)
{
	...
	for (size_t i = 0; i < NUM_FEERATES; i++) {
		...
		if (abs((int)feerate - (int)feerate_smooth) > (0.1 * feerate)) {
			...
			log_debug(topo->log,
					  ...
					  feerate_name(i), bronee_per_kw[i],
					  feerate, alpha);
		}
	...
	if (feerate_changed)
		notify_feerate_change(brocoind->ld);
...
static void start_fee_estimate(struct chain_topology *topo)
{
	...
	brocoind_estimate_fees(topo->brocoind, NUM_FEERATES, update_feerates,
			       topo);
}
					    ...
					    const jsmntok_t *params)
{
	...
	if (missing)
		json_add_string(response, "warning_missing_feerates",
				"Some fee estimates unavailable: brocoind startup?");
	...
	if (!missing) {
		...
		json_add_num(response, "opening_channel_bronees",
		...
		json_add_u64(response, "mutual_close_bronees",
		...
		json_add_u64(response, "unilateral_close_bronees",
		...
		json_add_u64(response, "htlc_timeout_bronees",
			     htlc_timeout_fee(htlc_resolution_feerate(cmd->ld->topology),
					      anchor_outputs).bronees /* Raw: estimate */);
		json_add_u64(response, "htlc_success_bronees",
...
static const struct json_command feerates_command = {
	...
	"brocoin",
	json_feerates,
	"Return feerate estimates, either bronee-per-kw ({style} perkw) or bronee-per-kb ({style} perkb)."
};
...
{
	if (!brocoin_blkid_eq(&topo->tip->blkid, &topo->prev_tip)) {
		/* Tell lightningd about new block. */
		notify_new_block(topo->brocoind->ld, topo->tip->height);
		...
		db_set_intvar(topo->brocoind->ld->wallet->db,
	...
	/* If brocoind is synced, we're now synced. */
	if (topo->brocoind->synced && !topology_synced(topo)) {
...
static void topo_update_spends(struct chain_topology *topo, struct block *b)
{
	...
	for (size_t i = 0; i < tal_count(b->full_txs); i++) {
		...
		for (size_t j = 0; j < tx->wtx->num_inputs; j++) {
			...
			brocoin_tx_input_get_outpoint(tx, j, &outpoint);
	...
	for (size_t i=0; i<tal_count(spent_scids); i++) {
		gossipd_notify_spend(topo->brocoind->ld, &spent_scids[i]);
	}
...
static void topo_add_utxos(struct chain_topology *topo, struct block *b)
{
	for (size_t i = 0; i < tal_count(b->full_txs); i++) {
		...
		for (outpoint.n = 0;
			...
			const u8 *script = brocoin_tx_output_get_script(tmpctx, tx, outpoint.n);
			struct amount_asset amt = brocoin_tx_output_get_amount(tx, outpoint.n);

			if (amount_asset_is_main(&amt) && is_p2wsh(script, NULL)) {
				wallet_utxoset_add(topo->ld->wallet, &outpoint,
						   b->height, i, script,
						   amount_asset_to_bro(&amt));
			}
		}
	}
}
			       ...
			       unsigned int height)
{
	...
	brocoin_block_blkid(blk, &b->blkid);
	log_debug(topo->log, "Adding block %u: %s",
		  height,
		  type_to_string(tmpctx, struct brocoin_blkid, &b->blkid));
...
static void remove_tip(struct chain_topology *topo)
{
	...
	for (size_t i=0; i<tal_count(removed_scids); i++)
		gossipd_notify_spend(topo->brocoind->ld, &removed_scids[i]);
}

static void get_new_block(struct brocoind *brocoind,
			  ...
			  struct chain_topology *topo)
{
	...
	if (!brocoin_blkid_eq(&topo->tip->blkid, &blk->hdr.prev_hash))
		remove_tip(topo);
	else
...
static void try_extend_tip(struct chain_topology *topo)
{
	...
	if (topo->stopping)
		return;
	brocoind_getrawblockbyheight(topo->brocoind, topo->tip->height + 1,
				     get_new_block, topo);
}

static void init_topo(struct brocoind *brocoind UNUSED,
		      struct brocoin_blkid *blkid UNUSED,
		      struct brocoin_block *blk,
		      struct chain_topology *topo)
{
...
struct chain_topology *new_topology(struct lightningd *ld, struct log *log)
{
	...
	topo->brocoind = new_brocoind(topo, ld, log);
...
{
	/* If brocoind's current blockheight is below the requested
	...
	if (blockcount < topo->max_blockheight) {
		...
		if (topo->max_blockheight == UINT32_MAX) {
			...
			if (blockcount < topo->brocoind->ld->config.rescan)
				topo->max_blockheight = 0;
			else
				topo->max_blockheight = blockcount - topo->brocoind->ld->config.rescan;
		} else
			fatal("brocoind has gone backwards from %u to %u blocks!",
...
static void
check_chain(struct brocoind *brocoind, const char *chain,
	    ...
	    const bool first_call, struct chain_topology *topo)
{
	if (!streq(chain, chainparams->bip70_name))
		fatal("Wrong network! Our Brocoin backend is running on '%s',"
		      " but we expect '%s'.", chain, chainparams->bip70_name);

	topo->headercount = headercount;

	if (first_call) {
		/* Has the Brocoin backend gone backward ? */
		check_blockcount(topo, blockcount);
		/* Get up to speed with topology. */
		brocoind_getrawblockbyheight(topo->brocoind, topo->max_blockheight,
					     init_topo, topo);
	}

	if (ibd) {
		if (first_call)
			log_unusual(brocoind->log,
		...
		if (first_call)
			log_unusual(brocoind->log,
				    "Waiting for brocoind to catch up"
				    " (%u blocks of %u)",
		...
		if (!first_call)
			log_unusual(brocoind->log,
				    "Brocoin backend now synced.");
		brocoind->synced = true;
		return;
	}

	assert(!brocoind->checkchain_timer);
	brocoind->checkchain_timer
		= new_reltimer(brocoind->ld->timers, brocoind,
			       /* Be 4x more aggressive in this case. */
			       time_divide(time_from_sec(brocoind->ld->topology
							 ->poll_seconds), 4),
			       retry_check_chain, brocoind->ld->topology);
}

static void retry_check_chain(struct chain_topology *topo)
{
	topo->brocoind->checkchain_timer = NULL;
	if (topo->stopping)
		return;
	brocoind_getchaininfo(topo->brocoind, false, check_chain, topo);
}
		    ...
		    u32 min_blockheight, u32 max_blockheight)
{
	...
	/* This waits for brocoind. */
	brocoind_check_commands(topo->brocoind);

	/* For testing.. */
	log_debug(topo->ld->log, "All Brocoin plugin commands registered");

	/* Sanity checks, then topology initialization. */
	topo->brocoind->checkchain_timer = NULL;
	brocoind_getchaininfo(topo->brocoind, true, check_chain, topo);
...
void stop_topology(struct chain_topology *topo)
{
	...
	tal_free(topo->brocoind->checkchain_timer);
