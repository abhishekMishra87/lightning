...
resolve_one_close_command(struct close_command *cc, bool cooperative)
{
	...
	brocoin_txid(cc->channel->last_tx, &txid);
...
static struct amount_bro calc_tx_fee(struct amount_bro bro_in,
				     const struct brocoin_tx *tx)
{
	struct amount_asset amt;
	struct amount_bro fee = bro_in;
	const u8 *oscript;
	size_t scriptlen;
	for (size_t i = 0; i < tx->wtx->num_outputs; i++) {
		amt = brocoin_tx_output_get_amount(tx, i);
		oscript = brocoin_tx_output_get_script(NULL, tx, i);
		...
		if (!amount_bro_sub(&fee, fee, amount_asset_to_bro(&amt)))
			fatal("Tx spends more than input %s? %s",
			      type_to_string(tmpctx, struct amount_bro, &bro_in),
...
static bool closing_fee_is_acceptable(struct lightningd *ld,
				      ...
				      const struct brocoin_tx *tx)
{
	struct amount_bro fee, last_fee, min_fee;
	u64 weight;
	u32 min_feerate;
	bool feerate_unknown;

	/* Calculate actual fee (adds in eliminated outputs) */
	fee = calc_tx_fee(channel->funding_bros, tx);
	last_fee = calc_tx_fee(channel->funding_bros, channel->last_tx);

	/* Weight once we add in sigs. */
	assert(!tx->wtx->inputs[0].witness
	       || tx->wtx->inputs[0].witness->num_items == 0);
	weight = brocoin_tx_weight(tx) + brocoin_tx_2of2_input_witness_weight();
	...
	log_debug(channel->log, "Their actual closing tx fee is %s"
		 " vs previous %s: weight is %"PRIu64,
		  ...
		  type_to_string(tmpctx, struct amount_bro, &last_fee),
		  weight);
	...
	if (amount_bro_less(fee, min_fee)) {
		log_debug(channel->log, "... That's below our min %s"
			  " for weight %"PRIu64" at feerate %u",
			  type_to_string(tmpctx, struct amount_bro, &min_fee),
			  weight, min_feerate);
		return false;
	}
					    ...
					    const u8 *msg)
{
	...
	funding_wscript = brocoin_redeem_2of2(tmpctx,
	...
	brocoin_txid(channel->last_tx, &tx_id);
...
void peer_start_closingd(struct channel *channel, struct peer_fd *peer_fd)
{
	...
	struct amount_mbro their_mbro;
	struct amount_bro feelimit;
	int hsmfd;
	struct lightningd *ld = channel->peer->ld;
	...
	if (!amount_bro_sub_mbro(&their_mbro,
		...
		log_broken(channel->log, "our_mbro overflow funding %s minus %s",
	...
	initmsg = towire_closingd_init(tmpctx,
				       ...
				       &channel->channel_info.remote_fundingkey,
				       channel->opener,
				       amount_mbro_to_bro_round_down(channel->our_mbro),
				       amount_mbro_to_bro_round_down(their_mbro),
...
{
	*outp = tal(cmd, struct brocoin_outpoint);
					 ...
					 const jsmntok_t *params)
{
	...
	if (!param(cmd, buffer, params,
		   ...
		   p_opt_def("unilateraltimeout", param_number, &timeout,
			     48 * 3600),
		   p_opt("destination", param_brocoin_address, &close_to_script),
