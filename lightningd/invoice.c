...
invoice_payment_hooks_done(struct invoice_payment_hook_payload *payload STEALS)
{
	...
	if (!wallet_invoice_resolve(ld->wallet, invoice, payload->mbro)) {
		htlc_set_fail(payload->set, take(failmsg_incorrect_or_unknown(
							 NULL, ld, payload->set->htlcs[0])));
		return;
	}
	...
	log_info(ld->log, "Resolved invoice '%s' with amount %s in %zu htlcs",
		 payload->label->s,
		 type_to_string(tmpctx, struct amount_mbro, &payload->mbro),
		 tal_count(payload->set->htlcs));
	htlc_set_fulfill(payload->set, &payload->preimage);

	notify_invoice_payment(ld, payload->mbro, payload->preimage,
			       payload->label);
}
		      ...
		      const struct secret *payment_secret)
{
	 ...
	 *     - MUST fail the HTLC.
	 */
	if (details->mbro != NULL) {
		struct amount_mbro twice;

		if (amount_mbro_less(mbro, *details->mbro)) {
			log_debug(ld->log, "Attept to pay %s with amount %s < %s",
				  type_to_string(tmpctx, struct sha256,
						 &details->rhash),
				  type_to_string(tmpctx, struct amount_mbro, &mbro),
				  type_to_string(tmpctx, struct amount_mbro, details->mbro));
			return tal_free(details);
		}

		if (amount_mbro_add(&twice, *details->mbro, *details->mbro)
		    && amount_mbro_greater(mbro, twice)) {
			log_debug(ld->log, "Attept to pay %s with amount %s > %s",
				  type_to_string(tmpctx, struct sha256,
						 &details->rhash),
				  type_to_string(tmpctx, struct amount_mbro, &mbro),
				  type_to_string(tmpctx, struct amount_mbro, &twice));
			/* BOLT #4:
			 *
...
static struct route_info **select_inchan(const tal_t *ctx,
					 struct lightningd *ld,
					 struct amount_mbro amount_needed,
					 ...
					 *candidates)
{
	...
	for (size_t i = 0; i < tal_count(candidates); i++) {
		struct amount_mbro excess, capacity;
		struct amount_bro cumulative_reserve;
		double excess_frac;

		/* Does the peer have sufficient balance to pay us,
		 * even after having taken into account their reserve? */
		if (!amount_mbro_sub(&excess, candidates[i].capacity,
				     amount_needed))
			continue;
		...
		|-------|-------------|--------------------------------|-------------------|
		0       ^             ^                                ^                funding
		   our_reserve     our_mbro	*/

		/* Find capacity and calculate its excess fraction */
		if (!amount_bro_add(&cumulative_reserve,
				    candidates[i].c->our_config.channel_reserve,
				    candidates[i].c->channel_info.their_config.channel_reserve)
			|| !amount_bro_to_mbro(&capacity, candidates[i].c->funding_bros)
			|| !amount_mbro_sub_bro(&capacity, capacity, cumulative_reserve)) {
		 ...
		 * only one!  So bump it by 1 mbro */
		if (!amount_mbro_add(&excess, excess, AMOUNT_MBRO(1))) {
		...
		excess_frac = amount_mbro_ratio(excess, capacity);
					     ...
					     *candidates)
{
	...
	struct amount_mbro gathered;
	/* Routehint array.  */
	struct route_info **routehints;

	gathered = AMOUNT_MBRO(0);
	...
	for (size_t i = 0; i < tal_count(candidates); i++) {
		if (amount_mbro_greater_eq(gathered, amount_needed))
			break;

		/* Add to current routehints set.  */
		if (!amount_mbro_add(&gathered, gathered, candidates[i].capacity)) {
			log_broken(ld->log,
				   "Gathered channel capacity overflow: "
				   "%s + %s",
				   type_to_string(tmpctx, struct amount_mbro, &gathered),
				   type_to_string(tmpctx, struct amount_mbro,
	       ...
	       bool *warning_private_unused)
{
	...
	struct amount_mbro avail_capacity, deadend_capacity, offline_capacity,
		private_capacity;
	struct routehint_candidate *candidates;
	struct amount_mbro total, needed;
	...
	needed = info->b11->mbro ? *info->b11->mbro : AMOUNT_MBRO(1);
	...
	log_debug(info->cmd->ld->log, "needed = %s, avail_capacity = %s, private_capacity = %s, offline_capacity = %s, deadend_capacity = %s",
		  type_to_string(tmpctx, struct amount_mbro, &needed),
		  type_to_string(tmpctx, struct amount_mbro, &avail_capacity),
		  type_to_string(tmpctx, struct amount_mbro, &private_capacity),
		  type_to_string(tmpctx, struct amount_mbro, &offline_capacity),
		  type_to_string(tmpctx, struct amount_mbro, &deadend_capacity));

	if (!amount_mbro_add(&total, avail_capacity, offline_capacity)
	    || !amount_mbro_add(&total, total, deadend_capacity)
	    || !amount_mbro_add(&total, total, private_capacity))
	...
	*warning_capacity = amount_mbro_greater_eq(needed, total);
	...
	*warning_private_unused = false;
	if (amount_mbro_greater(needed, avail_capacity)) {
		struct amount_mbro tot;

		/* We didn't get enough: would offline have helped? */
		if (!amount_mbro_add(&tot, avail_capacity, offline_capacity))
			abort();
		if (amount_mbro_greater_eq(tot, needed)) {
			*warning_offline = true;
			goto done;
		}

		/* Hmm, what about deadends? */
		if (!amount_mbro_add(&tot, tot, deadend_capacity))
			abort();
		if (amount_mbro_greater_eq(tot, needed)) {
		...
		if (!amount_mbro_add(&tot, tot, private_capacity))
			abort();
		if (amount_mbro_greater_eq(tot, needed)) {
		 ...
		 bool warning_private_unused)
{
	...
	notify_invoice_creation(info->cmd->ld, info->b11->mbro,
				       ...
				       const jsmntok_t *routetok)
{
	...
	json_for_each_arr(i, t, routetok) {
		...
		fee_base = json_get_member(buffer, t, "fee_base_mbro");
		...
		if (!json_to_node_id(buffer, pubkey, &r->pubkey)
		    || !json_to_short_channel_id(buffer, scid,
						 &r->short_channel_id)
		    || !json_to_number(buffer, fee_base, &r->fee_base_mbro)
...
static struct command_result *param_positive_mbro_or_any(struct command *cmd,
							 const char *name,
							 const char *buffer,
							 const jsmntok_t *tok,
							 struct amount_mbro **mbro)
{
	...
	*mbro = tal(cmd, struct amount_mbro);
	if (parse_amount_mbro(*mbro, buffer + tok->start, tok->end - tok->start)
	    && !amount_mbro_eq(**mbro, AMOUNT_MBRO(0)))
		return NULL;

	return command_fail_badparam(cmd, name, buffer, tok,
				     "should be positive mbro or 'any'");
}
					   ...
					   const jsmntok_t *params)
{
	...
	struct amount_mbro *mbronee_val;
	struct invoice_info *info;
	const char *desc_val;
	...
	if (!param(cmd, buffer, params,
		   p_req("mbronee", param_positive_mbro_or_any, &mbronee_val),
	...
	info->b11 = new_bolt11(info, mbronee_val);
...
static const struct json_command invoice_command = {
	...
	"payment",
	json_invoice,
	"Create an invoice for {mbronee} with {label} "
						 ...
						 const jsmntok_t *params)
{
	...
	if (b11) {
		...
		notify_invoice_creation(cmd->ld, b11->mbro, *preimage, label);
	} else {
		struct tlv_invoice *inv;
