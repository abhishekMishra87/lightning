...
static void dev_register_opts(struct lightningd *ld)
{
	...
	opt_register_arg("--dev-brocoind-poll", opt_set_u32, opt_show_u32,
...
static const struct config testnet_config = {
	...
	.htlc_minimum_mbro = AMOUNT_MBRO(0),
	.htlc_maximum_mbro = AMOUNT_MBRO(-1ULL),  /* no limit */

	/* Max amount of dust allowed per channel (50kbro) */
	.max_dust_htlc_exposure_mbro = AMOUNT_MBRO(50000000),
	...
	/* Sets min_effective_htlc_capacity - at 1000$/BRON this is 10ct */
	.min_capacity_bro = 10000,
...
{
	char *extra = tal_fmt(NULL, "\nA brocoin lightning daemon (default "
...
static char *opt_set_mbro(const char *arg, struct amount_mbro *amt)
{
	if (!parse_amount_mbro(amt, arg, strlen(arg)))
		return tal_fmt(NULL, "Unable to parse millibronee '%s'", arg);
...
static void register_opts(struct lightningd *ld)
{
	...
	opt_register_early_noarg("--large-channels|--wumbo",
				 opt_set_wumbo, ld,
				 "Allow channels larger than 0.16777215 BRON");
	...
	opt_register_arg("--fee-base", opt_set_u32, opt_show_u32,
			 &ld->config.fee_base,
			 "Millibronee minimum to charge for HTLC");
	...
	opt_register_arg("--fee-per-bronee", opt_set_u32, opt_show_u32,
			 &ld->config.fee_per_bronee,
			 "Microbronee fee for every bronee in HTLC");
	opt_register_arg("--htlc-minimum-mbro", opt_set_mbro, NULL,
			 &ld->config.htlc_minimum_mbro,
			 "The default minimal value an HTLC must carry in order to be forwardable for new channels");
	opt_register_arg("--htlc-maximum-mbro", opt_set_mbro, NULL,
	...
	opt_register_arg("--max-dust-htlc-exposure-mbro", opt_set_mbro,
			 NULL, &ld->config.max_dust_htlc_exposure_mbro,
			 "Max HTLC amount that can be trimmed");
	opt_register_arg("--min-capacity-bro", opt_set_u64, opt_show_u64,
			 &ld->config.min_capacity_bro,
			 "Minimum capacity in bronees for accepting channels");
	...
	opt_register_arg("--force-feerates",
			 ...
			 "Set testnet/regtest feerates in bros perkw, opening/mutual_close/unlateral_close/delayed_to_us/htlc_resolution/penalty: if fewer specified, last number applies to remainder");
		       ...
		       const char *name, size_t len)
{
	...
	if (opt->type & OPT_NOARG) {
		...
		} else if (opt->cb_arg == (void *)opt_set_mbro) {
			json_add_amount_mbro_only(response, name0, ld->config.max_dust_htlc_exposure_mbro);
#if EXPERIMENTAL_FEATURES
