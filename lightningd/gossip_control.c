...
static void got_txout(struct brocoind *brocoind,
		      ...
		      struct short_channel_id *scid)
{
	...
	subd_send_msg(
	    brocoind->ld->gossip,
	    towire_gossipd_get_txout_reply(scid, scid, bro, script));
	tal_free(scid);
}

static void got_filteredblock(struct brocoind *brocoind,
		      const struct filteredblock *fb,
		      struct short_channel_id *scid)
{
	struct filteredblock_outpoint *fbo = NULL, *o;
	struct brocoin_tx_output txo;

	/* If we failed to the filtered block we report the failure to
	 * got_txout. */
	if (fb == NULL)
		return got_txout(brocoind, NULL, scid);

	/* Only fill in blocks that we are not going to scan later. */
	if (brocoind->ld->topology->max_blockheight > fb->height)
		wallet_filteredblock_add(brocoind->ld->wallet, fb);
	...
	if (fbo) {
		...
		got_txout(brocoind, &txo, scid);
	} else
		got_txout(brocoind, NULL, scid);
}
...
static void get_txout(struct subd *gossip, const u8 *msg)
{
	...
	if (op) {
		subd_send_msg(gossip,
				  ...
				  scid, scid, op->bro, op->scriptpubkey));
		tal_free(scid);
		...
		subd_send_msg(gossip, take(towire_gossipd_get_txout_reply(
						   NULL, scid, AMOUNT_BRO(0), NULL)));
		tal_free(scid);
	} else {
		brocoind_getfilteredblock(topo->brocoind, short_channel_id_blocknum(scid), got_filteredblock, scid);
	}
}
				       ...
				       const u8 *msg)
{
	...
	u32 fee_base_mbro, fee_proportional_millionths;
	struct amount_mbro htlc_maximum_mbro;
	...
	subd_send_msg(ld->gossip,
		      take(towire_gossipd_local_channel_update
			   (NULL,
			    ...
			    htlc_minimum_mbro,
			    fee_base_mbro,
			    fee_proportional_millionths, htlc_maximum_mbro)));
}
						  ...
						  const jsmntok_t *params)
{
	...
	struct amount_bro *lease_base_bro;
	struct amount_mbro *channel_fee_base_mbro;
	u32 *lease_basis, *channel_fee_max_ppt, *funding_weight;

	if (!param(cmd, buffer, params,
		   p_req("lease_fee_base_mbro", param_bro, &lease_base_bro),
		   p_req("lease_fee_basis", param_number, &lease_basis),
		   p_req("funding_weight", param_number, &funding_weight),
		   p_req("channel_fee_max_base_mbro", param_mbro,
			 &channel_fee_base_mbro),
	...
	rates->lease_fee_base_bro = lease_base_bro->bronees; /* Raw: conversion */
	rates->channel_fee_max_base_mbro = channel_fee_base_mbro->millibronees; /* Raw: conversion */
	...
	if (lease_base_bro->bronees > rates->lease_fee_base_bro) /* Raw: comparison */
		return command_fail_badparam(cmd, "lease_fee_base_mbro",
					     buffer, params, "Overflow");

	if (channel_fee_base_mbro->millibronees > rates->channel_fee_max_base_mbro) /* Raw: comparison */
	...
	json_add_amount_bro_only(res, "lease_fee_base_mbro",
				 amount_bro(rates->lease_fee_base_bro));
	json_add_num(res, "lease_fee_basis", rates->lease_fee_basis);
	json_add_num(res, "funding_weight", rates->funding_weight);
	json_add_amount_mbro_only(res, "channel_fee_max_base_mbro",
				  amount_mbro(rates->channel_fee_max_base_mbro));
