...
routehint_candidates(const tal_t *ctx,
		     ...
		     bool *none_public,
		     struct amount_mbro *avail_capacity,
		     struct amount_mbro *private_capacity,
		     struct amount_mbro *deadend_capacity,
		     struct amount_mbro *offline_capacity)
{
	...
	*none_public = true;
	*deadend_capacity = AMOUNT_MBRO(0);
	*offline_capacity = AMOUNT_MBRO(0);
	*avail_capacity = AMOUNT_MBRO(0);
	*private_capacity = AMOUNT_MBRO(0);

	/* We combine the JSON output which knows the peers' details,
	 * with our internal information */
	json_for_each_arr(i, t, arr) {
		struct amount_mbro capacity;
		const char *err;
		struct routehint_candidate candidate;
		struct amount_mbro fee_base, htlc_max;
		struct route_info *r;
		struct peer *peer;
		bool is_public;
		...
		/* FIXME: we don't actually check htlc_minimum_mbro! */

		/* If they set an htlc_maximum_mbro, consider that the
		...
		if (amount_mbro_greater(candidate.capacity, htlc_max))
		...
		r->fee_base_mbro = fee_base.millibronees; /* Raw: route_info */
		/* Could wrap: if so ignore */
		if (!amount_mbro_eq(amount_mbro(r->fee_base_mbro), fee_base)) {
			log_debug(ld->log,
		...
		} else if (amount_mbro_eq(capacity, AMOUNT_MBRO(0))) {
			log_debug(ld->log, "%s: deadend",
				  type_to_string(tmpctx,
						 struct short_channel_id,
						 &r->short_channel_id));
			if (!amount_mbro_add(deadend_capacity,
					     *deadend_capacity,
					     candidate.capacity))
		...
		if (candidate.c->owner == NULL) {
			log_debug(ld->log, "%s: offline",
				  type_to_string(tmpctx,
						 struct short_channel_id,
						 &r->short_channel_id));
			if (!amount_mbro_add(offline_capacity,
					     *offline_capacity,
					     candidate.capacity))
		...
		if (is_public) {
			...
			if (!amount_mbro_add(avail_capacity,
					     *avail_capacity,
					     candidate.capacity)) {
			...
			if (!amount_mbro_add(private_capacity,
					     *private_capacity,
					     candidate.capacity)) {
