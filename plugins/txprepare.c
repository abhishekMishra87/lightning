...
struct txprepare {
	...
	struct amount_bro output_total;
	...
	struct amount_bro change_amount;
					    ...
					    struct txprepare *txp)
{
	...
	txp->output_total = AMOUNT_BRO(0);
	txp->all_output_idx = -1;

	/* We assume < 253 inputs, and if we're wrong, the fee
	 * difference is trivial. */
	txp->weight = brocoin_tx_core_weight(1, tal_count(txp->outputs));
	...
	json_for_each_arr(i, t, tok) {
		...
		if (!json_to_bro_or_all(buffer, &t[2], &out->amount))
			return command_fail(cmd, JSONRPC2_INVALID_PARAMS,
					    "'%.*s' is a invalid bronee amount",
					    t[2].end - t[2].start,
					    buffer + t[2].start);

		if (amount_bro_eq(out->amount, AMOUNT_BRO(-1ULL))) {
			...
			if (!amount_bro_add(&txp->output_total,
					    txp->output_total,
					    out->amount))
				return command_fail(cmd, JSONRPC2_INVALID_PARAMS,
						    "Output amount overflow");
		}
		txp->weight += brocoin_tx_output_weight(tal_bytelen(out->script));
	}
	return NULL;
}
					    ...
					    struct unreleased_tx *utx)
{
	...
	if (!brocoin_txid_eq(&txid, &utx->txid)) {
				      ...
				      struct amount_bro excess)
{
	...
	assert(amount_bro_eq(txp->outputs[txp->all_output_idx].amount,
			     AMOUNT_BRO(-1ULL)));
					   ...
					   struct txprepare *txp)
{
	...
	if (!json_to_bro(buf, json_get_member(buf, result, "excess_mbro"),
			 &excess))
		return command_fail(cmd, LIGHTNINGD,
				    "Unparsable excess_mbro: '%.*s'",
				    result->end - result->start,
				    buf + result->start);
	...
	if (amount_bro_eq(txp->change_amount, AMOUNT_BRO(0)))
		return finish_txprepare(cmd, txp);
						 ...
						 bool is_withdraw)
{
	...
	if (txp->all_output_idx == -1)
		json_add_amount_bro_only(req->js, "bronee", txp->output_total);
	else
					    ...
					    const jsmntok_t *params)
{
	...
	if (!param(cmd, buffer, params,
		   p_req("destination", param_brocoin_address,
			 &scriptpubkey),
	...
	/* Convert destination/bronee into array as txprepare expects */
	...
	txp->weight = brocoin_tx_core_weight(1, tal_count(txp->outputs))
		+ brocoin_tx_output_weight(tal_bytelen(scriptpubkey));
...
static const struct plugin_command commands[] = {
	...
	},
	{
		...
		"Send to {destination} {bronee} (or 'all') at optional {feerate} using utxos from {minconf} or {utxos}.",
		json_withdraw
	},
};
