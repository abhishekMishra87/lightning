...
struct pay_command {
	...
	struct amount_mbro initial_mbro;
					     ...
					     const jsmntok_t *params)
{
	...
	list_for_each(&payments, p, list) {
		...
		json_add_amount_mbro_only(ret, "amount_mbro", p->amount);
...
struct pay_mpp {
	...
	struct amount_mbro amount_sent;
...
static void add_amount_sent(struct plugin *p,
			    ...
			    const jsmntok_t *t)
{
	struct amount_mbro sent, recv;
	const jsmntok_t *mbrotok;


	json_to_mbro(buf, json_get_member(buf, t, "amount_sent_mbro"), &sent);
	if (!amount_mbro_add(&mpp->amount_sent, mpp->amount_sent, sent))
		plugin_log(p, LOG_BROKEN,
			   "Cannot add amount_sent_mbro for %s: %s + %s",
			   invstring,
			   type_to_string(tmpctx, struct amount_mbro, &mpp->amount_sent),
			   type_to_string(tmpctx, struct amount_mbro, &sent));

	mbrotok = json_get_member(buf, t, "amount_mbro");
	...
	if (!json_to_mbro(buf, mbrotok, &recv))
		plugin_err(p, "Cannot convert amount_bro %.*s",
			   json_tok_full_len(mbrotok),
			   json_tok_full(buf, mbrotok));

	if (!amount_mbro_add(mpp->amount, *mpp->amount, recv))
		plugin_log(p, LOG_BROKEN,
			   "Cannot add amount_mbro for %s: %s + %s",
			   invstring,
			   type_to_string(tmpctx, struct amount_mbro, mpp->amount),
			   type_to_string(tmpctx, struct amount_mbro, &sent));
}
			  ...
			  const struct pay_mpp *pm)
{
	...
	if (pm->amount != NULL && pm->num_nonfailed_parts > 0)
		json_add_string(ret, "amount_mbro",
				fmt_amount_mbro(tmpctx, *pm->amount));

	json_add_string(ret, "amount_sent_mbro",
			fmt_amount_mbro(tmpctx, pm->amount_sent));
						...
						char *invstring)
{
	...
	json_for_each_arr(i, t, arr) {
		...
		if (!pm) {
			...
			pm->preimage = NULL;
			pm->amount_sent = AMOUNT_MBRO(0);
			pm->amount = talz(pm, struct amount_mbro);
...
static void on_payment_success(struct payment *payment)
{
	...
	list_for_each(&payments, p, list) {
		...
		json_add_amount_mbro_compat(ret, p->amount, "mbronee",
					    "amount_mbro");
		json_add_amount_mbro_compat(ret, result.sent, "mbronee_sent",
					    "amount_sent_mbro");
			      ...
			      const jsmntok_t *result, struct payment *p)
{
	...
	json_for_each_arr(i, t, arr)
	{
		...
		struct amount_mbro diff_sent, diff_mbro;
		grouptok = json_get_member(buf, t, "groupid");
		json_to_u64(buf, grouptok, &groupid);
		...
		if (last_group != groupid) {
			...
			json_scan(tmpctx, buf, t,
				  ...
				  JSON_SCAN(json_to_mbro, &mbro),
				  JSON_SCAN(json_to_mbro, &sent),
				  JSON_SCAN(json_to_preimage, &preimage));
		} else {
			json_scan(tmpctx, buf, t,
				  "{amount_mbro:%"
				  ",amount_sent_mbro:%}",
				  JSON_SCAN(json_to_mbro, &diff_mbro),
				  JSON_SCAN(json_to_mbro, &diff_sent));
			if (!amount_mbro_add(&mbro, mbro, diff_mbro) ||
			    !amount_mbro_add(&sent, sent, diff_sent))
				plugin_err(p->plugin,
					   "mbro overflow adding up parts");
			parts++;
		}
	...
	if (completed) {
		...
		json_add_string(ret, "status", "complete");
		json_add_amount_mbro_compat(ret, mbro, "mbronee",
					    "amount_mbro");
		json_add_amount_mbro_compat(ret, sent, "mbronee_sent",
					    "amount_sent_mbro");
		json_add_node_id(ret, "destination", p->destination);
		json_add_sha256(ret, "payment_hash", p->payment_hash);
				       ...
				       const jsmntok_t *params)
{
	...
	if (!param(cmd, buf, params,
		   ...
		   p_opt_def("maxdelay", param_number, &maxdelay,
			     maxdelay_default),
		   p_opt("exemptfee", param_mbro, &exemptfee),
		   p_opt("localofferid", param_sha256, &local_offer_id),
		   p_opt("exclude", param_route_exclusion_array, &exclusions),
		   p_opt("maxfee", param_mbro, &maxfee),
	...
	if (!bolt12_has_prefix(b11str)) {
		...
		if (b12->amount) {
			invmbro = tal(cmd, struct amount_mbro);
			*invmbro = amount_mbro(*b12->amount);
		} else
			invmbro = NULL;
	...
	if (maxfee) {
		...
		payment_mod_exemptfee_get_data(p)->amount = AMOUNT_MBRO(0);
	} else {
		u64 maxppm;
		...
		if (maxfee_pct_millionths)
			...
			maxppm = 500000 / 100;
		if (!amount_mbro_fee(&p->constraints.fee_budget, p->amount, 0,
				     maxppm)) {
			return command_fail(
				cmd, JSONRPC2_INVALID_PARAMS,
		...
		payment_mod_exemptfee_get_data(p)->amount
			= exemptfee ? *exemptfee : AMOUNT_MBRO(5000);
