...
struct test_case {
	...
	struct amount_bro exp_our_funds;
	bool expect_err;
};
...
struct test_case cases[] = {
	/* Straight fixed */
	{
		.their_funds = AMOUNT_BRO(5000),
		.available_funds = AMOUNT_BRO(100000),
		.channel_max = AMOUNT_BRO(11000),
		.lease_request = AMOUNT_BRO(0),
		.policy = {
			.opt = FIXED,
			.mod = 1111,
			.min_their_funding = AMOUNT_BRO(0),
			.max_their_funding = AMOUNT_BRO(10000),
			.per_channel_max = AMOUNT_BRO(10000),
			.per_channel_min = AMOUNT_BRO(0),
			.fuzz_factor = 0,
			.reserve_tank = AMOUNT_BRO(0),
			.fund_probability = 100,
			.leases_only = false,
		},

		.exp_our_funds = AMOUNT_BRO(1111),
		.expect_err = false,
	},
	/* Match 0 */
	{
		.their_funds = AMOUNT_BRO(5000),
		.available_funds = AMOUNT_BRO(500),
		.channel_max = AMOUNT_BRO(11000),
		.lease_request = AMOUNT_BRO(0),
		.policy = {
			.opt = MATCH,
			...
			.max_their_funding = AMOUNT_BRO(10000),
			.per_channel_max = AMOUNT_BRO(10000),
			.per_channel_min = AMOUNT_BRO(1000),
			.fuzz_factor = 0,
			.reserve_tank = AMOUNT_BRO(0),
			.fund_probability = 100,
			.leases_only = false,
		...
		.exp_our_funds = AMOUNT_BRO(0),
		.expect_err = false,
	},
	/* Match 100 */
	{
		.their_funds = AMOUNT_BRO(5000),
		.available_funds = AMOUNT_BRO(6000),
		.channel_max = AMOUNT_BRO(11000),
		.lease_request = AMOUNT_BRO(0),
		.policy = {
			.opt = MATCH,
		...
		.exp_our_funds = AMOUNT_BRO(5000),
		.expect_err = false,
	},
	/* Match 200 */
	{
		.their_funds = AMOUNT_BRO(2500),
		.available_funds = AMOUNT_BRO(6000),
		.channel_max = AMOUNT_BRO(11000),
		.lease_request = AMOUNT_BRO(0),
		.policy = {
	...
	{
		.their_funds = AMOUNT_BRO(2500),
		.available_funds = AMOUNT_BRO(5000),
		.channel_max = AMOUNT_BRO(11000),
		.lease_request = AMOUNT_BRO(0),
		.policy = {
			.opt = AVAILABLE,
	...
	{
		.their_funds = AMOUNT_BRO(2500),
		.available_funds = AMOUNT_BRO(3000),
		.channel_max = AMOUNT_BRO(11000),
		.lease_request = AMOUNT_BRO(0),
		.policy = {
			.opt = AVAILABLE,
	...
	/* Fixed above per-channel max*/
	{
		...
		.policy = {
			...
			.min_their_funding = AMOUNT_BRO(0),
			.max_their_funding = AMOUNT_BRO(10000),
			.per_channel_max = AMOUNT_BRO(900),
			.per_channel_min = AMOUNT_BRO(100),
			.fuzz_factor = 0,
			.reserve_tank = AMOUNT_BRO(0),
			.fund_probability = 100,
			.leases_only = false,
	...
	{
		.their_funds = AMOUNT_BRO(5000),
		.available_funds = AMOUNT_BRO(1000),
		.channel_max = AMOUNT_BRO(10000),
		.lease_request = AMOUNT_BRO(0),
		.policy = {
			.opt = FIXED,
			.mod = 999,
			.min_their_funding = AMOUNT_BRO(5001),
			.max_their_funding = AMOUNT_BRO(10000),
			.per_channel_max = AMOUNT_BRO(10000),
			.per_channel_min = AMOUNT_BRO(0),
			.fuzz_factor = 0,
	...
	/* Peer exceeds 'max_their_funding' */
	{
		...
		.policy = {
			...
			.mod = 998,
			.min_their_funding = AMOUNT_BRO(0),
			.max_their_funding = AMOUNT_BRO(500),
			.per_channel_max = AMOUNT_BRO(10000),
			.per_channel_min = AMOUNT_BRO(0),
			.fuzz_factor = 0,
			.reserve_tank = AMOUNT_BRO(0),
	...
	{
		.their_funds = AMOUNT_BRO(5000),
		.available_funds = AMOUNT_BRO(999),
		.channel_max = AMOUNT_BRO(10000),
		.lease_request = AMOUNT_BRO(0),
		.policy = {
			.opt = FIXED,
	...
	/* Fixed below per-channel min */
	{
		...
		.policy = {
			...
			.max_their_funding = AMOUNT_BRO(10000),
			.per_channel_max = AMOUNT_BRO(10000),
			.per_channel_min = AMOUNT_BRO(989),
			.fuzz_factor = 0,
			.reserve_tank = AMOUNT_BRO(0),
			.fund_probability = 100,
			.leases_only = false,
	...
	/* By default, use lease request as ceiling */
	{
		...
		.available_funds = AMOUNT_BRO(100000),
		.channel_max = AMOUNT_BRO(11000),
		.lease_request = AMOUNT_BRO(980),
		.policy = {
			.opt = MATCH,
			.mod = 100,
			.min_their_funding = AMOUNT_BRO(0),
			.max_their_funding = AMOUNT_BRO(10000),
			.per_channel_max = AMOUNT_BRO(100000),
			.per_channel_min = AMOUNT_BRO(0),
			.fuzz_factor = 0,
			.reserve_tank = AMOUNT_BRO(0),
			.fund_probability = 100,
...
static void check_fuzzing(struct test_case fuzzcase)
{
	...
	struct amount_bro fuzz_max = AMOUNT_BRO(0),
			  fuzz_min = AMOUNT_BRO(UINT_MAX);
	...
	for (size_t i = 0; i < 100; i++) {
		calculate_our_funding(&fuzzcase.policy, id,
				      ...
				      &our_funds);
		if (amount_bro_greater(our_funds, fuzz_max))
			fuzz_max = our_funds;
		if (amount_bro_less(our_funds, fuzz_min))
			fuzz_min = our_funds;
	}

	assert(fuzz_max.bronees <= fuzzcase.policy.mod + fuzz_amt);
	assert(fuzz_min.bronees >= fuzzcase.policy.mod - fuzz_amt);
}
...
int main(int argc, const char *argv[])
{
	struct funder_policy *policy;
	struct node_id id;
	struct amount_bro empty = AMOUNT_BRO(0), our_funds;
	bool ok = true;
	size_t i = 0, flips = 0;
	...
	/* Check the default funder policy, at fixed (0mbro) */
	...
	err = calculate_our_funding(policy, id,
				    ...
				    AMOUNT_BRO(100000),
				    &our_funds);
	assert(amount_bro_eq(empty, our_funds));
	assert(!err);
	...
	for (i = 0; i < ARRAY_SIZE(cases); i++) {
		err = calculate_our_funding(&cases[i].policy, id,
					    ...
					    &our_funds);
		if (!amount_bro_eq(cases[i].exp_our_funds, our_funds)) {
			fprintf(stderr, "FAIL policy: %s. expected %s, got %s\n",
				funder_policy_desc(NULL, &cases[i].policy),
				type_to_string(NULL, struct amount_bro,
	...
	for (i = 0; i < 100 * flips; i++) {
		calculate_our_funding(&flipcase.policy, id,
				      ...
				      &our_funds);
		if (!amount_bro_eq(our_funds, AMOUNT_BRO(0)))
			flipcount++;
	}
