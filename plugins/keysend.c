					   ...
					   const jsmntok_t *params)
{
	struct payment *p;
	const char *label;
	struct amount_mbro *exemptfee, *mbro;
	...
	if (!param(cmd, buf, params,
		   ...
		   p_opt_def("maxdelay", param_number, &maxdelay,
			     maxdelay_default),
		   p_opt_def("exemptfee", param_mbro, &exemptfee, AMOUNT_MBRO(5000)),
#if DEVELOPER
			      ...
			      struct keysend_in *ki)
{
	...
	ki->payload->payment_data->total_mbro = *ki->payload->amt_to_forward;
	...
	towire_tu64(&field.value, ki->payload->payment_data->total_mbro);
						 ...
						 const jsmntok_t *params)
{
	...
	json_add_string(req->js, "mbronee", "any");
