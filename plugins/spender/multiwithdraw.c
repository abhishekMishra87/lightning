...
const struct plugin_command multiwithdraw_commands[] = {
	{
		"multiwithdraw",
		"brocoin",
		"Send to multiple {outputs} via a single Brocoin transaction.",
		    ...
		    struct multiwithdraw_destination **outputs)
{
	...
	*outputs = tal_arr(cmd, struct multiwithdraw_destination, t->size);
	json_for_each_arr (i, e, t) {
		...
		if (!json_to_bro_or_all(buf, &e[2], &dest->amount))
			return command_fail(cmd, JSONRPC2_INVALID_PARAMS,
		...
		dest->all = amount_bro_eq(dest->amount, AMOUNT_BRO(-1ULL));
		   ...
		   const jsmntok_t *params)
{
	...
	for (size_t i = 0; i < tal_count(mw->outputs); ++i)
		if (mw->outputs[i].all)
			mw->has_all = true;
		else if (amount_bro_less(mw->outputs[i].amount,
...
static struct command_result *start_mw(struct multiwithdraw_command *mw)
{
	...
	startweight = brocoin_tx_core_weight(1, tal_count(mw->outputs));
	for (size_t i = 0; i < tal_count(mw->outputs); ++i) {
		struct multiwithdraw_destination *dest;
		dest = &mw->outputs[i];
		startweight += brocoin_tx_output_weight(
	...
	if (mw->has_all)
		...
		for (size_t i = 0; i < tal_count(mw->outputs); ++i)
			if (!amount_bro_add(&sum, sum, mw->outputs[i].amount))
				return mw_fail(mw,
					       FUND_CANNOT_AFFORD,
		  ...
		  struct multiwithdraw_command *mw)
{
	...
	field = ok ? json_get_member(buf, result, "excess_mbro") : NULL;
	ok = ok && field;
	ok = ok && json_to_bro(buf, field, &excess_bro);
	...
	if (mw->has_all) {
		...
		for (size_t i = 0; i < tal_count(mw->outputs); ++i) {
			...
			if (!amount_bro_sub(&excess_bro, excess_bro,
					    mw->outputs[i].amount))
				return mw_fail(mw,
					       FUND_CANNOT_AFFORD,
					       "Insufficient funds.");
		...
		if (amount_bro_less(excess_bro, chainparams->dust_limit))
		...
		mw->outputs[all_index].amount = excess_bro;
		excess_bro = AMOUNT_BRO(0);
	}

	/* Handle any change output.  */
	mw->change_amount = change_amount(excess_bro, feerate_per_kw,
					  estimated_final_weight);
	mw->change_needed = !amount_bro_eq(mw->change_amount, AMOUNT_BRO(0));
...
mw_load_outputs(struct multiwithdraw_command *mw)
{
	...
	if (chainparams->is_elements) {
		...
		if (!amount_bro_eq(sum, AMOUNT_BRO(0)))
