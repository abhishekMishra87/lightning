	       ...
	       struct multifundchannel_command *mfc)
{
	...
	1.  `sendpsbt` launches `brocoin-cli`.
	2.  `brocoin-cli` connects to a `brocoind` over JSON-RPC
	    over HTTP.
	3.  `brocoind` validates the transactions and puts it int
	    its local mempool.
	4.  `brocoind` tells `brocoin-cli` it all went well.
	5.  `brocoin-cli` tells `sendpsbt` it all went well.
...
perform_funding_tx_finalize(struct multifundchannel_command *mfc)
{
	...
	/* Generate the TXID.  */
	mfc->txid = tal(mfc, struct brocoin_txid);
...
fundchannel_start_dest(struct multifundchannel_destination *dest)
{
	...
	json_add_string(req->js, "amount",
			fmt_amount_bro(tmpctx, dest->amount));
	...
	json_add_string(req->js, "push_mbro",
			fmt_amount_mbro(tmpctx, dest->push_mbro));
...
handle_mfc_change(struct multifundchannel_command *mfc)
{
	...
	struct amount_bro change_fee, fee_paid, total_fee;
	struct amount_bro change_min_limit;

	/* Determine if adding a change output is worth it.
	 * Get the weight of a change output and how much it
	 * costs.
	 */
	change_weight = brocoin_tx_output_weight(
				BROCOIN_SCRIPTPUBKEY_P2WPKH_LEN);
	...
	if (!amount_bro_sub(&change_fee, total_fee, fee_paid))
		abort();

	/* The limit is equal to the change_fee plus the dust limit.  */
	if (!amount_bro_add(&change_min_limit,
			    change_fee, chainparams->dust_limit))
		plugin_err(mfc->cmd->plugin,
			   "Overflow dust limit and change fee.");

	/* Is the excess over the limit?  */
	if (amount_bro_greater(mfc->excess_bro, change_min_limit)) {
		bool ok = amount_bro_sub(&mfc->change_amount,
					 mfc->excess_bro, change_fee);
		assert(ok);
...
compute_mfc_all(struct multifundchannel_command *mfc)
{
	...
	for (size_t i = 0; i < tal_count(mfc->destinations); ++i) {
		...
		if (!amount_bro_sub(&mfc->excess_bro,
				    mfc->excess_bro, dest->amount))
			/* Not enough funds!  */
	...
	if (amount_bro_less(mfc->excess_bro, chainparams->dust_limit))
	       ...
	       struct multifundchannel_command *mfc)
{
	...
	/* mbro LOL.  */
	field = json_get_member(buf, result, "excess_mbro");
	if (!field || !parse_amount_bro(&mfc->excess_bro,
					buf + field->start,
					field->end - field->start))
		goto fail;
...
perform_fundpsbt(struct multifundchannel_command *mfc, u32 feerate)
{
	...
	if (has_all(mfc))
		json_add_string(req->js, "bronee", "all");
	else {
		struct amount_bro sum = AMOUNT_BRO(0);
		for (size_t i = 0; i < tal_count(mfc->destinations); ++i) {
			struct amount_bro requested
				= mfc->destinations[i].request_amt;

			if (!amount_bro_add(&sum,
...

	{
		 ...
		 */
		startweight = brocoin_tx_core_weight(1, num_outs)
			    + ( brocoin_tx_output_weight(
					BROCOIN_SCRIPTPUBKEY_P2WSH_LEN)
			      * num_outs
			      );
	...
	*dests = tal_arr(cmd, struct multifundchannel_destination, tok->size);
	...
	json_for_each_arr(i, json_dest, tok) {
		...
		if (!param(cmd, buffer, json_dest,
			   ...
			   p_req("amount", param_bro_or_all, &amount),
			   p_opt_def("announce", param_bool, &announce, true),
			   p_opt_def("push_mbro", param_mbro, &push_mbro,
				     AMOUNT_MBRO(0)),
			   ...
			   p_opt("close_to", param_string,
				 &dest->close_to_str),
			   p_opt_def("request_amt", param_bro, &request_amt,
				     AMOUNT_BRO(0)),
		...
		if (!amount_bro_eq(*amount, AMOUNT_BRO(-1ULL)) &&
		    amount_bro_less(*amount, chainparams->dust_limit))
			return command_fail_badparam(cmd, name, buffer,
						     json_dest,
		...
		dest->all = amount_bro_eq(*amount, AMOUNT_BRO(-1ULL));
		dest->amount = dest->all ? AMOUNT_BRO(0) : *amount;
		dest->announce = *announce;
		dest->push_mbro = *push_mbro;
		dest->psbt = NULL;
