...
struct payment_tree_result payment_collect_result(struct payment *p)
{
	...
	for (size_t i = 0; i < numchildren; i++) {
		 ...
		 * we sent in total. */
		if (!amount_mbro_add(&res.sent, res.sent, cres.sent))
			paymod_err(
			    p,
...
static void channel_hints_update(struct payment *p,
				 ...
				 const struct amount_mbro *estimated_capacity,
				 u16 *htlc_budget)
{
	struct payment *root = payment_root(p);
	...
	for (size_t i=0; i<tal_count(root->channel_hints); i++) {
		...
		if (short_channel_id_eq(&hint->scid.scid, &scid) &&
			...
			if (estimated_capacity != NULL &&
			    amount_mbro_greater(hint->estimated_capacity,
						*estimated_capacity)) {
...
static void payment_exclude_most_expensive(struct payment *p)
{
	...
	struct amount_mbro fee, worst = AMOUNT_MBRO(0);

	for (size_t i = 0; i < tal_count(p->route)-1; i++) {
		if (!amount_mbro_sub(&fee, p->route[i].amount, p->route[i+1].amount))
			paymod_err(p, "Negative fee in a route.");

		if (amount_mbro_greater_eq(fee, worst)) {
			e = &p->route[i];
			worst = fee;
		}
	}
...
static struct amount_mbro payment_route_fee(struct payment *p)
{
	struct amount_mbro fee;
	if (!amount_mbro_sub(&fee, p->route[0].amount, p->amount)) {
		paymod_log(
		    p,
		    LOG_BROKEN,
		    ...
		    type_to_string(tmpctx, struct amount_mbro,
				   &p->route[0].amount),
		    type_to_string(tmpctx, struct amount_mbro, &p->amount));
		abort();
	}
	return fee;
}
...
payment_constraints_update(struct payment_constraints *cons,
			   const struct amount_mbro delta_fee,
			   const u32 delta_cltv)
{
	if (delta_cltv > cons->cltv_budget)
		return false;

	/* amount_mbro_sub performs a check before actually subtracting. */
	if (!amount_mbro_sub(&cons->fee_budget, cons->fee_budget, delta_fee))
		return false;
...
static bool payment_chanhints_apply_route(struct payment *p, bool remove)
{
	...
	for (size_t i = 0; i < tal_count(p->route); i++) {
		...
		apply &= amount_mbro_greater(curhint->estimated_capacity,
					     curhop->amount);
...
payment_get_excluded_channels(const tal_t *ctx, struct payment *p)
{
	...
	for (size_t i = 0; i < tal_count(root->channel_hints); i++) {
		...
		else if (amount_mbro_greater_eq(p->amount,
						hint->estimated_capacity))
				...
				struct payment *p)
{
	...
	if (amount_mbro_greater_eq(amount, hint->estimated_capacity))
		/* We exclude on equality because we've set the
		 * estimate to the smallest failed attempt. */
		return false;
			 ...
			 struct amount_mbro amount)
{
	...
	u64 capmbro, amtmbro = amount.millibronees; /* Raw: lengthy math */

	/* Can fail in theory if gossmap changed underneath. */
	if (!gossmap_chan_get_capacity(map, c, &capacity))
		return 0;

	capmbro = capacity.bronees * 1000; /* Raw: lengthy math */
	return -log((capmbro + 1 - amtmbro) / (capmbro + 1));
}

/* Prioritize costs over distance, but bias to larger channels. */
static u64 route_score(u32 distance,
		       struct amount_mbro cost,
		       struct amount_mbro risk,
		       int dir,
		       const struct gossmap_chan *c)
{
	u64 cmbro = cost.millibronees; /* Raw: lengthy math */
	u64 rmbro = risk.millibronees; /* Raw: lengthy math */
	u64 bias = capacity_bias(global_gossmap, c, dir, cost);

	/* Smoothed harmonic mean to avoid division by 0 */
	u64 costs = (cmbro * rmbro * bias) / (cmbro + rmbro + bias + 1);
						    ...
						    const jsmntok_t *toks)
{
	...
	const jsmntok_t *senttok = json_get_member(buffer, toks, "amount_sent_mbro");
	...
	if (codetok != NULL && datatok != NULL) {
		...
		senttok = json_get_member(buffer, datatok, "amount_sent_mbro");
	...
	json_to_mbro(buffer, senttok, &result->amount_sent);
					   ...
					   const u8 *onion_message)
{
	...
	struct amount_mbro unused_mbro;
	u32 unused32;
							  ...
							  struct payment *p)
{
	...
	json_add_amount_mbro_only(req->js, "amount_mbro", first->amount);
	json_add_num(req->js, "delay", first->delay);
	json_add_node_id(req->js, "id", &first->node_id);
	json_object_end(req->js);

	json_add_sha256(req->js, "payment_hash", p->payment_hash);
	json_add_amount_mbro_only(req->js, "mbronee", p->amount);
					  ...
					  const u8 *payment_metadata)
{
	...
	u64 mbro = next->amount.millibronees; /* Raw: TLV payload generation*/
	...
	if (payment_secret != NULL) {
		...
		tlvstream_set_tlv_payload_data(
			...
			root->amount.millibronees); /* Raw: TLV payload generation*/
	}
...
static void payment_add_attempt(struct json_stream *s, const char *fieldname, struct payment *p, bool recurse)
{
	...
	json_add_amount_mbro_only(s, "amount", p->amount);
	if (p->parent != NULL)
...
static void payment_finished(struct payment *p)
{
	...
	if (p->parent == NULL) {
		...
		if (cmd == NULL) {
			...
			json_add_amount_mbro_compat(ret, p->amount, "mbronee",
						    "amount_mbro");
			json_add_amount_mbro_compat(ret, result.sent,
						    "mbronee_sent",
						    "amount_sent_mbro");
...
local_channel_hints_listpeers(struct command *cmd, const char *buffer,
			      const jsmntok_t *toks, struct payment *p)
{
	const jsmntok_t *peers, *peer, *channels, *channel, *spendbros, *scid,
	...
	json_for_each_arr(i, peer, peers) {
		...
		json_for_each_arr(j, channel, channels) {
			...
			spendbros = json_get_member(buffer, channel, "spendable_mbro");
			...
			if (spendbros == NULL || scid == NULL || dir == NULL ||
			...
			json_to_mbro(buffer, spendbros, &h.estimated_capacity);
					     ...
					     struct route_info **hints)
{
	...
	for (size_t i = 0; i < tal_count(hints) && src != NULL; i++) {
		...
		distance = dijkstra_distance(
		    dijkstra(tmpctx, map, entrynode, AMOUNT_MBRO(1), 1,
...
static bool route_mbronee(struct amount_mbro *total,
			       ...
			       const struct route_info *routehint)
{
	...
	for (size_t i = 0; i < tal_count(routehint); i++) {
		...
		struct amount_mbro needed_capacity;
		if (!route_mbronee(&needed_capacity, p->amount,
		...
		for (size_t j = 0; j < tal_count(hints); j++) {
			...
			if (amount_mbro_greater_eq(needed_capacity,
...
/* Calculate how many millibronee we need at the start of this route
 * to get mbronee to the end. */
static bool route_mbronee(struct amount_mbro *total,
			   const struct amount_mbro mbro,
			   const struct route_info *route, size_t num_route)
{
	*total = mbro;
	for (ssize_t i = num_route - 1; i >= 0; i--) {
		if (!amount_mbro_add_fee(total,
					 route[i].fee_base_mbro,
...
/* Change the destination and compute the final mbronee amount to send to the
...
static void routehint_pre_getroute(struct routehints_data *d, struct payment *p)
{
	...
	if (d->current_routehint != NULL) {
		if (!route_mbronee(&p->getroute->amount, p->amount,
				    d->current_routehint,
...
static void routehint_check_reachable(struct payment *p)
{
	...
	if (dst == NULL)
		...
		dij = dijkstra(tmpctx, gossmap, dst, AMOUNT_MBRO(1000),
			       10 / 1000000.0,
			       payment_route_can_carry_even_disabled,
			       route_score_cheaper, p);
...
static void routehint_step_cb(struct routehints_data *d, struct payment *p)
{
	...
	if (p->step == PAYMENT_STEP_INITIALIZED) {
		...
		struct amount_mbro dest_amount;
		...
		for (ssize_t i = 0; i < tal_count(routehint); i++) {
			...
			if (!route_mbronee(&dest_amount, p->amount,
				    routehint + i + 1,
...
static void exemptfee_cb(struct exemptfee_data *d, struct payment *p)
{
	...
	if (amount_mbro_greater_eq(d->amount, p->constraints.fee_budget)) {
		paymod_log(
		    p, LOG_INFORM,
		    "Payment fee constraint %s is below exemption threshold, "
		    "allowing a maximum fee of %s",
		    type_to_string(tmpctx, struct amount_mbro, &p->constraints.fee_budget),
		    type_to_string(tmpctx, struct amount_mbro, &d->amount));
					       ...
					       struct payment *p)
{
	...
	size_t i;
	struct amount_mbro best_fee;
	const jsmntok_t *brotok, *delaytok, *basefeetok, *propfeetok, *desttok,
	...
	assert(amount_mbro_greater_eq(p->constraints.fee_budget,
	...
	json_for_each_arr(i, chan, channelstok) {
		...
		brotok = json_get_member(buf, chan, "bronees");
		delaytok = json_get_member(buf, chan, "delay");
		basefeetok = json_get_member(buf, chan, "base_fee_millibronee");
		...
		if (!amount_mbro_fee(
			    &fee, p->amount, curr.fee_base_mbro,
			    curr.fee_proportional_millionths)) {
			/* Fee computation failed... */
			continue;
		}

		if (amount_mbro_greater_eq(fee, cons->fee_budget))
			continue;
	...
	if (best != NULL) {
		...
		if (d->fuzz_amount &&
		    (amount_mbro_greater(best_fee, d->constraints.fee_budget) ||
		     (amount_mbro_greater(best_fee,
					  p->constraints.fee_budget)))) {
			best = NULL;
			goto next;
		}
		...
		paymod_log(
		    ...
		    type_to_string(tmpctx, struct amount_mbro, &best_fee),
		...
		if (!amount_mbro_sub(&d->constraints.fee_budget,
				     d->constraints.fee_budget, best_fee) ||
		    !amount_mbro_sub(&p->constraints.fee_budget,
					      ...
					      struct payment *p)
{
	...
	d->constraints.fee_budget
		= amount_mbro_div(p->constraints.fee_budget, 4);
...
static void direct_pay_override(struct payment *p) {
	...
	if (hint && hint->enabled &&
	    amount_mbro_greater(hint->estimated_capacity, p->amount)) {
 ...
 * we found that the 80th percentile of capacities is >= 9765 bros.
 *
 * Rounding to 10e6 mbros per part there is a ~80% chance that the payment
...
static struct amount_mbro fuzzed_near(struct amount_mbro target,
				      struct amount_mbro max)
{
	s64 fuzz;
	struct amount_mbro res = target;

	/* Somewhere within 25% of target please. */
	fuzz = pseudorand(target.millibronees / 2) /* Raw: fuzz */
		- target.millibronees / 4; /* Raw: fuzz */
	res.millibronees = target.millibronees + fuzz; /* Raw: fuzz < mbro */

	if (amount_mbro_greater(res, max))
		res = max;
	return res;
}
...
static void presplit_cb(struct presplit_mod_data *d, struct payment *p)
{
	...
	if (p->step == PAYMENT_STEP_ONION_PAYLOAD) {
		 ...
		 * number of parts. */
		while (amount_mbro_less(amount_mbro(target_amount * PRESPLIT_MAX_SPLITS),
					p->amount))
		...
		int targethtlcs =
		    p->amount.millibronees / target_amount; /* Raw: division */
		if (htlcs == 0) {
			p->abort = true;
			return payment_fail(
			...
			target = amount_mbro(target_amount);

		/* If we are already below the target size don't split it
		 * either. */
		if (amount_mbro_greater(target, p->amount))
			return payment_continue(p);
		...
		while (!amount_mbro_eq(amt, AMOUNT_MBRO(0))) {
			double multiplier;
			...
			if (!amount_mbro_sub(&amt, amt, c->amount))
				paymod_err(
				    p,
			 ...
			 * when splitting. */
			multiplier = amount_mbro_ratio(c->amount, p->amount);
			if (!amount_mbro_scale(&c->constraints.fee_budget,
		...
		p->why = tal_fmt(
		    ...
		    count,
		    type_to_string(tmpctx, struct amount_mbro, &root->amount),
		    type_to_string(tmpctx, struct amount_mbro, &target));
...
#define MPP_ADAPTIVE_LOWER_LIMIT AMOUNT_MBRO(100 * 1000)
...
static void adaptive_splitter_cb(struct adaptive_split_mod_data *d, struct payment *p)
{
	...
	if (p->step == PAYMENT_STEP_ONION_PAYLOAD) {
		...
		if (amount_mbro_greater(p->amount, MPP_ADAPTIVE_LOWER_LIMIT)) {
			struct payment *a, *b;
			/* Random number in the range [90%, 110%] */
			double rand = pseudorand_double() * 0.2 + 0.9;
			u64 mid = p->amount.millibronees / 2 * rand; /* Raw: multiplication */
			bool ok;
			...
			a->amount.millibronees = mid;  /* Raw: split. */
			b->amount.millibronees -= mid; /* Raw: split. */

			double multiplier = amount_mbro_ratio(a->amount,
							      p->amount);
			assert(multiplier >= 0.4 && multiplier < 0.6);

			/* Adjust constraints since we don't want to double our
			 * fee allowance when we split. */
			if (!amount_mbro_scale(&a->constraints.fee_budget,
					       pconstraints->fee_budget,
					       multiplier))
				abort();

			ok = amount_mbro_sub(&b->constraints.fee_budget,
					     pconstraints->fee_budget,
