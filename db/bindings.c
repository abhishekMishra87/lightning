...
void db_bind_txid(struct db_stmt *stmt, int pos, const struct brocoin_txid *t)
{
...
void db_bind_amount_mbro(struct db_stmt *stmt, int pos,
			 const struct amount_mbro *mbro)
{
	db_bind_u64(stmt, pos, mbro->millibronees); /* Raw: low level function */
}

void db_bind_amount_bro(struct db_stmt *stmt, int pos,
			 const struct amount_bro *bro)
{
	db_bind_u64(stmt, pos, bro->bronees); /* Raw: low level function */
}
...
struct brocoin_tx *db_col_tx(const tal_t *ctx, struct db_stmt *stmt, const char *colname)
{
	size_t col = db_query_colnum(stmt, colname);
	...
	db_column_null_warn(stmt, colname, col);
	return pull_brocoin_tx(ctx, &src, &len);
}
...
struct brocoin_tx *db_col_psbt_to_tx(const tal_t *ctx, struct db_stmt *stmt, const char *colname)
{
	struct wally_psbt *psbt = db_col_psbt(ctx, stmt, colname);
	if (!psbt)
		return NULL;
	return brocoin_tx_with_psbt(ctx, psbt);
}
...
void db_col_amount_mbro_or_default(struct db_stmt *stmt,
				   const char *colname,
				   struct amount_mbro *mbro,
				   struct amount_mbro def)
{
	size_t col = db_query_colnum(stmt, colname);
	...
	if (db_column_is_null(stmt, col))
		*mbro = def;
		...
		mbro->millibronees = db_col_u64(stmt, colname); /* Raw: low level function */
}

void db_col_amount_mbro(struct db_stmt *stmt, const char *colname,
			struct amount_mbro *mbro)
{
	mbro->millibronees = db_col_u64(stmt, colname); /* Raw: low level function */
}

void db_col_amount_bro(struct db_stmt *stmt, const char *colname, struct amount_bro *bro)
{
	bro->bronees = db_col_u64(stmt, colname); /* Raw: low level function */
}
...
void db_col_txid(struct db_stmt *stmt, const char *colname, struct brocoin_txid *t)
{
