...
static struct amount_bro dust_limit;
...
static struct amount_mbro our_mbro;
...
struct tracked_output {
	...
	const struct brocoin_signature *remote_htlc_sig;
...
static void trim_maximum_feerate(struct amount_bro funding,
				 const struct tx_parts *commitment)
{
	size_t weight;
	struct amount_bro fee = funding;

	/* FIXME: This doesn't work for elements? */
	if (chainparams->is_elements)
		return;

	weight = brocoin_tx_core_weight(tal_count(commitment->inputs),
	...
	weight += brocoin_tx_input_weight(false, 1 + 1 + 2 * brocoin_tx_input_sig_weight());

	for (size_t i = 0; i < tal_count(commitment->outputs); i++) {
		struct amount_asset amt;
		weight += brocoin_tx_output_weight(commitment->outputs[i]
						   ->script_len);

		amt = wally_tx_output_get_amount(commitment->outputs[i]);
		if (!amount_asset_is_main(&amt))
			continue;
		if (!amount_bro_sub(&fee, fee, amount_asset_to_bro(&amt))) {
	...
	while (amount_bro_greater(amount_tx_fee(max_possible_feerate, weight),
				  fee))
		max_possible_feerate--;
...
static void record_channel_withdrawal(const struct brocoin_txid *tx_txid,
				      struct tracked_output *out,
				      u32 blockheight,
				      enum mvt_tag tag)
{
	send_coin_mvt(take(new_onchaind_withdraw(NULL, &out->outpoint, tx_txid,
						 blockheight, out->bro, tag)));
}

static void record_external_spend(const struct brocoin_txid *txid,
				  struct tracked_output *out,
				  u32 blockheight,
				  enum mvt_tag tag)
{
...
static void record_external_output(const struct brocoin_outpoint *out,
...
static void record_external_deposit(const struct tracked_output *out,
				    ...
				    enum mvt_tag tag)
{
	record_external_output(&out->outpoint, out->bro, blockheight, tag);
}
				...
				u32 blockheight)
{
	...
	for (size_t i = 0; i < tal_count(tx->outputs); i++) {
		...
		record_external_output(&out,
				       amount_bro(tx->outputs[i]->bronee),
				       blockheight,
				       TO_THEM);
		break;
	}
...
static void record_ignored_wallet_deposit(struct tracked_output *out)
{
	...
	brocoin_txid(out->proposal->tx, &outpoint.txid);
	outpoint.n = 0;
...
static bool grind_htlc_tx_fee(struct amount_bro *fee,
			      struct brocoin_tx *tx,
			      const struct brocoin_signature *remotesig,
			      const u8 *wscript,
			      u64 weight)
{
	struct amount_bro prev_fee = AMOUNT_BRO(UINT64_MAX), input_amt;
	...
	for (u64 i = min_possible_feerate; i <= max_possible_feerate; i++) {
		 ...
		 *        and divide by 1000 (rounding down).
		 */
		struct amount_bro out;

		*fee = amount_tx_fee(i, weight);

		/* Minor optimization: don't check same fee twice */
		if (amount_bro_eq(*fee, prev_fee))
			continue;

		prev_fee = *fee;
		if (!amount_bro_sub(&out, input_amt, *fee))
			break;

		brocoin_tx_output_set_amount(tx, 0, out);
...
static bool set_htlc_timeout_fee(struct brocoin_tx *tx,
				 const struct brocoin_signature *remotesig,
				 const u8 *wscript)
{
	static struct amount_bro amount, fee = AMOUNT_BRO_INIT(UINT64_MAX);
	struct amount_asset asset = brocoin_tx_output_get_amount(tx, 0);
	size_t weight;
	...
	amount = amount_asset_to_bro(&asset);

	if (amount_bro_eq(fee, AMOUNT_BRO(UINT64_MAX))) {
		struct amount_bro grindfee;
	...
	if (!amount_bro_sub(&amount, amount, fee))
		status_failed(STATUS_FAIL_INTERNAL_ERROR,
			      "Cannot deduct htlc-timeout fee %s from tx %s",
			      type_to_string(tmpctx, struct amount_bro, &fee),
			      type_to_string(tmpctx, struct brocoin_tx, tx));

	brocoin_tx_output_set_amount(tx, 0, amount);
	brocoin_tx_finalize(tx);
	return check_tx_sig(tx, 0, NULL, wscript,
			    &keyset->other_htlc_key, remotesig);
}

static void set_htlc_success_fee(struct brocoin_tx *tx,
				 const struct brocoin_signature *remotesig,
				 const u8 *wscript)
{
	static struct amount_bro amt, fee = AMOUNT_BRO_INIT(UINT64_MAX);
	...
	if (amount_bro_eq(fee, AMOUNT_BRO(UINT64_MAX))) {
		if (!grind_htlc_tx_fee(&fee, tx, remotesig, wscript, weight))
			status_failed(STATUS_FAIL_INTERNAL_ERROR,
				      ...
				      type_to_string(tmpctx, struct brocoin_tx,
						     tx),
				      weight,
	...
	asset = brocoin_tx_output_get_amount(tx, 0);
	assert(amount_asset_is_main(&asset));
	amt = amount_asset_to_bro(&asset);

	if (!amount_bro_sub(&amt, amt, fee))
		status_failed(STATUS_FAIL_INTERNAL_ERROR,
			      "Cannot deduct htlc-success fee %s from tx %s",
			      type_to_string(tmpctx, struct amount_bro, &fee),
			      type_to_string(tmpctx, struct brocoin_tx, tx));
	brocoin_tx_output_set_amount(tx, 0, amt);
	brocoin_tx_finalize(tx);
	...
	status_failed(STATUS_FAIL_INTERNAL_ERROR,
		      ...
		      type_to_string(tmpctx, struct amount_bro, &fee),
		      type_to_string(tmpctx, struct brocoin_tx, tx),
		      type_to_string(tmpctx, struct brocoin_signature, remotesig),
		      tal_hex(tmpctx, wscript));
}
 ...
 */
static struct brocoin_tx *tx_to_us(const tal_t *ctx,
...
{
	struct brocoin_tx *tx;
	struct amount_bro fee, min_out, amt;
	struct brocoin_signature sig;
	size_t weight;
	u8 *msg;
	u8 **witness;

	tx = brocoin_tx(ctx, chainparams, 1, 1, locktime);
	brocoin_tx_add_input(tx, &out->outpoint, to_self_delay,
			     NULL, out->bro, NULL, wscript);

	brocoin_tx_add_output(
	    tx, scriptpubkey_p2wpkh(tmpctx, &our_wallet_pubkey), NULL, out->bro);
	psbt_add_keypath_to_last_output(tx, our_wallet_index, &our_wallet_ext_key);

	/* Worst-case sig is 73 bytes */
	weight = brocoin_tx_weight(tx) + 1 + 3 + 73 + 0 + tal_count(wscript);
	weight += elements_tx_overhead(chainparams, 1, 1);
	...
	witness = brocoin_witness_sig_and_element(tx, &sig, elem,
						  elemsize, wscript);
	brocoin_tx_input_set_witness(tx, 0, take(witness));
	return tx;
}
 ...
 */
static struct brocoin_tx *
replace_penalty_tx_to_us(const tal_t *ctx,
			 u8 *(*hsm_sign_msg)(const tal_t *ctx,
					     struct brocoin_tx *tx,
					     const u8 *wscript),
			 const struct brocoin_tx *penalty_tx,
			 struct amount_bro *output_amount)
{
	struct brocoin_tx *tx;
	...
	struct brocoin_outpoint input_outpoint;
	u8 *input_wscript;
	u8 *input_element;
	struct amount_bro input_amount;
	...
	brocoin_tx_input_get_txid(penalty_tx, 0, &input_outpoint.txid);
	...
	tx = brocoin_tx(ctx, chainparams, 1, 1, /*locktime*/ 0);
	/* Reconstruct the input.  */
	brocoin_tx_add_input(tx, &input_outpoint,
			     BROCOIN_TX_RBF_SEQUENCE,
			     NULL, input_amount, NULL, input_wscript);
	/* Reconstruct the output with a smaller amount.  */
	if (amount_bro_greater(*output_amount, dust_limit)) {
		brocoin_tx_add_output(tx,
	...
	witness = brocoin_witness_sig_and_element(tx, &sig,
						  input_element,
 ...
 */
static struct amount_bro min_rbf_bump(size_t weight,
				      size_t index)
{
	struct amount_bro min_relay_fee;
	struct amount_bro min_rbf_bump;
	 ...
	 */
	if (mul_overflows_u64(index, min_relay_fee.bronees)) /* Raw: multiplication.  */
		min_rbf_bump = AMOUNT_BRO(UINT64_MAX);
	else
		min_rbf_bump.bronees = index * min_relay_fee.bronees; /* Raw: multiplication.  */
...
compute_penalty_output_amount(struct amount_bro initial_amount,
			      u32 depth, u32 max_depth,
			      size_t weight)
{
	struct amount_bro max_output_amount;
	struct amount_bro output_amount;
	struct amount_bro deducted_amount;
	 ...
	 */
	if (!amount_bro_sub(&max_output_amount,
	...
	deducted_amount.bronees = (u64) (y * initial_amount.bronees); /* Raw: multiplication.  */

	/* output_amount = initial_amount - deducted_amount.  */
	if (!amount_bro_sub(&output_amount,
			    initial_amount, deducted_amount))
		/* If underflow, force to 0.  */
		output_amount = AMOUNT_BRO(0);

	/* If output exceeds max, return max.  */
	if (amount_bro_less(max_output_amount, output_amount))
...
static void hsm_sign_local_htlc_tx(struct brocoin_tx *tx,
				   const u8 *wscript,
				   struct brocoin_signature *sig)
{
...
new_tracked_output(struct tracked_output ***outs,
		   ...
		   const u8 *wscript,
		   const struct brocoin_signature *remote_htlc_sig TAKES)
{
	struct tracked_output *out = tal(*outs, struct tracked_output);

	status_debug("Tracking output %s: %s/%s",
		     type_to_string(tmpctx, struct brocoin_outpoint, outpoint),
		     tx_type_name(tx_type),
	...
	out->remote_htlc_sig = tal_dup_or_null(out, struct brocoin_signature,
					       remote_htlc_sig);
...
static void proposal_should_rbf(struct tracked_output *out)
{
	...
	if (out->proposal->tx_type == OUR_PENALTY_TX) {
		...
		u32 my_depth = depth;
		size_t weight = brocoin_tx_weight(out->proposal->tx);
		struct amount_bro initial_amount;
		struct amount_bro new_amount;
		...
		brocoin_tx_output_get_amount_bro(out->proposal->tx, 0,
						 &initial_amount);
		...
		new_amount = compute_penalty_output_amount(initial_amount,
							   my_depth, max_depth,
							   weight);
		assert(amount_bro_less_eq(new_amount, initial_amount));
...
static void proposal_meets_depth(struct tracked_output *out)
{
	...
	status_debug("Broadcasting %s (%s) to resolve %s/%s",
		     tx_type_name(out->proposal->tx_type),
		     type_to_string(tmpctx, struct brocoin_tx, out->proposal->tx),
		     tx_type_name(out->tx_type),
		     output_type_name(out->output_type));
			       ...
			       enum tx_type tx_type)
{
	status_debug("Propose handling %s/%s by %s (%s) after %u blocks",
		     ...
		     output_type_name(out->output_type),
		     tx_type_name(tx_type),
		     tx ? type_to_string(tmpctx, struct brocoin_tx, tx):"IGNORING",
		     depth_required);
...
static bool is_local_commitment(const struct brocoin_txid *txid,
				const struct brocoin_txid *our_broadcast_txid)
{
	return brocoin_txid_eq(txid, our_broadcast_txid);
}
...
static void unwatch_txid(const struct brocoin_txid *txid)
{
	u8 *msg;
...
static void handle_htlc_onchain_fulfill(struct tracked_output *out,
					const struct tx_parts *tx_parts,
					const struct brocoin_outpoint *htlc_outpoint)
{
			    ...
			    u32 tx_blockheight)
{
	...
	u8 *wscript = brocoin_wscript_htlc_tx(htlc_tx, to_self_delay[LOCAL],
			  ...
			  const struct brocoin_outpoint *htlc_outpoint)
{
	...
	struct amount_asset asset;
	struct amount_bro htlc_out_amt;

	u8 *wscript = brocoin_wscript_htlc_tx(htlc_tx, to_self_delay[REMOTE],
					      &keyset->self_revocation_key,
					      &keyset->self_delayed_payment_key);

	asset = wally_tx_output_get_amount(htlc_tx->outputs[htlc_outpoint->n]);
	assert(amount_asset_is_main(&asset));
	htlc_out_amt = amount_asset_to_bro(&asset);
...
static void onchain_annotate_txout(const struct brocoin_outpoint *outpoint,
...
static void onchain_annotate_txin(const struct brocoin_txid *txid, u32 innum,
				  enum wallet_tx_type type)
{
			 ...
			 u32 tx_blockheight)
{
	for (size_t i = 0; i < tal_count(*outs); i++) {
		...
		struct brocoin_outpoint htlc_outpoint;
...
static void tx_new_depth(struct tracked_output **outs,
			 const struct brocoin_txid *txid, u32 depth)
{
	size_t i;

	/* Special handling for commitment tx reaching depth */
	if (brocoin_txid_eq(&outs[0]->resolved->txid, txid)
	    && depth >= reasonable_depth
	    && missing_htlc_msgs) {
	...
	for (i = 0; i < tal_count(outs); i++) {
		/* Update output depth. */
		if (brocoin_txid_eq(&outs[i]->outpoint.txid, txid))
			outs[i]->depth = depth;

		/* Is this tx resolving an output? */
		if (outs[i]->resolved) {
			if (brocoin_txid_eq(&outs[i]->resolved->txid, txid)) {
		 ...
		 * resolution for? */
		if (outs[i]->proposal
		    && brocoin_txid_eq(&outs[i]->outpoint.txid, txid)
			    ...
			    const struct preimage *preimage)
{
	...
	for (i = 0; i < tal_count(outs); i++) {
		...
		if (outs[i]->remote_htlc_sig) {
			struct amount_mbro htlc_amount;
			if (!amount_bro_to_mbro(&htlc_amount, outs[i]->bro))
				status_failed(STATUS_FAIL_INTERNAL_ERROR,
			...
			witness = brocoin_witness_htlc_success_tx(
...
{
	struct brocoin_tx *tx = NULL;
	struct brocoin_signature localsig;
	size_t i;
	struct amount_mbro htlc_amount;
	u8 **witness;

	if (!amount_bro_to_mbro(&htlc_amount, out->bro))
		status_failed(STATUS_FAIL_INTERNAL_ERROR,
	...
	if (i == tal_count(matches)) {
		...
		status_failed(STATUS_FAIL_INTERNAL_ERROR,
			      ...
			      type_to_string(tmpctx, struct amount_bro,
					     &out->bro),
			      type_to_string(tmpctx, struct brocoin_signature,
					     out->remote_htlc_sig),
			      cltvs, wscripts,
			      option_anchor_outputs
			      ? "option_anchor_outputs" : "");
	...
	witness = brocoin_witness_htlc_timeout_tx(tx, &localsig,
						  out->remote_htlc_sig,
						  htlc_scripts[matches[i]]);
...
static void get_anchor_scriptpubkeys(const tal_t *ctx, u8 **anchor)
{
	...
	for (enum side side = 0; side < NUM_SIDES; side++) {
		u8 *wscript = brocoin_wscript_anchor(tmpctx,
						     &funding_pubkey[side]);
...
static void handle_our_unilateral(const struct tx_parts *tx,
				  ...
				  const struct brocoin_signature *remote_htlc_sigs,
				  struct tracked_output **outs)
{
	u8 **htlc_scripts;
...
static void steal_to_them_output(struct tracked_output *out, u32 csv)
{
	 ...
	 *    <revocation_sig> 1
	 */
	wscript = brocoin_wscript_to_local(tmpctx, to_self_delay[REMOTE], csv,
					   &keyset->self_revocation_key,
					   &keyset->self_delayed_payment_key);

	tx = tx_to_us(tmpctx, penalty_to_us, out, BROCOIN_TX_RBF_SEQUENCE, 0,
...
static void steal_htlc(struct tracked_output *out)
{
	...
	tx = tx_to_us(out, penalty_to_us, out, BROCOIN_TX_RBF_SEQUENCE, 0,
...
int main(int argc, char *argv[])
{
	...
	struct brocoin_txid our_broadcast_txid;
	struct brocoin_signature *remote_htlc_sigs;
	...
	trim_maximum_feerate(funding_bros, tx);
