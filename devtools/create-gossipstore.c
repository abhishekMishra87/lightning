...
/* read scid,bronees csv file and create return an array of scidbro pointers */
static struct scidbro *load_csv_file(FILE *scidf)
{
	...
	if (r != 2 || strcmp(title, "scid") != 0 || strcmp(&title[6], "bronees") != 0)
		err(1, "reading 'scid ,bronees' from csv failed");

        while(fscanf(scidf, "%s ,%"SCNu64"\n", str, &scidbros[i].bro.bronees) == 2 ) { /* Raw: read from file */
...
int main(int argc, char *argv[])
{
	...
	opt_register_arg("--bro", opt_set_charp, NULL, &cbro,
			 "default bronee value if --csv flag not present");
	...
	opt_parse(&argc, argv, opt_log_stderr_exit);
		...
		if (!parse_amount_bro(&bro, cbro, strlen(cbro))) {
		        errx(1, "Invalid bronee amount %s", cbro);
		}
	}
	else {
