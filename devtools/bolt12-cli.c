...
static void print_chains(const struct brocoin_blkid *chains)
{
	printf("chains:");
	for (size_t i = 0; i < tal_count(chains); i++) {
		printf(" %s", type_to_string(tmpctx, struct brocoin_blkid, &chains[i]));
	}
	printf("\n");
}

static void print_chain(const struct brocoin_blkid *chain)
{
	printf("chain: %s\n",
	       type_to_string(tmpctx, struct brocoin_blkid, chain));
}

static bool print_amount(const struct brocoin_blkid *chains,
			 const char *iso4217, u64 amount)
{
	 ...
	 *     lightning-payable unit (e.g. milli-bronees for brocoin).
	 * - otherwise:
			...
			if (blindedpay) {
				if (bp_idx < tal_count(blindedpay))
					printf("fee=%u/%u,cltv=%u,features=%s",
					       blindedpay[bp_idx]->fee_base_mbro,
