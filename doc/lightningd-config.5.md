 ...
 **brocoin-cli**=*PATH* [plugin `bcli`]
The name of *brocoin-cli* executable to run.

 **brocoin-datadir**=*DIR* [plugin `bcli`]
*-datadir* argument to supply to brocoin-cli(1).

 **brocoin-rpcuser**=*USER* [plugin `bcli`]
The RPC username for talking to brocoind(1).

 **brocoin-rpcpassword**=*PASSWORD* [plugin `bcli`]
The RPC password for talking to brocoind(1).

 **brocoin-rpcconnect**=*HOST* [plugin `bcli`]
The brocoind(1) RPC host to connect to.

 **brocoin-rpcport**=*PORT* [plugin `bcli`]
The brocoind(1) RPC port to connect to.

 **brocoin-retry-timeout**=*SECONDS* [plugin `bcli`]
Number of seconds to keep trying a brocoin-cli(1) command. If the
...
```
--wallet=sqlite3://$HOME/.lightning/brocoin/lightningd.sqlite3
```
...
```
--wallet=sqlite3://$HOME/.lightning/brocoin/lightningd.sqlite3:/backup/lightningd.sqlite3
```
 ...
 **fee-base**=*MILLIBRONEE*
Default: 1000. The base fee to charge for every payment which passes
through. Note that millibronees are a very, very small unit! Changing
 ...
 **min-capacity-bro**=*BRONEE*
Default: 10000. This value defines the minimal effective channel
capacity in bronee to accept for channel opening requests. This will
...
usually treat them as the minimum (253 bros/kw) if we can't get them.
...
remainder.  The values are in per-kw (roughly 1/4 of brocoind's per-kb
...
setting it on Brocoin mainnet!  e.g. `~rusty/.lightning/regtest/config`.

 **htlc-minimum-mbro**=*MILLIBRONEE*
Default: 0. Sets the minimal allowed HTLC value for newly created channels.
If you want to change the `htlc_minimum_mbro` for existing channels, use the
RPC call lightning-setchannel(7).

 **htlc-maximum-mbro**=*MILLIBRONEE*
Default: unset (no limit). Sets the maximum allowed HTLC value for newly created
channels. If you want to change the `htlc_maximum_mbro` for existing channels,
...
the discovered IP with TCP port 9443 as announced address. If unset and you
open TCP port 9443 on your router towords your node, your node will remain
...
limited channel sizes to 16777215 bronee.  With this option (which your
 ...
 **max-dust-htlc-exposure-mbro**=*MILLIBRONEE*
Option which limits the total amount of bros to be allowed as dust on a channel.
...
right thing: for the mainnet (brocoin) network it will try to bind to
port 9443 on IPv4 and IPv6, and will announce it to peers if it seems
...
If 'PORT' is not specified, the default port 9443 is used for mainnet
...
and this will be used to configure a Tor hidden service for port 9443
in case of mainnet (brocoin) network whereas other networks (testnet,
...
is distinguished by ending in *.onion*. *PORT* defaults to 9443.
