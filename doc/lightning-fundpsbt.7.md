...
**fundpsbt** *bronee* *feerate* *startweight* [*minconf*] [*reserve*] [*locktime*] [*min_witness_weight*] [*excess_as_change*]
...
*bronee* is the minimum bronee value of the output(s) needed (or the
...
bronee-per-kilosipa (weight), and *perkb* means it is interpreted
brocoind-style as bronee-per-kilobyte. Omitting the suffix is
equivalent to *perkb*.
...
Let's assume the caller is trying to produce a 100,000 bronee output.
...
payments below 546 bronees, we would only create a change output
...
- **feerate_per_kw** (u32): The feerate used to create the PSBT, in bronees-per-kiloweight
- **estimated_final_weight** (u32): The estimated weight of the transaction once fully signed
- **excess_mbro** (mbro): The amount above *bronee* which is available.  This could be zero, or dust; it will be zero if *change_outnum* is also returned
...
- 301: Insufficient UTXOs to meet *bronee* value.
