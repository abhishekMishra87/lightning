...
You can use the `listfunds` command and take a ratio of `our_amount_mbro` over
`amount_mbro`. Note that this doesn't account for the [channel reserve](https://github.com/lightningnetwork/lightning-rfc/blob/master/02-peer-protocol.md#rationale).
...
If you need a lot of inbound liquidity, you can use a service that trustlessly swaps on-chain Brocoin
...
For this to work binhind a NAT router you need to forward the TCP port 9443 to your node.
...
### Can I use a single `brocoind` for multiple `lightningd` ?

Yes. All `brocoind` calls are handled by the bundled `bcli` plugin. `lightningd` does not use
`brocoind`'s wallet. While on the topic, `lightningd` does not require the `-txindex` option on `brocoind`.

If you use a single `brocoind` for multiple `lightningd`'s, be sure to raise the `brocoind`
max RPC thread limit (`-rpcthreads`), each `lightningd` can use up to 4 threads, which is
the default `brocoind` max.
...
[see issue #3484](https://github.com/abhishekMishra87/lightning-network/issues/3484). Currently unusable.
...
In summary: as a Brocoin user, one may be familiar with a file or a seed
(or some mnemonics) from which
it can recover all its funds.

Core Lightning has an internal brocoin wallet, which you can use to make "on-chain"
...
- `rescanblockchain`: A `brocoind` RPC call which rescans the blockchain
...
-  `dev-rescan-outputs`: A `lightningd` RPC call. Only available if your node has been
   ...
   will sync the state for known UTXOs in the `lightningd` wallet with `brocoind`.
...
[mandelbit-recovery]: https://github.com/mandelbit/brocoin-tutorials/blob/master/CLightningRecoverFunds.md
