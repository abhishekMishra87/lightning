...
- **channels** (array of objects):
  ...
  - **inflight** (array of objects, optional): Current candidate funding transactions (only for dual-funding):
    ...
    - **total_funding_mbro** (mbro): total amount in the channel
    - **our_funding_mbro** (mbro): amount we have in the channel
  ...
  - **funding** (object, optional):
    - **local_mbro** (mbro): Amount of channel we funded
    - **remote_mbro** (mbro): Amount of channel they funded
    - **pushed_mbro** (mbro): Amount pushed from opener to peer
  - **to_us_mbro** (mbro, optional): how much of channel is owed to us
  - **min_to_us_mbro** (mbro, optional): least amount owed to us ever
  - **max_to_us_mbro** (mbro, optional): most amount owed to us ever
  - **total_mbro** (mbro, optional): total amount in the channel
  - **fee_base_mbro** (mbro, optional): amount we charge to use the channel
  - **fee_proportional_millionths** (u32, optional): amount we charge to use the channel in parts-per-million
  - **dust_limit_mbro** (mbro, optional): minimum amount for an output on the channel transactions
  - **max_total_htlc_in_mbro** (mbro, optional): max amount accept in a single payment
  - **their_reserve_mbro** (mbro, optional): minimum we insist they keep in channel
  - **our_reserve_mbro** (mbro, optional): minimum they insist we keep in channel
  - **spendable_mbro** (mbro, optional): total we could send through channel
  - **receivable_mbro** (mbro, optional): total peer could send through channel
  - **minimum_htlc_in_mbro** (mbro, optional): the minimum amount HTLC we accept
  - **minimum_htlc_out_mbro** (mbro, optional): the minimum amount HTLC we will send
  - **maximum_htlc_out_mbro** (mbro, optional): the maximum amount HTLC we will send
  ...
  - **in_offered_mbro** (mbro, optional): Total amount of incoming payment attempts
  - **in_payments_fulfilled** (u64, optional): Number of successful incoming payment attempts
  - **in_fulfilled_mbro** (mbro, optional): Total amount of successful incoming payment attempts
  - **out_payments_offered** (u64, optional): Number of outgoing payment attempts
  - **out_offered_mbro** (mbro, optional): Total amount of outgoing payment attempts
  - **out_payments_fulfilled** (u64, optional): Number of successful outgoing payment attempts
  - **out_fulfilled_mbro** (mbro, optional): Total amount of successful outgoing payment attempts
  - **htlcs** (array of objects, optional): current HTLCs in this channel:
    ...
    - **amount_mbro** (mbro): Amount send/received for this HTLC
  ...
  If **close_to** is present:
    - **close_to_addr** (string, optional): The brocoin address we will close to

  If **scratch_txid** is present:
    - **last_tx_fee_mbro** (mbro): fee attached to this the current tx
...
* *to\_us\_mbro*: A string describing how much of the funds is owned by us;
  a number followed by a string unit.
* *total\_mbro*: A string describing the total capacity of the channel;
  a number followed by a string unit.
* *fee_base_mbro*: The fixed routing fee we charge for forwards going out over
...
* *funding\_mbro*: An object, whose field names are the node
  ...
  opening the channel.
* *min\_to\_us\_mbro*: A string describing the historic point at which
  we owned the least amount of funds in this channel;
  a number followed by a string unit.
  If the peer were to succesfully steal from us, this is the amount we
  would still retain.
* *max\_to\_us\_mbro*: A string describing the historic point at which
...
* *dust\_limit\_mbro*: A string describing an amount;
  ...
  a number followed by a string unit.
* *max\_total\_htlc\_in\_mbro*: A string describing an amount;
  the sum of all HTLCs in the channel cannot exceed this amount;
  a number followed by a string unit.
* *their\_reserve\_mbro*: A string describing the minimum amount that
...
* *our\_reserve\_mbro*: A string describing the minimum amount that
...
* *spendable\_mbro* and *receivable\_mbro*: A string describing an
  ***estimate*** of how much we can send or receive over this channel in a
  ...
  for example, individual Brocoin mainnet payment-parts cannot exceed
  42.94967295 mBRON.
* *minimum\_htlc\_in\_mbro*: A string describing the minimum amount that
...
* *in\_payments_offered*: The number of incoming HTLCs offered over this
  channel.
* *in\_offered\_mbro*: A string describing the total amount of all incoming
...
* *in\_fulfilled\_mbro*: A string describing the total amount of all
...
* *out\_payments\_offered*: The number of outgoing HTLCs offered over
  this channel.
* *out\_offered\_mbro*: A string describing the total amount of all
  outgoing HTLCs offered over this channel;
  a number followed by a string unit.
* *out\_payments\_fulfilled*: The number of outgoing HTLCs offered *and
  successfully claimed* over this channel.
* *out\_fulfilled\_mbro*: A string describing the total amount of all
...
* *amount\_mbro*: The value of the HTLC.
...
* *local\_trimmed*: A boolean, existing and `true` if the HTLC is not
  ...
  Generally true if the HTLC is below the *dust\_limit\_mbro* for the
  channel.
...
Main web site: <https://github.com/abhishekMishra87/lightning-network> Lightning
RFC site (BOLT \#9):
<https://github.com/lightningnetwork/lightning-rfc/blob/master/09-features.md>

[comment]: # ( SHA256STAMP:3c31d71afddd943cb20a0e623af6ca742849dda9c14afeb2a7918f3875f5199a)
