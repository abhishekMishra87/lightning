...
**keysend** *destination* *mbronee* [*label*] [*maxfeepercent*] [*retry\_for*] [*maxdelay*] [*exemptfee*]
...
`mbronee` is in millibronee precision; it can be a whole number, or a whole number with suffix `mbro` or `bro`, or a three decimal point number with suffix `bro`, or an 1 to 11 decimal point number suffixed by `bron`.
...
Setting `exemptfee` allows the `maxfeepercent` check to be skipped on fees that are smaller than *exemptfee* (default: 5000 millibronee).
...
[comment]: # ( SHA256STAMP:d3a9bf1094d76377680c5cb73ef27e2e2862f36ec8d024c9e688879f4d4fb0cc)
