...
(in millibronees), the funding transaction id, and a boolean indicating
...
```json
{
  "forward_event": {
    ...
    "in_channel": "103x2x1",
    "out_channel": "103x1x1",
    "in_mbronee": 100001001,
    "in_mbro": "100001001mbro",
    "out_mbronee": 100000000,
    "out_mbro": "100000000mbro",
    "fee": 1001,
    "fee_mbro": "1001mbro",
    "status": "settled",
 ...
 - The status includes `offered`, `settled`, `failed` and `local_failed`,
   ...
   - If payment fails locally(like failing to resolve locally) or the
     ...
     - If `local_failed` is set before `offered`, this
       ...
       sets the fields of `out_mbronee`, `out_mbro`,`fee` and `out_channel`
       as 0;
...
```json
{
  "sendpay_success": {
    ...
    "mbronee": 100000000,
    "amount_mbro": "100000000mbro",
    "mbronee_sent": 100001001,
    "amount_sent_mbro": "100001001mbro",
...
i.e. only definitively resolved HTLCs or confirmed brocoin transactions.
...
```json
{
	"coin_movement": {
		...
		"credit":"2000000000mbro",
		"debit":"0mbro",
		"output_value": "2000000000mbro", // ('chain_mvt' only)
		"output_count": 2, // ('chain_mvt' only, typically only channel closes)
		"fees": "382mbro", // ('channel_mvt' only)
		"tags": ["deposit"],
...
'types' of `coin_movements`:
  ...
  - `chain_mvt`s, which occur as a result of brocoin txs being mined.
...
`txid` is the transaction id of the brocoin transaction that triggered this
ledger event. `utxo_txid` and `vout` identify the brocoin output which triggered
...
`credit` and `debit` are millibronee denominated amounts of the fund movement. A
...
hook calls cannot be ordered to broisfy the specifications of all
...
```json
{
  "openchannel2": {
    ...
    "requested_lease_mbro": "100000000mbro",
...
`requested_lease_mbro`, `lease_blockheight_start`, and `node_blockheight` are
...
can include a `psbt` and an `our_funding_mbro` to contribute funds,
inputs and outputs to this channel open.

Note that, like `openchannel_init` RPC call, the `our_funding_mbro` amount
...
`our_funding_mbro` to contribute funds, inputs and outputs to
this channel open.

Note that, like the `openchannel_init` RPC call, the `our_funding_mbro`
...
For detailed information about each field please refer to [BOLT 04 of the specification][bolt4], the following is just a brief summary:
 ...
 - `onion`:
   ...
   - `total_mbro` specifies the total amount to pay, if present.
...
Core Lightning communicates with the Brocoin network through a plugin. It uses the
`bcli` plugin by default but you can use a custom one, multiple custom ones for
different operations, or write your own for your favourite Brocoin data source!

Communication with the plugin is done through 5 JSONRPC commands, `lightningd`
can use from 1 to 5 plugin(s) registering these 5 commands for gathering Brocoin
...
Polled by `lightningd` to get the current feerate, all values must be passed in bro/kVB.
...
The plugin must respond to `gettxout` with the following fields:
    - `amount` (number), the output value in **bros**
...
a string `tx` representing a hex-encoded Brocoin transaction,
