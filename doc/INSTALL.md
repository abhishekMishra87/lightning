...
* pip3: to install python-brocoinlib
* valgrind: for extra debugging checks

You will also need a version of brocoind with segregated witness and `estimatesmartfee` with `ECONOMICAL` mode support, such as the 0.16 or above.
...
If you don't have Brocoin installed locally you'll need to install that
as well. It's now available via [snapd](https://snapcraft.io/brocoin-core).
...
Clone lightning:
    ...
    git clone https://github.com/abhishekMishra87/lightning-network.git
    cd lightning
...
Make sure you have [brocoind](https://github.com/brocoin/brocoin) available to run
...
Configure brocoind, if not already: add `rpcuser=<foo>` and `rpcpassword=<bar>`
...
`/usr/local/etc/lightningd-brocoin.conf` and edit according to your needs.
    ...
    # service brocoind start
    # service lightningd start
    # lightning-cli --rpc-file /var/db/c-lightning/brocoin/lightning-rpc --lightning-dir=/var/db/c-lightning help
...
If you don't have brocoind installed locally you'll need to install that
as well:
    ...
    $ ./autogen.sh
    $ ./configure
    $ make src/brocoind src/brocoin-cli && make install
...
**Note**: Edit your `~/Library/Application\ Support/Brocoin/brocoin.conf`
...
A working example that compiles both brocoind and Core Lightning for Armbian can
be found [here](https://github.com/Sjors/armbian-brocoin-core).
...
Additional steps
--------------------
Go to [README](https://github.com/abhishekMishra87/lightning-network/blob/master/README.md) for more information how to create an address, add funds, connect to a node, etc.
