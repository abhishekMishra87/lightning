...
{
	assert(chan->bro.bronees == (unsigned long)chan); /* Raw: dev-hack */
}
#endif
...
#if DEVELOPER
	chan->bro.bronees = (unsigned long)chan; /* Raw: dev-hack */
#endif
	tal_free(chan);
}
...
static u8 *check_channel_announcement(const tal_t *ctx,
	...
	const struct pubkey *brocoin1_key, const struct pubkey *brocoin2_key,
	const secp256k1_ecdsa_signature *node1_sig,
	const secp256k1_ecdsa_signature *node2_sig,
	const secp256k1_ecdsa_signature *brocoin1_sig,
	const secp256k1_ecdsa_signature *brocoin2_sig, const u8 *announcement)
{
	...
	if (!check_signed_hash(&hash, brocoin1_sig, brocoin1_key)) {
		return towire_warningfmt(ctx, NULL,
					 "Bad brocoin_signature_1 %s hash %s"
					 " on channel_announcement %s",
					 type_to_string(tmpctx,
							secp256k1_ecdsa_signature,
							brocoin1_sig),
	...
	if (!check_signed_hash(&hash, brocoin2_sig, brocoin2_key)) {
		return towire_warningfmt(ctx, NULL,
					 "Bad brocoin_signature_2 %s hash %s"
					 " on channel_announcement %s",
					 type_to_string(tmpctx,
							secp256k1_ecdsa_signature,
							brocoin2_sig),
...
static void add_channel_announce_to_broadcast(struct routing_state *rstate,
					      ...
					      u32 index)
{
	u8 *addendum = towire_gossip_store_channel_amount(tmpctx, chan->bro);
				      ...
				      struct peer *peer)
{
	...
	secp256k1_ecdsa_signature brocoin_signature_1, brocoin_signature_2;
	u8 *features;
	struct brocoin_blkid chain_hash;
	struct short_channel_id scid;
	struct node_id node_id_1;
	struct node_id node_id_2;
	struct pubkey brocoin_key_1;
	struct pubkey brocoin_key_2;
	...
	if (!fromwire_channel_announcement(
		    ...
		    &brocoin_signature_1, &brocoin_signature_2, &features, &chain_hash,
		    &scid, &node_id_1, &node_id_2, &brocoin_key_1, &brocoin_key_2))
		return false;
				...
				struct peer *peer)
{
	...
	if (!fromwire_channel_announcement(pending, pending->announce,
					   ...
					   &pending->node_id_1,
					   &pending->node_id_2,
					   &pending->brocoin_key_1,
					   &pending->brocoin_key_2)) {
	...
	if (!brocoin_blkid_eq(&chain_hash, &chainparams->genesis_blockhash)) {
		status_peer_debug(peer ? &peer->id : NULL,
		    ...
		    type_to_string(pending, struct short_channel_id,
				   &pending->short_channel_id),
		    type_to_string(pending, struct brocoin_blkid, &chain_hash));
		goto ignored;
	}
	...
	if (warn) {
		/* BOLT #7:
		 *
		 * - if `brocoin_signature_1`, `brocoin_signature_2`,
	 ...
	 * catching up with the brocoin chain, though, they can definitely
	 * pile up. */
				  ...
				  const u8 *outscript)
{
	 ...
	 *     (using `brocoin_key_1` and `brocoin_key_2`, as specified in
	 *    [BOLT #3](03-transactions.md#funding-transaction-output)) ...
	 *    - MUST ignore the message.
	 */
	s = scriptpubkey_p2wsh(pending,
			       brocoin_redeem_2of2(pending,
						   &pending->brocoin_key_1,
						   &pending->brocoin_key_2));
	...
	if (!routing_add_channel_announcement(rstate, pending->announce, bro, 0,
				...
				bool ignore_timestamp)
{
	...
	u16 expiry;
	struct amount_mbro htlc_minimum, htlc_maximum;
	u32 fee_base_mbro;
	...
	if (!fromwire_channel_update(update, &signature, &chain_hash,
				     ...
				     &expiry, &htlc_minimum, &fee_base_mbro,
				     &fee_proportional_millionths))
		return false;
	/* If it's flagged as containing the optional field, reparse for
	 * the optional field */
	if ((message_flags & ROUTING_OPT_HTLC_MAX_MBRO) &&
	...
	if (message_flags & ROUTING_OPT_HTLC_MAX_MBRO) {
		/* Reject update if the `htlc_maximum_mbro` is greater
		 * than the total available channel bronees */
		if (amount_mbro_greater_bro(htlc_maximum, bro))
			return false;
	} else {
		/* If not indicated, set htlc_max_mbro to channel capacity */
		if (!amount_bro_to_mbro(&htlc_maximum, bro)) {
			  ...
			  bool force)
{
	...
	if (!brocoin_blkid_eq(&chain_hash, &chainparams->genesis_blockhash)) {
		status_peer_debug(peer ? &peer->id : NULL,
				  ...
				  type_to_string(tmpctx, struct brocoin_blkid,
						 &chain_hash));
		return NULL;
	}
...
#if DEVELOPER
		c->bro = amount_bro((unsigned long)c);
#endif
		tal_free(c);
	}
