...
static bool brocoin_blkid_from_hex(const char *hexstr, size_t hexstr_len,
				   struct brocoin_blkid *blockid)
{
	struct brocoin_txid fake_txid;
	if (!brocoin_txid_from_hex(hexstr, hexstr_len, &fake_txid))
		return false;
...
{
	struct brocoin_blkid prev;
	struct sha256_double merkle;
	struct brocoin_txid txid, expected_txid;
	struct brocoin_block *b;

	common_setup(argv[0]);
	chainparams = chainparams_for_network("brocoin");
	b = brocoin_block_from_hex(NULL, chainparams,
				   block, strlen(block));

	assert(b);
	assert(b->hdr.version == CPU_TO_LE32(0x6592a000));
	brocoin_blkid_from_hex("0000000000000f31173e973bc00e452b1fac350066df7db2adec1e3224ea5bc1", strlen("0000000000000f31173e973bc00e452b1fac350066df7db2adec1e3224ea5bc1"), &prev);
	assert(brocoin_blkid_eq(&prev, &b->hdr.prev_hash));
	...
	brocoin_txid(b->tx[0], &txid);
	brocoin_txid_from_hex("14d86acd2158acd1f59ab77ab251e3f5073db905a7b2aed25d3ba7780c3d790c",
			      strlen("14d86acd2158acd1f59ab77ab251e3f5073db905a7b2aed25d3ba7780c3d790c"),
			      &expected_txid);
	assert(brocoin_txid_eq(&txid, &expected_txid));

	brocoin_txid(b->tx[1], &txid);
	brocoin_txid_from_hex("c261a53121cc9841f843e2e6e0cff337e4f3c5eee788c982a0bffe771ce69919",
			      strlen("c261a53121cc9841f843e2e6e0cff337e4f3c5eee788c982a0bffe771ce69919"),
			      &expected_txid);
	assert(brocoin_txid_eq(&txid, &expected_txid));

	brocoin_txid(b->tx[2], &txid);
	brocoin_txid_from_hex("80cea306607b708a03a1854520729da884e4317b7b51f3d4a622f88176f5e034",
