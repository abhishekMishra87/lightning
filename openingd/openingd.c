...
/* We always set channel_reserve_bronees to 1%, rounded down. */
static void set_reserve(struct state *state, const struct amount_bro dust_limit)
{
	state->localconf.channel_reserve
		= amount_bro_div(state->funding_bros, 100);

	/* BOLT #2:
	 *
	 * The sending node:
	 *...
	 * - MUST set `channel_reserve_bronees` greater than or equal to
         *   `dust_limit_bronees` from the `open_channel` message.
	 */
	if (amount_bro_greater(dust_limit,
			       state->localconf.channel_reserve))
		state->localconf.channel_reserve
			= dust_limit;
}
...
static bool setup_channel_funder(struct state *state)
{
	...
	if (!feature_negotiated(state->our_features,
	    ...
	    && amount_bro_greater(state->funding_bros,
				  chainparams->max_funding)) {
		status_failed(STATUS_FAIL_MASTER_IO,
			      "funding_bronees must be < %s, not %s",
...
static u8 *funder_channel_start(struct state *state, u8 channel_flags)
{
	...
	if (amount_bro_greater(state->remoteconf.dust_limit,
			       state->localconf.channel_reserve)) {
		negotiation_failed(state,
				   "dust limit %s"
	...
	if (!check_config_bounds(tmpctx, state->funding_bros,
				 state->feerate_per_kw,
					  ...
					  struct penalty_base **pbase)
{
	...
	*tx = initial_channel_tx(state, &wscript, state->channel,
	...
	status_debug("signature %s on tx %s using key %s",
		     type_to_string(tmpctx, struct brocoin_signature, sig),
...
static u8 *funder_channel_complete(struct state *state)
{
	...
	peer_billboard(false,
		       ...
		       type_to_string(tmpctx, struct brocoin_txid, &state->funding.txid));

	/* We recalculate the local_mbro from cached values; should
	 * succeed because we checked it earlier */
	if (!amount_bro_sub_mbro(&local_mbro, state->funding_bros, state->push_mbro))
		status_failed(STATUS_FAIL_INTERNAL_ERROR,
			      "push_mbro %s > funding %s?",
			      type_to_string(tmpctx, struct amount_mbro,
					     &state->push_mbro),
			      type_to_string(tmpctx, struct amount_bro,
					     &state->funding_bros));

	if (!funder_finalize_channel_setup(state, local_mbro, &sig, &tx,
					   &pbase))
		return NULL;
...
static u8 *fundee_channel(struct state *state, const u8 *open_channel_msg)
{
	...
	struct pubkey their_funding_pubkey;
	struct brocoin_signature theirsig, sig;
	struct brocoin_tx *local_commit, *remote_commit;
	...
	if (!feature_negotiated(state->our_features, state->their_features,
				OPT_LARGE_CHANNELS)
	    && amount_bro_greater(state->funding_bros, chainparams->max_funding)) {
		negotiation_failed(state,
				   "funding_bronees %s too large",
				   type_to_string(tmpctx, struct amount_bro,
						  &state->funding_bros));
		return NULL;
	}
	 ...
	 */
	if (amount_mbro_greater_bro(state->push_mbro, state->funding_bros)) {
		peer_failed_err(state->pps, &state->channel_id,
				"Their push_mbro %s"
				" would be too large for funding_bronees %s",
				type_to_string(tmpctx, struct amount_mbro,
					       &state->push_mbro),
				type_to_string(tmpctx, struct amount_bro,
					       &state->funding_bros));
	 ...
	 * - MUST set `channel_reserve_bronees` greater than or equal to
	 *   `dust_limit_bronees` from the `open_channel` message.
	 * - MUST set `dust_limit_bronees` less than or equal to
         *   `channel_reserve_bronees` from the `open_channel` message.
	 */
	if (amount_bro_greater(state->remoteconf.dust_limit,
			       state->localconf.channel_reserve)) {
		negotiation_failed(state,
	...
	if (amount_bro_greater(state->localconf.dust_limit,
			       state->remoteconf.channel_reserve)) {
		negotiation_failed(state,
				   "Our dust limit %s"
	...
	if (!check_tx_sig(local_commit, 0, NULL, wscript, &their_funding_pubkey,
		...
		peer_failed_err(state->pps, &state->channel_id,
				...
				type_to_string(tmpctx, struct brocoin_signature,
					       &theirsig),
				type_to_string(tmpctx, struct brocoin_tx, local_commit),
				type_to_string(tmpctx, struct pubkey,
					       &their_funding_pubkey));
	}
...
int main(int argc, char *argv[])
{
	...
	state->remoteconf.max_dust_htlc_exposure_mbro = AMOUNT_MBRO(0);
